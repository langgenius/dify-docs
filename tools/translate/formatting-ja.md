...
Same-page anchors keep the heading verbatim, full-width punctuation included: `## 例：コンテンツレビューワークフロー` → `(#例：コンテンツレビューワークフロー)`. Do not strip or hyphenate `：`/`・` (only English headings convert `:` and spaces to hyphens).
