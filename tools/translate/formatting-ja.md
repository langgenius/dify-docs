...
Same-page anchors keep CJK characters and full-width punctuation (`：`, `・`) as-is, not stripped or hyphenated: `## 例：コンテンツレビューワークフロー` → `(#例：コンテンツレビューワークフロー)`. Latin letters still lowercase and spaces still become hyphens, as in English headings.

### API Reference Cross-Links

In OpenAPI spec descriptions, cross-links carry the language prefix and English slugs — identical to the English URL except the prefix: `/ja/api-reference/{en-tag-kebab}/{en-summary-kebab}`. Translate the link text; never translate the URL segments.

| English | Japanese |
|:--------|:---------|
| `[Get Human Input Form](/en/api-reference/human-input/get-human-input-form)` | `[人間の入力フォームを取得](/ja/api-reference/human-input/get-human-input-form)` |
...
```mdx
> このドキュメントは AI によって自動翻訳されています。不正確な部分がある場合は、[英語版](/en/path/to/page) を参照してください。
```
