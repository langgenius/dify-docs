...
对话流应用中，工作流进度以 `workflow_started`、节点事件（`node_started` 和 `node_finished`，以及迭代和循环的相应变体）流式返回，回复则以 `message` 事件返回（设置 `reasoning_format: separated` 的 LLM 节点还会同时发送 `reasoning_chunk` 事件，承载模型的推理内容）。开启文本转语音（TTS）自动播放时，`tts_message_end` 跟随在结束事件之后。结束序列取决于运行结果：
