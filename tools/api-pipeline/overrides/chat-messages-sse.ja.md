...
New Agent アプリでは、回答は `message` テキストチャンクとしてストリーミングされ、モデルの推論とツール呼び出しは `agent_thought` イベントとして並行して届きます。`message_end` の `metadata` には `usage` が含まれ（アノテーションが一致した場合は `annotation_reply` も含まれます）、`retriever_resources` は含まれません。
...
| `agent_thought` | Agent、New Agent | 推論またはツール呼び出しの各ステップ | `position`、`thought`、`tool`、`tool_input`（JSON）、`observation`、`message_files` |
