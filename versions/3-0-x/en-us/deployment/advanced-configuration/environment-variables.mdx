---
title: Environment Variables Explanation
---

<Note>
  The variables listed in this document are for reference only. For a full list of environment variables, please refer to the [`.env.example`](https://github.com/langgenius/dify/blob/main/docker/.env.example) file.  If you can't find a specific variable in the `.values.yaml` file, please check the [FAQs](#faqs) for guidance.
</Note>
...
### How to add environment variables that are not found in the `values.yaml` file, but available in the `.env.example` file?

Please add or modify the variable in the `values.yaml` file followed by the example below. For instance, to add `ENV_FROM_COMMUNITY1` and `ENV_FROM_COMMUNITY2` to the `values.yaml` file:
