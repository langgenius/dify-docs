...
- 外部ユーザー：Dify エンタープライズ版ワークスペース外のユーザー
</Info>
...
- Application name（必須項目）：アプリケーション名、自由に設定可能です。
- Homepage URL（必須項目）：ホームページリンク、Dify エンタープライズプラットフォーム URL の入力を推奨します。
- Application description（任意項目）：アプリケーション説明です。
- Authorization Callback URL（必須項目）：コールバックリンク、取得方法は[下記](#2-コールバック-url-の取得)をご参照ください。

> GitHub の OAuth2 アプリケーション作成方法の詳細については、[GitHub ドキュメント](https://docs.github.com/en/apps/oauth-apps/building-oauth-apps/creating-an-oauth-app)を参照してください。

## 2. コールバック URL の取得

有効化する必要がある SSO 範囲に応じて、**コールバック URL が異なります**。システム管理者は、Dify エンタープライズのコールバック URL を、対応する GitHub OAuth2 アプリケーション内に入力して、作成プロセスを完了する必要があります。
