...
| The host is rejected | Only `https://` hosts are accepted; a host without a scheme defaults to `https://`. For a plain `http://` host, or an `https://` host with a self-signed certificate (local development only), add `--insecure`. |
