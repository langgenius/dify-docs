...
<Steps>
  ...
  <Tip>
    ...
    | --- | --- | --- |
    | `DIFY_VERSION` | The Dify release tag to install `difyctl` from, matching a tag on [Dify's Releases](https://github.com/langgenius/dify/releases) (no `v` prefix). Defaults to the latest release; set it to your server's release tag when your server isn't on the latest. | `1.15.0` |
    | `DIFYCTL_VERSION` | Pin a specific `difyctl` build, written without the `v` prefix. Used only when `DIFY_VERSION` is unset. | `0.1.0-alpha` |
