...
<Note>
`difyctl` is available for Dify 1.15.0 and later. Each build targets a specific Dify version, so install the one that matches your server (see `DIFY_VERSION` below).
</Note>
