...
{/* VERIFY before publish (release-gated): (1) the raw.githubusercontent.com one-liner URLs are what the scripts' own headers document — confirm they're the final public channel once the first difyctl-v* release exists; (2) confirm the asset names and target list against that real release; (3) optionally capture real installer output then. */}
