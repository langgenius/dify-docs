...
`difyctl` ships with each Dify release and is matched to a server version, so a version change on either side can leave the two out of sync. It surfaces two ways, both exiting `6`:
