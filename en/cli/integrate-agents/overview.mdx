...
The agent doesn't work from a hard-coded command list. `difyctl` describes itself through [machine-readable help](/en/cli/reference/help#machine-readable-help), and the agent reads that at runtime, so it always tracks the current CLI.
...
- **Reads skills** (Claude Code, Codex, and similar): [install the `difyctl` skill](/en/cli/integrate-agents/install-the-difyctl-skill). 

    One command writes a small file that walks the agent through that discovery flow, and it onboards itself from there. No integration code.
- **Doesn't read skills**: point it there yourself. Add a line to the agent's system prompt or instructions, such as:
    ...
    That's the hand-off the skill otherwise does for you.
