...
Sign in on the target machine as the account the agent should act as. The agent inherits everything that account can reach. For Dify Cloud, use `https://cloud.dify.ai` for `--host`; on a self-hosted deployment, use the [console API URL](/en/self-host/deploy/configuration/environments#console_api_url).
