...
Sign in on the target machine as the account the agent should act as. The agent inherits everything that account can reach. On a self-hosted deployment, use the [console API URL](/en/self-host/deploy/configuration/environments#console_api_url) for `--host`.
...
<Steps>
  <Step title="Sign in on a machine without an OS keychain">
    ...
    Before copying, confirm `tokens.yml` exists in the config directory (`~/.config/difyctl` by default). If it's missing, a keychain captured the token.
  </Step>
