...
- [Install](/en/cli/install) `difyctl` and [sign in](/en/cli/integrate-agents/auth-for-agent-deployments) on the machine the agent runs on, so it can reuse your session.
...
| Commands fail with exit code 4 | The agent has no session to reuse. [Sign in](/en/cli/integrate-agents/auth-for-agent-deployments) on its machine first. |
