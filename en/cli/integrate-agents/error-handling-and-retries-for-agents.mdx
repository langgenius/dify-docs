...
Keep agent-side retries for the transient errors above, cap the attempts, and log every retry decision. An agent that silently re-runs writes is the failure mode the [`effect` labels](/en/cli/reference/help#machine-readable-help) exist to prevent.

Every command carries the tag `read`, `write`, or `destructive`, in the compact command map and in its own descriptor alike, so your loop can gate auto-retry on the tag and never re-send a `write` blindly.
