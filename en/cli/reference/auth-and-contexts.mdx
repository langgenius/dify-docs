...
For the step-by-step sign-in walkthrough, see Authenticate for [account users](/en/cli/account-users/authenticate) or [SSO users](/en/cli/sso-users/authenticate).
...
If your organization uses SSO, the same device flow routes through your corporate identity provider and produces an `external SSO` context (a `dfoe_` token) instead of a standard account context; see [SSO Users: Authenticate](/en/cli/sso-users/authenticate) for SSO specifics.
