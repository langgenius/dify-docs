...
| `DIFY_WORKSPACE_ID` | The workspace commands run against when no `--workspace` flag is set. Must be a UUID. See [How difyctl Picks a Workspace](/en/cli/reference/workspaces#how-difyctl-picks-a-workspace) for the full resolution order. | none |
