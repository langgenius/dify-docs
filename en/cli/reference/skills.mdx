...
The skill deliberately lists no commands: it points the agent at `difyctl help -o json --compact` and per-command help, so it discovers the live command surface itself.

See the [help reference](/en/cli/reference/help#machine-readable-help) for those discovery surfaces, and [Install the difyctl Skill](/en/cli/integrate-agents/install-the-difyctl-skill) for the full integration.
