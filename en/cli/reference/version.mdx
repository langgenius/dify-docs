...
- **`too_old`**: the command stops with exit [`6`](/en/cli/reference/output-formats-and-exit-codes) before sending any request, and tells you to upgrade the Dify server to at least the minimum this build supports (or [install the `difyctl` that matches your server](/en/cli/install)).
- **`too_new`**: the command runs, and prints a one-line warning to stderr.
- **`unknown`**: the command runs; there is nothing to gate on.

A passing check is cached per host for about an hour, so `difyctl` doesn't re-probe on every command. Upgrading the server clears it at once.
...
`--check-compat` makes the verdict scriptable: anything other than `compatible`, including every `unknown` case, exits `64`. The check probes the server live every time, so a scripted gate always reflects the server's current version rather than a cached result.
