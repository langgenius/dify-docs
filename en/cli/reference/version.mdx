...
Any server that clears the minimum-version gate (`compatible` or `too_new`) is cached per host for about an hour, so `difyctl` doesn't re-probe on every command. A `too_old` server is never cached, so it's re-checked each time and starts working as soon as you upgrade it.
