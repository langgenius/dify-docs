...
<Tip>
    If there's a ready-made trigger plugin for your target external system, we recommend using the [plugin trigger](/en/documentation/nodes/trigger/plugin-trigger) instead.
</Tip>
...
<Tip>
    ...
    When the parallel branches contain identical consecutive nodes, you can add a [Variable Aggregator](/en/documentation/nodes/variable-aggregator) node to merge the branches before the common section, without duplicating the same nodes across each branch.
</Tip>
