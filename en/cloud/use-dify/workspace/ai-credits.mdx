...
For an app request, Workflow run, or other task, the total is the combined cost of all model runs it triggers. Each model has a fixed credit cost, regardless of input or output length.

For example, one Workflow run uses a 5-credit LLM three times to analyze three customer reviews and once to summarize them. The run uses 20 AI credits in total.
