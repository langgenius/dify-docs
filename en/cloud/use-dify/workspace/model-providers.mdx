...
Create an API key in the provider's account or developer console, then add it to Dify to access models through that account.
...
3. Click **Add API Key**, enter the key, and complete any other fields required by the provider.
...
Requests made with your API key are tracked and charged by the model provider. They do not deduct AI credits. For providers that do not support AI credits, an API key is the only option.
...
Click **Add Model** on the provider's card, then complete the model details and credentials in the form. Required fields depend on the provider and model. Providers that serve only a fixed set of models don't offer this option.
