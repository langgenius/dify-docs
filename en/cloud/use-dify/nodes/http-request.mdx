...
### Pay-per-call (x402) APIs

Some third-party APIs sell access **per call** using the `x402` scheme. When such an API is called without payment, it responds with HTTP `402 Payment Required` and a small JSON body describing what the call costs. The caller reads the price, completes a payment, and retries the request — no account signup, no API key, no subscription.

The HTTP Request node exposes the response `Status Code` and `Response Body` as variables, so a workflow can branch on a `402` response and route to a payment step. A pay-per-call endpoint returns a body such as:
...
The `accepts[]` array lists the payment methods the seller accepts; the caller picks one, pays the listed `target`, and retries the original request. The flow is documented by sellers and wallets, for example [OpenSea's x402 docs](https://docs.opensea.io/docs/x402) and [MetaMask's explainer](https://www.metamask.io/news/what-is-x402). Because the status code and body are available as node variables, you can use an `IF/ELSE` node to detect the `402`, extract the payment requirement with the variable extractor, and route to your own payment step before retrying the request.
