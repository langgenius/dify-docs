...
<Tabs>
  ...
  **Memory Management** balance context retention with token efficiency based on your use case requirements.
  ...
  <Info>
  The new Agent node is in beta and available in Workflow apps only.
  </Info>

  The new Agent node runs an [agent](/en/cloud/use-dify/build/new-agent/overview) as one step of a workflow. Unlike the classic node's model-with-tools setup, the agent arrives as a complete worker with its own capabilities and sandbox.
  ...
  <Tabs>
    ...
    <Tab title="Start from Scratch">
      Create a one-time agent that lives on this node alone, and configure it right here, the same way you [build](/en/cloud/use-dify/build/new-agent/build) in **Agents**.
  ...
  Each file the agent returns is capped at 50 MB.
