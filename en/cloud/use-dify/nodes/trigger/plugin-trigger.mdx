...
Once the trigger works as expected, click **Publish** > **Publish Update** to make it live. Until the app is published, real events from the external system won't start the workflow—for example, a Lark/Feishu event delivery fails with an `app not online` error even though the trigger is configured correctly.
