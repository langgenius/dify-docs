...
1. When you add a webhook trigger to your workflow, a unique webhook URL is generated—a dedicated endpoint that listens for external HTTP requests. This URL is hosted on the Dify Cloud domain and is publicly reachable by external systems.
...
2. You use this URL to create a webhook subscribing to the events you want to monitor in an external system. Then you configure the webhook trigger to define how it processes incoming requests and extracts request data.
    ...
    <Note>
        ...
        <Frame>
          ![Test Webhook URL](/images/use-dify/workflow/test-webhook-url.png)
        </Frame>
...
<Tip>
    If there's a ready-made trigger integration for your target external system, we recommend using the [Integration Trigger](/en/cloud/use-dify/nodes/trigger/plugin-trigger) instead.
</Tip>
...
<Tip>
    A workflow can have multiple webhook triggers. If these trigger branches share identical downstream nodes, add a [Variable Aggregator](/en/cloud/use-dify/nodes/variable-aggregator) to converge them and avoid duplicating those nodes on each branch.
</Tip>
