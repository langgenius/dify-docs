...
<Info>
  Sandbox accounts are limited to 2 triggers per workflow. Higher trigger counts are available on <Badge color="blue">Professional</Badge> and <Badge color="blue">Team</Badge>. [Learn more](https://dify.ai/pricing).
</Info>
...
## Publish Triggers to Production

Triggers run against the published version of your workflow. Once your workflow works as expected in test runs, click **Publish** > **Publish Update** to make it live.

<Note>
  A test run only puts triggers into a temporary listening state. Until the app is published, real events from external systems won't start the workflow—for example, a Lark/Feishu event delivery fails with an `app not online` error even though the trigger is configured correctly.
</Note>

## Enable or Disable Triggers

To pause a published trigger, open the app's **Access Point** tab and switch it off on the **Trigger** card. While a trigger is off, its events are ignored.

<Note>
  Only published triggers appear on the **Trigger** card. If you don't see an added trigger listed, ensure it has been published first.
</Note>
