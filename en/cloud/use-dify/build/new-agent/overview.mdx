...
An *Agent* is an AI worker you set up once and then put to work. It's a different kind of agent than the [Legacy Agent app](/en/cloud/use-dify/build/agent):
...
- You [build](/en/cloud/use-dify/build/new-agent/build) it **once** and use it **two ways**: on its own as a chat app, or as [a step](/en/cloud/use-dify/nodes/agent#new-agent) inside a workflow.
...
You invite the agent into an [Agent node](/en/cloud/use-dify/nodes/agent#new-agent) to handle one step of a larger process. There you set only the task you want done. It's like asking a colleague to take care of one thing: you describe the task, and they bring their own skills to it.
