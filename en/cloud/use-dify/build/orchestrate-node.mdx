...
For nodes that should run multiple times (once per item in a list or until a condition is met), place them inside an [Iteration](/en/cloud/use-dify/nodes/iteration) or [Loop](/en/cloud/use-dify/nodes/loop) node.