...
<Info>
  Restoring a version, exporting a specific version's DSL, and running a specific published version through the [Service API](/en/api-reference/workflow-runs/run-workflow-by-id) are available on paid plans. On the Sandbox plan, apps always run the latest published version.
</Info>
