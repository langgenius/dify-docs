...
description: Prepare and validate a .difypkg, submit it for Marketplace review, respond to the pull request, and review the published plugin's performance and feedback in Creator Center
---
...
<Tabs>
  ...
  <Tab title="Dependencies">
    Keep dependencies minimal and specify version constraints so installs are reproducible. Direct URL and Git-based dependency installs are not allowed.
...
Your package must include a `PRIVACY.md` file that describes what user data the plugin collects, stores, logs, or sends to third parties. You can also link to a hosted privacy policy from `manifest.yaml`. If the plugin collects no user data, say so explicitly.
...
## Handle PR Checks and Review {#after-you-submit}
...
## Review Plugin Performance and Feedback in Creator Center {#manage-published-plugins}
...
After Creator Center verifies ownership of the **Unique handle**, plugins published under it appear in the organization. Invite teammates to view the same plugins without connecting their own GitHub accounts.
...
Track the request in **Claim history**. After approval, all plugins published under that handle appear under the corresponding organization. If the claim is rejected, review the reason and select **New Claim** to update the evidence and resubmit it.
