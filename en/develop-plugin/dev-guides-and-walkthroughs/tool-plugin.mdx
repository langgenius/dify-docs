...
- **`parameters`**: The parameter list.
  ...
  - **`type`** (required): Parameter type. Values fall into three groups:
    ...
    - **Pickers and uploads**: `secret-input` for sensitive values, which renders as an encrypted input box; `file` and `files` for uploads; `app-selector` and `model-selector` for choosing an app or a model from the workspace.
