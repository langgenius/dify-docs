...
- **`parameters`**: The parameter list.
  ...
  - **`type`** (required): Parameter type. Values fall into three groups:
    - **Plain values**: `string`, `number`, `boolean`, `any`.
    - **Choices**: `select` for a fixed dropdown built from `options`, `dynamic-select` for a dropdown whose options the plugin supplies at configuration time, and `checkbox`.
    - **Pickers and uploads**:
        - `secret-input` for sensitive values, which renders as an encrypted input box
        - `file` and `files` for uploads
        - `app-selector` and `model-selector` for choosing an app or a model from the workspace
        - `date` for a single date, which renders as a date picker and is passed as a `YYYY-MM-DD` string
        - `date-range` for a start and end date, which renders as a date range picker and is passed as an object with `start` and `end` date strings

    Two further values, `array` and `object`, exist for MCP server tools rather than plugin-declared parameters, and `system-files` is deprecated.
...
To publish the plugin to Dify Marketplace, make sure it meets the [Marketplace preparation and validation requirements](/en/develop-plugin/publishing/marketplace-listing/prepare-plugin-for-marketplace). After review, the package is merged into the main branch and released through the Marketplace pipeline.
