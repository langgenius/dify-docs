...
Endpoints are HTTP interfaces exposed by a plugin for integration with external systems. This guide explains their structure using the Neko Cat project as an example. For complete plugin code samples, see the [official plugin examples](https://github.com/langgenius/dify-official-plugins).
