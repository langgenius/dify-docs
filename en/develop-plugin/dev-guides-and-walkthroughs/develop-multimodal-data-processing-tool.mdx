---
title: 构建支持多模态数据输出的工具插件
---

在知识流水线中，知识库节点支持接收两种多模态数据格式：`multimodal-Parent-Child` 和 `multimodal-General`。

开发用于数据处理的工具插件时，若希望插件能够输出包含文字、图片、音视频等多模态数据并被知识库节点正确识别并向量化，需要完成以下配置：
...
可根据其结构，将文件信息（如 `name`, `size`, `extension`, `mime_type` 等）映射到多模态输出结构中的 `files` 字段。

<CodeGroup>
    ```yaml multimodal_parent_child_structure
    {
    ...
    ```yaml multimodal_general_structure
    {
...
## 在提供者 YAML 文件中声明多模态输出结构

多模态数据的结构通过 Dify 官方提供的 JSON Schema 进行约束。

为了让知识库节点识别插件的输出类型，需在插件的提供者 YAML 文件中将 `output_schema` 的 `result` 字段指向对应的官方 Schema URL。
