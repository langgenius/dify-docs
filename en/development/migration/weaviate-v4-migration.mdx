...
5. **Distance Calculation Fix:** Fixed bug where distance=0.0 was incorrectly converted to 1.0, causing perfect matches to be scored as worst matches
...
- **Critical Bug Fixes:** Resolved distance score calculation issues in vector search that affected search accuracy
...
### Issue: Empty Search Results or Incorrect Scores

**Cause:** Distance score calculation bug (fixed in weaviate-client v4.17.0+) or data not properly indexed.

**Solution:**

1. Verify you're on weaviate-client v4.17.0 or higher:
   ```bash
   docker compose exec api pip show weaviate-client
   ```

2. Re-index your knowledge base:
   - Go to Knowledge Base settings in Dify UI
   - Delete and re-upload documents
   - This ensures vectors are created with the new client

3. Check that search results now return proper distance scores (0.0 = perfect match)
...
✅ **Critical Bug Fixes:** Resolved distance calculation bug where distance=0.0 was incorrectly converted to 1.0
