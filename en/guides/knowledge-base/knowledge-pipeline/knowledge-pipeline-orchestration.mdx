...
<Warning>
Need help with authorization? Please check [Authorize Data Source](link) for detailed guidance on authorizing different data sources.
</Warning>
...
In this stage, these tools extract, chunk, and transform the content for optimal knowledge base storage and retrieval. Think of this step like meal preparation. We clean raw materials, chop them into bite-sized pieces, and organize everything, so the dish can be cooked up quickly when someone orders it.

<Warning>
Please check [Adding OAuth Support to Your Tool Plugin](link) for data processing tools setup.
</Warning>

### Doc Processor

Documents come in different formats—PDF, XLSX, DOCX. However, LLMs can't read these files directly. That's where extractors come in. They support multiple formats and handle the conversion, so your content is ready for the next step of the LLMs.

You can choose Dify's Doc Extractor to process files, or select tools based on your needs from the Marketplace which offers Dify Extractor and third-party tools like MinerU, Unstructured, and more.

#### Doc Extractor

As an information processing center, the document extractor node identifies and reads files from input variables, extracts information, and finally converts them into a format that works with the next node.
...
Similar to human limited attention span, large language models cannot process huge amounts of information simultaneously. Therefore, after information extraction, the chunker splits large document content into smaller and manageable segments (called "chunks").
...
By using a dual-layer segmentation structure to resolve the contradiction between context and accuracy, parent-child chunker achieves the balance between precise matching and comprehensive contextual information in Retrieval Augmented Generation (RAG) systems.
...
| Configuration Item          | Description                      |
|-----------------------------|----------------------------------|
| Column Number for Question  |                                  |
| Column Number for Answer    |                                  |
...
Input variables receive processing results from data processing nodes as the data source for the knowledge base. You need to connect the output from the chunker to the knowledge base as input.

The standard input for this node is `{x} Array[Chunk]`.

### Chunk Structure

![Chunk Structure](image-url)
...
|-------------------|--------------------|--------------------|-------------------------------------|
| General mode      | High Quality       | Embedding Model    | Vector Retrieval                    |
|                   |                    |                    | Full-text Retrieval                 |
|                   |                    |                    | Hybrid Retrieval                    |
|                   | Economy            | Number of Keywords | Inverted Index                      |
...
User input forms are essential for collecting the initial information your pipeline needs to run effectively. Similar to the [start node](/en/guides/workflow/node/start) in a workflow, this form gathers necessary details from users—such as files to upload, specific parameters for document processing—ensuring your pipeline has all the information it needs to deliver accurate results.
...
There are two ways to create a user input field:
...
2. **Node Parameter Panel**  
   Select a node. Then, in the parameter input on the right-side panel, click `+ Create user input` for new input items. New input items will also be collected in the Input Field.  
...
<Warning>
For more information about supported field types, please refer to the [Input Fields documentation](link).
</Warning>
...
2. **Import Test File**: Import files in the data source window that pops up on the right  
   ⚠️ **Important Note**: For better debugging and observation, only one file upload is allowed per test run.
...
- **Publish as a Knowledge Pipeline**  
  Publish this knowledge pipeline as a template. You can access this template in Create Knowledge Base Pipeline - Custom.