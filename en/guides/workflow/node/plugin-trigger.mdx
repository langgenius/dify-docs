...
2. Select an existing subscription or [create a new one](/en/guides/workflow/node/plugin-trigger#create-a-new-subscription).

    <Tip>
        You can view how many workflows use a specific subscription from the plugin's details panel under **Plugins**.
    </Tip>
...
Dify supports the following two ways to create subscriptions (webhooks). The available method depends on how the specific plugin is designed by its developer.
...
![Ways to Create Subscriptions](/images/create_subscription_method.png)

<Tip>
    It's recommended to select all available events when creating a subscription.
    
    A plugin trigger works only when its corresponding event is included in the linked subscription. Selecting all available events ensures that any plugin trigger you add to the workflow later can use the same subscription, without requiring another one.
</Tip>
                ...
                1. Select **Create with OAuth** > **Default** > **Save and Authorize**.
                    ...
                    <Info>
                        **Save** means the selected option is set as the default OAuth method for future subscriptions.
                        ...
                        ![OAuth Client Settings Icon](/images/oauth_client_settings_icon.png)
                    </Info>
