...
5.      Target variable data type: `Boolean`

        •       **Overwrite**: Directly overwrite the target variable with the source variable.

        •       **Clear**: Clear the contents of the selected target variable.

        •       **Set**: Choose between True or False without requiring a source variable.

6.      Target variable data type: `Array[Boolean]`

        •       **Overwrite**: Directly overwrite the target variable with the source variable.

        •       **Clear**: Clear the contents of the selected target variable.

        •       **Append**: Add a new True/False element to the array in the target variable.

        •       **Extend**: Add a new array of booleans to the target variable.

        •       **Remove**: Remove an element from the array, with options to remove from the first position (First) or the last position (Last).
