...
The Iteration node processes arrays by running the same workflow steps on each element sequentially. It's your solution for batch processing tasks that would otherwise hit limits or be inefficient as single operations.

Think of iteration as a production line for data - you feed in an array, and each item gets the same treatment through your defined workflow steps. This is essential for handling tasks that need to be broken down into smaller, manageable pieces.

## How It Works

The node takes an array input and creates a mini-workflow that runs once for each array element. During each iteration, the current item is available as variables that your internal nodes can reference.

**Core Components:**
- **Input Variables**: Array data from upstream nodes
- **Internal Workflow**: The steps to perform on each array element  
...
**Token Limit Management**: Break large texts into chunks for translation, summarization, or analysis when the full content exceeds LLM context windows.

**Batch Processing**: Apply the same LLM prompt, API call, or data transformation to multiple items efficiently.
...
**Streaming Results**: Output progress incrementally instead of waiting for all processing to complete.

## Example: Long Article Generator

This workflow demonstrates how to generate lengthy content by breaking it into manageable chapters:
...
**Process Flow:**
1. **Start Node**: User provides story title and high-level outline
2. **Content Planning**: LLM generates detailed chapter breakdown from user input
3. **Array Preparation**: Code node converts chapter list into proper array format
4. **Iteration Processing**: For each chapter, generate detailed content using LLM
5. **Streaming Output**: Answer node inside iteration provides real-time progress

**Why This Works Better Than Single Processing:**
- Avoids token limits that would truncate a full book generation
- Maintains consistent quality across chapters
- Provides progress feedback to users
- Handles errors per chapter rather than failing entirely

## Implementation Architecture

In Dify's workflow engine, iteration nodes create isolated execution contexts. Each iteration runs with:
- **Loop variables**: Access to current array element and iteration index
- **Isolated state**: Variables don't leak between iterations unless explicitly passed
- **Error containment**: Individual iteration failures don't stop the entire process
- **Streaming capability**: Results can be output as they complete

This architecture enables robust batch processing while maintaining the flexibility to handle complex, multi-step operations on each array element.
...
[Edit this page](https://github.com/langgenius/dify-docs/edit/main/en/guides/workflow/node/iteration.mdx) | [Report an issue](https://github.com/langgenius/dify-docs/issues/new?title=Documentation%20Issue%3A%20t&body=%23%23%20Issue%20Description%0A%3C%21--%20Please%20briefly%20describe%20the%20issue%20you%20found%20--%3E%0A%0A%23%23%20Page%20Link%0Ahttps%3A%2F%2Fgithub.com%2Flanggenius%2Fdify-docs%2Fblob%2Fmain%2Fen/guides/workflow/node%2Fiteration.mdx%0A%0A%23%23%20Suggested%20Changes%0A%3C%21--%20If%20you%20have%20specific%20suggestions%20for%20changes%2C%20please%20describe%20them%20here%20--%3E%0A%0A%3C%21--%20Thank%20you%20for%20helping%20improve%20our%20documentation%21%20--%3E)
