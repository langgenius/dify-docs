...
If you need to use variables from other nodes in the code node, you must define the variable names in the `input variables` and reference these variables. The code node supports all data types including strings, numbers, booleans, objects, and arrays. You can refer to [Variable References](/en/guides/workflow/variables).
