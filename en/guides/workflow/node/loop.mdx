...
Use loops for scenarios where you need to refine, improve, or build upon previous results until you reach a satisfactory outcome or hit a stopping condition.

## Loop vs. Iteration: Key Differences

In Dify's architecture, these nodes serve different purposes:
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Each cycle depends on previous results</td>
      <td>Refinement, optimization, progressive improvement</td>
    </tr>
    <tr>
      <td><strong>Iteration</strong></td>
      <td>Each item processed independently</td>
      <td>Batch processing, parallel data transformation</td>
    </tr>
  </tbody>
</table>
...
**Content Refinement**: Start with a draft and progressively improve it through multiple LLM reviews and edits until quality meets your standards.

**Problem Solving**: Tackle complex problems by breaking them into steps, where each loop cycle tackles the next logical piece based on previous progress.

**Research Workflows**: Iteratively search, analyze, and refine research queries based on findings from each search cycle.

**Quality Assurance**: Repeatedly test and improve outputs until they pass validation criteria.

## Implementation Notes

In Dify's loop architecture, each cycle has access to:
- **Previous results**: All outputs from the last loop iteration
- **Loop counter**: Current iteration number for conditional logic
- **Exit conditions**: Boolean logic to determine when to stop
- **Maximum iterations**: Safety limit to prevent infinite loops

This enables sophisticated workflows that can adapt and improve their approach based on intermediate results.
