...
<Info>
  This quick start uses **Dify Cloud**, the fastest way to get going: free to start, AI credits included, and nothing to install. Prefer to run Dify yourself? [Self-host Dify](/en/self-host/deploy/quick-start/docker-compose), then follow the same steps on your own instance.
</Info>
...
<Steps>
  <Step title="Sign Up">
    ...
    <Info>
      The 200 AI Credits in the Sandbox plan are a one-time allocation and don't renew monthly.
    </Info>
  </Step>
  ...
  <Step title="Set Up the Model Provider">
    ...
    No API key is required while you're using Sandbox AI Credits. The plugin is ready to use once installed. You can also configure your own API key and use it instead.
