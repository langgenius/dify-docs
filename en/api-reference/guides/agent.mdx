...
title: Agent API
sidebarTitle: Agent
description: API for Agent apps, covering streaming chat messages, conversation management, file uploads, and app configuration
tag: New
---

<Info>
    Agent here means the new Agent app type (`agent` mode). The Legacy Agent app (`agent-chat`) is covered on [Chatbot and Legacy Agent](/en/api-reference/guides/chat).
</Info>

[Agent apps](/en/self-host/use-dify/build/new-agent/overview) run in `agent` mode: you send a message, stream the reply as it generates, and keep multi-turn context in conversations.
...
- **[Send Chat Message](/en/api-reference/chat-messages/send-chat-message)**: send a query to your Agent app. Streaming mode only; blocking mode returns a 400 `bad_request` error. 

    Reply text streams as `agent_message` deltas, batched at roughly half-second intervals rather than token by token.

    The closing `message_end` event reports token usage. It never includes knowledge-retrieval citations (`retriever_resources`).

    Two limits end a run early, and either one closes the stream with the `agent_run_limit_exceeded` error code:

    - **Time**: a run is stopped after 1 hour by default.

        For longer runs on self-hosted deployments, raise `APP_MAX_EXECUTION_TIME`, `WORKFLOW_MAX_EXECUTION_TIME`, and `DIFY_AGENT_RUN_TIMEOUT_SECONDS` above the duration you need. See [Environment Variables](/en/self-host/deploy/configuration/environments) for details.
    - **Model requests**: each run is capped at 500.
...
- **[Get App Meta](/en/api-reference/applications/get-app-meta)**: app metadata; the `tool_icons` map is empty for Agent apps.
- **[Get App WebApp Settings](/en/api-reference/applications/get-app-webapp-settings)**: the web app's site configuration, theme, and customization options.
