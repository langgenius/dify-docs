...
[Chatbot apps](/en/cloud/use-dify/build/chatbot) (`chat` mode) and [Agent apps](/en/cloud/use-dify/build/agent) (`agent-chat` mode) share the same endpoints to send messages, manage conversations, handle files, and retrieve app settings. Only the streamed reply differs: Chatbot apps stream the reply as `message` events, while Agent apps stream `agent_thought` and `agent_message` events.
