---
title: Chat App API
sidebarTitle: Chatbot and Legacy Agent
description: API for Chatbot and Legacy Agent apps, covering chat messages, conversation management, file uploads, voice, and annotations
---

[Chatbot apps](/en/cloud/use-dify/build/chatbot) (`chat` mode) and [Legacy Agent apps](/en/cloud/use-dify/build/agent) (`agent-chat` mode) share the same endpoints to send messages, manage conversations, handle files, and retrieve app settings. Only the streamed reply differs: Chatbot apps stream the reply as `message` events, while Legacy Agent apps stream `agent_thought` and `agent_message` events.
...
- **[Send Chat Message](/en/api-reference/chat-messages/send-chat-message)**: send a query to your Chatbot or Legacy Agent app. Blocking mode returns the full reply once it finishes; Legacy Agent apps support streaming mode only.
