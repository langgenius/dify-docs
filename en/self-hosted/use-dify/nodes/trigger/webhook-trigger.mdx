...
<Tip>
    If there's a ready-made trigger plugin for your target external system, we recommend using the [plugin trigger](/en/self-hosted/use-dify/nodes/trigger/plugin-trigger) instead.
</Tip>
...
<Tip>
    A workflow can have multiple webhook triggers. If these trigger branches share identical downstream nodes, add a [Variable Aggregator](/en/self-hosted/use-dify/nodes/variable-aggregator) to converge them and avoid duplicating those nodes on each branch.
</Tip>
