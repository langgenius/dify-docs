...
| Summary Auto-Gen        | Uses an LLM to automatically generate summaries for document chunks.<Tip>Once enabled, this only applies for newly added document and chunks. For existing chunks, select the document(s) in the document list and click **Generate summary**.</Tip>|
