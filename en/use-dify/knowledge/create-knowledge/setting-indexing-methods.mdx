...
<Tabs>
  <Tab title="High Quality">
    ...
    Once embedded, the chunks can be retrieved using one of three strategies: vector search, full-text search, or hybrid search. Learn more in [Configure the Retrieval Settings](#configure-the-retrieval-settings).
