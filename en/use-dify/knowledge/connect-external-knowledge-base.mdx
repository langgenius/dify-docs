...
2. Fill in the following fields:
    ...
    - **API Endpoint**: Your external knowledge service's retrieval URL. See the [External Knowledge API specification](/en/use-dify/knowledge/external-knowledge-api) for the required endpoint format.
...
2. Fill in the following fields:
    ...
    - **External Knowledge API**: Select the API connection you registered. Dify routes all retrieval requests through this API.
    ...
    - **Retrieval Settings**:
      ...
      - **Score Threshold**: Minimum similarity score for returned chunks. Enable this to filter out low-relevance results. Use higher thresholds for stricter relevance or lower thresholds to include broader matches. 

          When disabled, all results up to the Top K limit are returned regardless of score.

Once created, the external knowledge base is available for use in your applications just like any built-in knowledge base. See [Integrate Knowledge Within Application](/en/use-dify/knowledge/integrate-knowledge-within-application) for next steps.

## Connection Example

### LlamaCloud

The [LlamaCloud plugin](https://marketplace.dify.ai/plugin/langgenius/llamacloud) on Dify Marketplace connects to LlamaCloud knowledge bases without requiring you to build a custom API. Install the plugin and follow the configuration wizard to enter your LlamaCloud API key.

The following video walks through the LlamaCloud plugin setup:
...
<Info>
To understand how the LlamaCloud plugin implements the external knowledge base connection, see its [source code on GitHub](https://github.com/langgenius/dify-official-plugins/tree/main/extensions/llamacloud).
</Info>

## Troubleshoot

### Self-Hosted: Connection Refused or Timeout
...
If retrieval fails or returns unexpected results, verify your API response against the [External Knowledge API specification](/en/use-dify/knowledge/external-knowledge-api#response-elements). 
