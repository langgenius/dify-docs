...
<Steps>
  <Step title="Add the Node">
    Let's go back to your Email Assistant Workflow. Hover over the line between the Start and LLM nodes. Click the **Plus (+)** icon and select the **Knowledge Retrieval** node.
  ...
  <Step title="Connect Knowledge Base">
    Click the node, and head to the right panel. Click the **plus (+)** button next to **Knowledge** to add knowledge.
  ...
  <Step title="Configure Query Text">
    ...
    Stay at the panel, navigate to **Query text** above, and select `email_content`. By doing this, we are telling AI: "Take the customer's message and use it as a search keyword to flip through our cookbook and find the matching info." Without a query, the AI is just staring at a closed book.
...
<Steps>
  <Step title="Add Context">
    Click the **LLM Node**. You'll see a new section called **Context**. Click it and select **result** from the Knowledge Retrieval node.
  ...
  <Step title="Update the Prompt">
    We need to tell the AI to generate reply based on the context. In **System**, add additional requirement **Generate response based on** `/` and select **Context**.
