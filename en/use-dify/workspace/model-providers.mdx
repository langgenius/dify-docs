...
|Tag|<div style={{width: '200px'}}>Description</div>|
|:-|:-|
|`LLM` / `Rerank` / `Text Embedding`/`Moderation`/`TTS`/`Speech2Text`<br></br><br></br>(Model type)| **LLM** models<br></br><br></br>**Rerank** models reorder knowledge retrieval results.<br></br><br></br>**Text Embedding** models convert document chunks into vector representations for semantic search.<br></br><br></br>**Moderation** models detect and filter inappropriate content.<br></br><br></br>**TTS** models convert text to speech.<br></br><br></br>**Speech2Text** models convert speech to text.|
|`Chat` / `Completion`<br></br><br></br>(LLM type)| **Chat** models handle conversations and follow complex instructions. They support multi-role prompts (System/User/Assistant).<br></br><br></br>**Completion** models are designed for simple text continuation and are generally older legacy models.|
|`8K` / `32k` / `128k`...<br></br><br></br>(Context window)| How much text the model can process at once—including your prompt, context, conversation history, and output.|
|`Vision` / `Audio`/ `Video` / `Document`<br></br><br></br>(Multimodal capabilities)| The input types the model can understand directly. `Vision` indicates image support.|
|`Tool Call`| Whether the model can natively call tools to complete complex tasks.<Info>Models without native support can still call tools through prompt engineering, though results may be less reliable.</Info>|
