...
1. Select the Knowledge Retrieval node's `result` variable as **Context**.

2. In the instruction, reference the `Context` variable.

3. (Optional) If the LLM is vision-capable, enable **Vision** so it can process image attachments in the retrieval results.

    <Info>
        No need to change the default input variable in the **Vision** field to `result` or  `Context`. The LLM will automatically access any retrieved images.
    </Info>
