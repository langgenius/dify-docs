...
- **Query Images**: Select an image variable, e.g., the image(s) uploaded by the user through a User Input node, to search by image. The image size limit is 10 MB.

    <Tip>
        For self-hosted deployments, you can adjust the image size limit via the environment variable `UPLOAD_IMAGE_FILE_SIZE_LIMIT`.
    </Tip>

    <Info>
        The **Query Image** option is available only when at least one multimodal knowledge base are added. 

        Such knowledge bases are marked with the **Vision** icon, indicating that they are using a multimodal embedding model.
    </Info>
...
<Info>
   Knowledge bases marked with the **Vision** icon support cross-modal retrieval, allowing the use of text and/or images as queries to retrieve semantically related text chunks and image attachments.
</Info>
...
- **Rerank Settings**
    ...
    <Note>
        If retrieved results may include image attachments, select a multimodal rerank model (indicated by the **Vision** icon). Otherwise, images will be excluded from reranking and the final output.
    </Note>
...
2. In the LLM prompt field, reference both the `Context` variable and the user input variable (e.g., `userinput.query` in Chatflows).
    ...
    <Info>
        LLMs that support **Vision** can directly understand any image attachments included in the retrieval results.
    </Info>
