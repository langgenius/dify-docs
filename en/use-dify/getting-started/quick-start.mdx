...
2. Inside the Iteration node, add an LLM node and configure it:
    ...
    2. In the system instruction field, paste the following:
        ...
        ```
    3. Click **Add Message** to add a user message, then paste the following. Type `{` or `/` to replace `Iteration/item` with the corresponding variable from the list.
...
1. Make sure your Checklist is clear.
    ...
    <Frame>
      ![Check Clecklist](/images/deeper_dive_checklist_clear.png)
    </Frame>
