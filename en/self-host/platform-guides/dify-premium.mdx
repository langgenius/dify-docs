...
Values in `/dify/.env` take precedence over values in any `/dify/envs/*.env` files. If your AWS deployment injects variables through `docker-compose.override.yaml` or another service-level environment configuration, keep those settings there; service-level environment values take precedence over values loaded from env files.

Restart Dify after changing environment variables:
...
<Warning>
    ...
    Dify's current Docker Compose files require Docker Compose 2.24.0 or later. Run `docker compose version` to confirm before upgrading.
</Warning>
...
<Steps>
    ...
    <Step title="Check for New Environment Variables">
        ...
        If you need to customize a newly added optional template, copy it beside itself without the `.example` suffix before editing it.
    </Step>
