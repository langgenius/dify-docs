...
The dedicated `api_websocket` container that serves collaboration traffic is tuned through the `API_WEBSOCKET_*` variables in [Container Startup Configuration](#container-startup-configuration).
...
| `NEXT_PUBLIC_ENABLE_FEATURE_PREVIEW` | `true` | Show in-development preview features in the console. Currently gates the command-palette `/create` and `/refine` slash commands for AI-assisted workflow and chatflow generation. Set to `false` to hide these preview features. |
