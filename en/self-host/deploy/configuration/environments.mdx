...
Shows the new Agent (beta) in the web UI: **Agent** appears in the main navigation, and the workflow node picker offers the new Agent node.

When off, the Agent page returns 404 and workflows get the classic Agent node only. Turning it on reveals the UI only; agents also need the agent runtime services running.

For source-code deployments, set `NEXT_PUBLIC_ENABLE_AGENT_V2` instead.
