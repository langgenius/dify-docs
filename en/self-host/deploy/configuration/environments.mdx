...
| `MARKETPLACE_ENABLED` | `true` | Controls whether Dify Marketplace is available for plugins. When disabled, the console hides Marketplace plugin browsing and installation, the API rejects Marketplace plugin installs and upgrades, and the periodic upgrade check (`ENABLE_CHECK_UPGRADABLE_PLUGIN_TASK`) stops.<br/><br/>Template import is unaffected. Already-installed plugins keep working, and installing from GitHub or a local package stays available. |
| `MARKETPLACE_API_URL` | `https://marketplace.dify.ai` | Marketplace API endpoint for browsing and downloading plugins and templates, and checking plugin upgrades.<br/><br/>The web service and users' browsers both use this URL, so both need network access to it. |
...
Creator Center (`creators.dify.ai`) is the submission portal where users upload Dify apps as templates. Submitted templates are reviewed and, once approved, listed on Dify Marketplace. Creator Center pages open in a new browser tab.
...
| `CREATORS_PLATFORM_OAUTH_CLIENT_ID` | (empty) | OAuth client ID for passwordless SSO to Creator Center. Leave it empty on self-hosted deployments. Publishing works without it; an ID not registered with the deployment's OAuth server makes **Publish to Marketplace** fail. |
