...
| `MARKETPLACE_API_URL` | `https://marketplace.dify.ai` | Marketplace API endpoint for browsing and downloading plugins and templates, and checking plugin upgrades.<br/><br/>The Dify API service, Celery workers, web service, and users' browsers use this URL, so it must be reachable from each environment. |
