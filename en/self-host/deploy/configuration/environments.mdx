...
| `MARKETPLACE_ENABLED` | `true` | Enable Dify Marketplace as a plugin and template source. When disabled, the console hides Marketplace browsing and install surfaces, the API rejects Marketplace installs and upgrades, and the periodic upgrade check (`ENABLE_CHECK_UPGRADABLE_PLUGIN_TASK`) stops.<br/><br/>Already-installed plugins keep working, and installing from GitHub or a local package stays available. |
| `MARKETPLACE_API_URL` | `https://marketplace.dify.ai` | Marketplace API endpoint for browsing, downloading, and upgrade checks on plugins and templates.<br/><br/>Called by both the API service and users' browsers (the console fetches Marketplace listings client-side), so both need network access to it. |
| `MARKETPLACE_URL` | (empty; the Compose stack falls back to `https://marketplace.dify.ai`) | Base URL for links that send users out to the Marketplace website (plugin detail pages and the Integrations page's Marketplace entry).<br/><br/>Distinct from `MARKETPLACE_API_URL`, which the console and plugin daemon use to fetch Marketplace data. |
...
Creator Center (`creators.dify.ai`) is the submission portal where users upload Dify apps as templates. Submitted templates are reviewed and, once approved, listed on Dify Marketplace.

The console's **Publish to Marketplace** action exports the app's DSL (its portable YAML definition), uploads it to Creator Center, and sends the user's browser there to complete the submission. See [Publish Apps to Marketplace](/en/self-host/use-dify/publish/publish-to-marketplace) for the user-facing flow.
...
| `CREATORS_PLATFORM_API_URL` | `https://creators.dify.ai` | Base URL of the Creator Center service. **Publish to Marketplace** uploads the app's DSL here from the API service, then redirects the user's browser to the same URL to complete the submission, so the API service and users' browsers both need to reach it. |
| `CREATORS_PLATFORM_OAUTH_CLIENT_ID` | (empty) | OAuth client ID that lets Creator Center attribute template submissions to the publishing Dify account. Leave it empty: publishing works without it, and an ID not registered with the deployment's OAuth server makes **Publish to Marketplace** fail. |
