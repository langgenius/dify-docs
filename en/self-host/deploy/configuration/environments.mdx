...
Deploy the Dify [1.16.0-rc1](https://github.com/langgenius/dify/releases) release to try out the New Agent. It runs as two extra services in the Docker Compose deployment, an agent backend and a sandbox, that start automatically with `docker compose up`. 
...
Default: (placeholder in `.env.example`; must be replaced)

Root secret the agent backend uses to sign and verify its internal tokens. It must be unpadded base64url text that decodes to exactly 32 bytes, and the shipped placeholder is invalid, so the agent backend refuses to start until you set a real value. 
...
Shows the New Agent in the web UI: the **Agent** item in the main navigation and the New Agent node in the workflow node picker. 
...
The remaining `DIFY_AGENT_*` variables (Redis, plugin daemon, and inner-API connections, plus run-retention and shutdown tuning) derive from your existing Compose values and don't normally need changes.
