...
| `MARKETPLACE_ENABLED` | `true` | Controls whether Dify Marketplace is available for plugins. When disabled, the console hides Marketplace plugin browsing and installation, the API rejects Marketplace plugin installs and upgrades, and the periodic upgrade check (`ENABLE_CHECK_UPGRADABLE_PLUGIN_TASK`) stops.<br/><br/>Template import itself is unaffected. However, if an imported template requires Marketplace plugins that are not already installed, those dependencies cannot be installed while Marketplace is disabled. Already-installed plugins keep working, and installing from GitHub or a local package stays available. |
| `MARKETPLACE_API_URL` | `https://marketplace.dify.ai` | Marketplace API endpoint for browsing and downloading plugins and templates, and checking plugin upgrades.<br/><br/>The Dify API service, web service, and users' browsers use this URL, so all three need network access to it. |
| `MARKETPLACE_URL` | (empty; the Compose stack falls back to `https://marketplace.dify.ai`) | Base URL for Marketplace links, including plugin detail pages and the Integrations page's Marketplace entry. Dify also uses it to generate Creator Center guide links.<br/><br/>Distinct from `MARKETPLACE_API_URL`, which the console and plugin daemon use to fetch Marketplace data. |
...
| `CREATORS_PLATFORM_OAUTH_CLIENT_ID` | (empty) | OAuth client ID for passwordless SSO to Creator Center. Leave it empty on self-hosted deployments. Publishing still works, but users log in to Creator Center manually. An ID not registered with the deployment's OAuth server makes **Publish to Marketplace** fail. |
