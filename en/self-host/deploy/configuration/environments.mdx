...
| `OPS_TRACE_RETRYABLE_DISPATCH_DELAY_SECONDS` | `5` | Seconds between those retries; the total tolerance window is delay × max retries (about 65 minutes at defaults, covering the one-hour workflow limit).<br/>Lower delivers nested traces sooner but exhausts the retry budget faster. Cannot be `0`. |
