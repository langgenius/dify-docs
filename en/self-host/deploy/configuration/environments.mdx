...
If the editor keeps dropping and reconnecting while collaborators work on a large workflow, raise `WEBSOCKET_MAX_HTTP_BUFFER_SIZE`.
...
| `DEPLOYMENT_EDITION` | `COMMUNITY` | Which edition this deployment runs as: `COMMUNITY`, `ENTERPRISE`, or `CLOUD`. Keep the default.<br/><br/>Replaces the former `EDITION`, `ENTERPRISE_ENABLED`, and `BILLING_ENABLED` variables, which are ignored from 1.17.0 onward. |
...
Where Dify keeps the per-workspace keys that encrypt stored credentials—model provider API keys, tool credentials, and other workspace secrets:

- `local`: generates an RSA key pair per workspace and keeps the private key in the `STORAGE_TYPE` backend.

- `azure-keyvault`: keeps each workspace's RSA key in Azure Key Vault instead, so the private key never leaves the vault; Dify authenticates with `DefaultAzureCredential` (managed identity, environment variables, or Azure CLI login).

<Warning>
Choose the provider before the first workspace is created. Credentials encrypted by one provider cannot be decrypted by the other, so switching on a deployment that already stores credentials makes them unreadable until re-entered.
</Warning>

| Variable | Default | Description |
|---|---|---|
| `AZURE_KEYVAULT_VAULT_URL` | (empty) | Key Vault instance URL. Required when `KEY_PROVIDER_TYPE` is `azure-keyvault`; the API service fails to start without it. Example: `https://your-vault-name.vault.azure.net` |
| `AZURE_KEYVAULT_KEY_SIZE` | `2048` | RSA key size in bits for the per-workspace keys Dify provisions in the vault. Applies only when a new key is created; existing keys keep their size. |
| `AZURE_KEYVAULT_ROTATION_INTERVAL_DAYS` | (empty) | When set (minimum 7), each newly provisioned per-workspace key gets a Key Vault auto-rotation policy that rotates it every N days. Old key versions stay usable, so credentials encrypted before a rotation keep decrypting without re-encryption. Leave empty to manage rotation yourself in Azure. |
...
| `APP_MAX_EXECUTION_TIME` | `3600` | Maximum time in seconds an app execution can run before it is terminated. <br/><br/>An outer limit alongside `WORKFLOW_MAX_EXECUTION_TIME`: both default to one hour, with this one applied at the app queue level and the other at the workflow engine level. Increase both together if your workflows need more time.<br/><br/>If you run agents, keep these outer limits at or above the agent run deadline (`DIFY_AGENT_RUN_TIMEOUT_SECONDS`); otherwise agent runs are cut short. |
...
| `WEBSOCKET_MAX_HTTP_BUFFER_SIZE` | `10485760` | Maximum collaboration WebSocket message size in bytes (10 MiB by default). Raise it if saving a large workflow causes repeated disconnections because the workflow graph exceeds this limit.<br/><br/>Set it on the API process serving `/socket.io/`: the `api_websocket` container in Docker Compose. |
...
| `HUAWEI_OBS_ACCESS_KEY` | (empty) | OBS access key. Required only when `HUAWEI_OBS_USE_OIDC` is `false`. |
| `HUAWEI_OBS_SECRET_KEY` | (empty) | OBS secret key. Required only when `HUAWEI_OBS_USE_OIDC` is `false`. |
...
| `HUAWEI_OBS_USE_OIDC` | `false` | Use the OBS SDK's built-in OIDC credential provider instead of static AK/SK credentials. Requires a Huawei Cloud IAM identity provider and a Kubernetes/CCE service account token. |
| `HUAWEI_CLOUD_IDP_ID` | (empty) | Huawei Cloud IAM identity provider ID. Required when `HUAWEI_OBS_USE_OIDC` is `true`. |
| `OIDC_TOKEN_FILE` | `/var/run/secrets/tokens/oidc-token` | Projected Kubernetes/CCE service account token file read by the OBS SDK. Set only when using a non-default token path. |
...
Separate S3-compatible storage for archiving workflow run logs. The `archive-workflow-runs` maintenance command packs runs older than a chosen cutoff into monthly Parquet bundles; CSV exports generated from those archives are staged in the export bucket.
...
| `ARCHIVE_STORAGE_EXPORT_BUCKET` | (empty) | Bucket for temporary CSV export ZIPs generated from archived logs. Safe to auto-expire with a bucket lifecycle rule. |
...
| `TIDB_VECTOR_ENABLE_FULLTEXT_SEARCH` | `false` | Unlocks the Full-Text Search and Hybrid Search retrieval options for knowledge bases on TiDB Vector.<br/><br/>New knowledge base tables get a multilingual full-text index; existing tables gain it the next time documents are indexed.<br/><br/>Requires a TiDB deployment that supports full-text indexes. |
...
| Variable | Default | Description |
|---|---|---|
| `ENABLE_LEARN_APP` | `true` | Shows the Learn Dify section: tutorial templates on the Explore page, the getting-started panel when Studio has no apps, and its show/hide toggle in the help menu.<br/><br/>Set to `false` to hide all three. |
| `ENABLE_SKILL` | `true` | Shows the [Skills](/en/self-host/use-dify/build/skills) entry in the main navigation.<br/><br/>Set to `false` to hide skills from the workspace UI. |
...
| `EMAIL_CODE_LOGIN_TOKEN_EXPIRY_MINUTES` | `5` | How long a sign-in code stays valid (in minutes). An expired code is rejected; request a new one to continue. |
| `EMAIL_CODE_LOGIN_MAX_ATTEMPTS` | `5` | How many wrong entries one sign-in code survives before it is invalidated and a new code must be requested. |
...
| Variable | Default | Description |
|---|---|---|
| `OPS_TRACE_RETRYABLE_DISPATCH_MAX_RETRIES` | `780` | How many times a trace delivery is retried after a transient failure before the trace is dropped. <br/><br/>The default retry window (retries × `OPS_TRACE_RETRYABLE_DISPATCH_DELAY_SECONDS`) is sized to outlast the one-hour workflow limit plus the export grace period. <br/><br/>Today only Arize/Phoenix tracing of nested workflows hits this path (a child trace arriving before its parent's context). Cannot be `0`. |
| `OPS_TRACE_RETRYABLE_DISPATCH_DELAY_SECONDS` | `5` | Seconds between those retries; the total tolerance window is delay × max retries (about 65 minutes at defaults).<br/><br/>Lower delivers nested traces sooner but exhausts the retry budget faster. Cannot be `0`. |
| `OPS_TRACE_PARENT_CONTEXT_TTL_SECONDS` | `3900` | How long in seconds a parent workflow's trace context stays available for nested child traces to attach to.<br/><br/>Keep it at or above the retry window (retries × delay). |
...
| `WORKFLOW_MAX_EXECUTION_TIME` | `3600` | Maximum wall-clock time in seconds per workflow run. Exceeding this terminates the workflow.<br/><br/>An outer limit paired with `APP_MAX_EXECUTION_TIME` (both default to one hour): increase both together. If you run agents, keep them at or above the agent run deadline (`DIFY_AGENT_RUN_TIMEOUT_SECONDS`). |
...
| `GRAPH_ENGINE_SCALE_UP_THRESHOLD` | `0` | How many tasks may wait for a free worker before the pool adds another, up to `GRAPH_ENGINE_MAX_WORKERS`.<br/><br/>At the default `0`, a worker is added as soon as any task waits; raise it to tolerate short backlogs before scaling up. |
...
| `MARKDOWN_FORM_FIELD_NAME_EXTRA_CHARS` | (empty) | Extra characters accepted in the `name` attributes of form fields that apps render from Markdown in chat responses.<br/><br/>By default a field name must start with a letter and may contain letters, digits, underscores, and hyphens; fields whose names use other characters are not rendered.<br/><br/>Set the additional characters as one string; both half-width and full-width characters work, e.g. `()！？`. |
| `MARKDOWN_FORM_FIELD_NAME_MAX_LENGTH` | `128` | Maximum length of the `name` attribute of form fields that apps render from Markdown in chat responses; fields with longer names are not rendered. Works alongside `MARKDOWN_FORM_FIELD_NAME_EXTRA_CHARS`, which governs the characters a name may contain.<br/><br/>Raise it if generated field names run past the limit and the rendered form is missing those fields. Must be a positive integer. |
...
The new Agent runs as extra services in the Docker Compose deployment: an agent backend, a sandbox, and a dedicated egress proxy for the sandbox. All start automatically with `docker compose up`. 
...
| `DIFY_AGENT_RUN_RETENTION_SECONDS` | `7200` | How long to keep each agent run's record and event stream in Redis, in seconds after its last write (2 hours by default). Lower it to free Redis memory sooner, at the cost of a shorter window for replaying events after a dropped connection. |
| `DIFY_AGENT_RUN_EVENT_STREAM_MAX_LENGTH` | `5000` | Approximate maximum number of events to keep in each agent run's Redis stream. Lower it to reduce Redis memory use per run.<br/><br/>New events trim the oldest ones once the limit is reached. Trimmed events cannot be replayed after a dropped connection. |
| `DIFY_AGENT_STREAM_TEXT_DELTA_COALESCING_ENABLED` | `true` | Batches consecutive model text deltas before writing them to Redis and streaming them. Keep enabled to reduce Redis writes and event counts.<br/><br/>Set to `false` to stream each delta separately. Tool calls and other non-text events remain unchanged and in order. |
| `DIFY_AGENT_STREAM_TEXT_DELTA_FLUSH_INTERVAL_MS` | `100` | Target interval in milliseconds for publishing batched text deltas. Lower it for more frequent text updates; raise it for fewer, larger batches.<br/><br/>This is not a strict timing limit. Batches can be published sooner when they reach `DIFY_AGENT_STREAM_TEXT_DELTA_MAX_CHARS`. Minimum: 1. To disable batching, set `DIFY_AGENT_STREAM_TEXT_DELTA_COALESCING_ENABLED` to `false`. |
| `DIFY_AGENT_STREAM_TEXT_DELTA_MAX_CHARS` | `4096` | Character threshold for publishing a text-delta batch before the flush interval elapses. Lower it to publish smaller batches sooner.<br/><br/>A batch can exceed this threshold because deltas are added whole. Minimum: 1. |
| `DIFY_AGENT_RUN_TIMEOUT_SECONDS` | `3600` | Wall-clock deadline for one agent run's model and tool work. A run that exceeds it fails with `agent_run_limit_exceeded`. Replaces the former `AGENT_BACKEND_RUN_TIMEOUT_SECONDS`.<br/><br/>The API-side `APP_MAX_EXECUTION_TIME` and `WORKFLOW_MAX_EXECUTION_TIME` (default `3600`) cap runs earlier if set below this.<br/><br/>On the `e2b` backend the sandbox's `DIFY_AGENT_E2B_ACTIVE_TIMEOUT_SECONDS` (ceiling 3600) still applies, so values above an hour only take effect on the `local` backend. |
...
| `DIFY_AGENT_OUTBOUND_HTTP_CONNECT_TIMEOUT` | `10` | Connect timeout in seconds for the agent service's outbound HTTP calls, such as tool and plugin traffic. |
| `DIFY_AGENT_OUTBOUND_HTTP_READ_TIMEOUT` | `600` | Read timeout in seconds; generous by default so slow tool responses and large downloads can finish. |
| `DIFY_AGENT_OUTBOUND_HTTP_WRITE_TIMEOUT` | `30` | Write timeout in seconds for outbound requests. |
| `DIFY_AGENT_OUTBOUND_HTTP_POOL_TIMEOUT` | `10` | How long in seconds a request waits for a free connection from the pool. |
| `DIFY_AGENT_OUTBOUND_HTTP_MAX_CONNECTIONS` | `100` | Connection pool size for the agent service's outbound HTTP. |
| `DIFY_AGENT_OUTBOUND_HTTP_MAX_KEEPALIVE_CONNECTIONS` | `20` | How many idle connections stay pooled for reuse. |
| `DIFY_AGENT_OUTBOUND_HTTP_KEEPALIVE_EXPIRY` | `30` | Seconds an idle pooled connection is kept before it closes. |
...
| `DIFY_AGENT_LOCAL_SANDBOX_ENDPOINT` | `http://local_sandbox:5004` | Address of the bundled sandbox service. Replaces the former `DIFY_AGENT_SHELLCTL_ENTRYPOINT`, which still works as a legacy alias. |
| `DIFY_AGENT_LOCAL_SANDBOX_AUTH_TOKEN` | (empty) | Optional bearer token securing the agent backend's calls to the sandbox; Compose passes the same value to both services. <br/><br/>Replaces the former `DIFY_AGENT_SHELLCTL_AUTH_TOKEN`, which still works as a legacy alias. |
| `EXPOSE_LOCAL_SANDBOX_PORT` | `5004` | Host port at which the middleware stack for [source-code deployments](/en/self-host/deploy/advanced-deployments/local-source-code) publishes the agent sandbox, so an agent backend run from source reaches it at `http://localhost:5004`. Set it in that stack's `middleware.env`; if you change it, point `DIFY_AGENT_LOCAL_SANDBOX_ENDPOINT` in the agent backend's `.env` at the new port.<br/><br/>Middleware stack only: the full Compose stack keeps the sandbox on an internal network. |
...
| `DIFY_AGENT_SANDBOX_FILES_BASE_URL` | `http://api:5001` | Dify API address the sandboxed agent uses for file transfers. Keep the default on Docker Compose. A remote sandbox backend needs your publicly reachable Dify address here. |
| `AGENT_BACKEND_BINDING_FILE_DOWNLOAD_TIMEOUT_SECONDS` | `240` | How long the API waits when fetching a file from an agent's sandbox before failing the transfer. Keep it above the agent-side `DIFY_AGENT_BINDING_FILE_DOWNLOAD_COMMAND_TIMEOUT_SECONDS`. |
| `AGENT_BACKEND_HOME_SNAPSHOT_TIMEOUT_SECONDS` | `45` | How long the API waits on agent backend calls that transfer a home snapshot: capturing or deleting one, and starting a run that restores one.<br/><br/>If applying a build or starting runs fails for agents with large home environments, increase this. |
| `DIFY_AGENT_BINDING_FILE_DOWNLOAD_COMMAND_TIMEOUT_SECONDS` | `210` | How long the sandbox command serving that file may run. The default stays below the API's timeout so the agent's own error surfaces first. |
| `DIFY_AGENT_STUB_UPLOAD_FILE_SIZE_LIMIT` | `50` | Maximum size in MiB for one file the sandboxed agent uploads back to Dify, such as the files it generates for a reply. |
...
| `DIFY_AGENT_SHELL_REDACT_PATTERNS` | (empty) | JSON array of regex patterns masked out of shell output before the agent reads it, for secrets like API keys. The agent's internal bearer token is always masked. |

### DIFY_AGENT_RUNTIME_BACKEND

Default: `local`

Selects where agent sandboxes run. The default `local` uses the bundled sandbox container. Set to `e2b` to run each agent's sandbox as an [E2B](https://e2b.dev) cloud VM instead, for stronger isolation than the bundled container provides.

The `e2b` backend reads its own group of variables, and the `docker/docker-compose.e2b.yaml` overlay adjusts the Compose stack for it:
...
| `MARKETPLACE_ENABLED` | `true` | Controls whether Dify Marketplace is available for plugins. When disabled, the console hides the entry points for Marketplace plugin browsing and installation, the API rejects Marketplace plugin installs and upgrades, and the periodic upgrade check (`ENABLE_CHECK_UPGRADABLE_PLUGIN_TASK`) stops.<br/><br/>Template import itself is unaffected. However, if an imported template requires Marketplace plugins that are not already installed, those dependencies cannot be installed while Marketplace is disabled. Already-installed plugins keep working, and installing from GitHub or a local package stays available when permitted by the installation policy. |
| `MARKETPLACE_API_URL` | `https://marketplace.dify.ai` | Marketplace API endpoint for browsing and downloading plugins and templates, and checking plugin upgrades.<br/><br/>The Dify API service, background workers, web service, and users' browsers use this URL, so it must be reachable from each environment. |
| `MARKETPLACE_URL` | (empty; the Compose stack falls back to `https://marketplace.dify.ai`) | Base URL for Marketplace links, including plugin detail pages and the Integrations page's Marketplace entry. Dify also uses it to generate Creator Center guide links.<br/><br/>Distinct from `MARKETPLACE_API_URL`, the endpoint for Marketplace data and packages. |
...
Creator Center (`creators.dify.ai`) is the submission portal where users upload Dify apps as templates. Submitted templates are reviewed and, once approved, listed on Dify Marketplace. The Dify console opens Creator Center pages in a new browser tab.

The console's **Publish to Marketplace** action exports the app's DSL (its portable YAML definition) and uploads it to Creator Center, where the user completes the submission. See [Publish Apps to Marketplace](/en/self-host/use-dify/publish/publish-to-marketplace) for the user-facing flow.

| Variable | Default | Description |
|---|---|---|
| `CREATORS_PLATFORM_FEATURES_ENABLED` | `true` | Controls whether the **Publish to Marketplace** action appears in the app's Publish panel. Set to `false` to hide the action; the underlying console API also returns 403. Useful on deployments where users shouldn't have a one-click path to submit templates externally. |
| `CREATORS_PLATFORM_API_URL` | `https://creators.dify.ai` | Base URL of the Creator Center service. **Publish to Marketplace** uploads the app's DSL here from the API service, and users' browsers access the same URL to complete the submission, so both need to reach it. |
| `CREATORS_PLATFORM_OAUTH_CLIENT_ID` | (empty) | OAuth client ID for passwordless SSO to Creator Center. Leave it empty on self-hosted deployments. Publishing still works, but users who are not already signed in must log in to Creator Center manually. An ID not registered with the deployment's OAuth server makes **Publish to Marketplace** fail. |
...
| `ENABLE_CLEAN_MESSAGES` | `false` | Delete conversation messages older than `SANDBOX_EXPIRED_RECORDS_RETENTION_DAYS` at 4:00 AM daily. |
...
| `ENABLE_CONVERSATION_CLEANUP_TASK` | `true` | Safety net for conversation deletion. A deleted conversation disappears instantly while a background job removes its messages and the files generated by tools during the conversation. Files end users uploaded are kept.<br/><br/>This sweep re-runs that job for conversations whose cleanup was lost, so their data does not linger. |
| `CONVERSATION_CLEANUP_TASK_INTERVAL` | `5` | How often the sweep runs (in minutes). |
| `CONVERSATION_CLEANUP_BATCH_SIZE` | `100` | Maximum conversations re-dispatched per sweep. |

#### Record Retention & Cleanup

These control the scheduled cleanup of old messages and workflow run records: anything older than the retention window is deleted in batches.
