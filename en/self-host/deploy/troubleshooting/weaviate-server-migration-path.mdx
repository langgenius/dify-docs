...
Dify pins the Weaviate server image it ships with. That pin is moving from `1.27.0` to `1.39.0`, and an existing Weaviate data volume cannot cross that gap in one step. Deploy Dify fresh and none of this applies to you—you get the pinned version on an empty volume.
...
Your objects are not lost. The graph is. Testing this on 1.39.0, a hard kill immediately after importing 500 objects into a 700-object collection left it like this:
