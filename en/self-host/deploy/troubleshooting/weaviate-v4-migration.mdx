...
<Info>
  Already on 1.27 and moving to a newer server? Dify's bundled Weaviate image is moving to 1.39, and crossing that gap means stepping through the minor releases in order. See the [Weaviate server upgrade path](/en/self-host/deploy/troubleshooting/weaviate-server-migration-path) for the ladder.
</Info>
