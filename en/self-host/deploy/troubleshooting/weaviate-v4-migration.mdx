...
> This guide explains how to migrate from Weaviate client v3 to weaviate-client v4 (v4.17.0 or later) and upgrade your Weaviate server from version 1.19.0 to 1.27.0 or higher. This migration is required for Dify versions that include the weaviate-client v4 upgrade.

<Info>
  Already on 1.27 and upgrading to Dify 1.17.1? Its bundled Weaviate image is `1.39.2`, and crossing that gap means stepping through the minor releases in order. See the [Weaviate server upgrade path](/en/self-host/deploy/troubleshooting/weaviate-server-migration-path) for the ladder.
</Info>
...
| 1.17.0 - 1.17.1 | v4.22.0                | 1.27.0 or higher                    |
...
For Docker installations, make sure you are running the correct Dify version. Run this from the `docker` directory:
...
For source installations, install the client version your checkout pins. Run this from the repository root, not from `docker`:
...
- [Dify Self-Hosting Guide](/en/self-host/deploy/quick-start/docker-compose)
- [Dify Environment Variables](/en/self-host/deploy/configuration/environments)
