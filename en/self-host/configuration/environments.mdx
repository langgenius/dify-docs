...
- WORKFLOW_LOG_CLEANUP_ENABLED
  ...
  Whether to enable automatic log cleanup task for workflows and chatflows. Default is false. When enabled, it cleans app execution logs that exceed the retention period at 2:00 AM daily.

- WORKFLOW_LOG_RETENTION_DAYS

  Log retention days for workflows and chatflows. Default is 30 days. Only effective when `WORKFLOW_LOG_CLEANUP_ENABLED` is `true`.

- WORKFLOW_LOG_CLEANUP_BATCH_SIZE

  Number of log entries deleted per cleanup task for workflows and chatflows; can be adjusted based on system performance. Default is 100.  Only effective when `WORKFLOW_LOG_CLEANUP_ENABLED` is `true`.
