...
Master switch for real-time collaboration on the Workflow canvas, covering simultaneous editing, in-canvas comments, and @mentions. When `true`, the frontend activates the collaborative UI.

Setting this alone is not sufficient. Collaboration runs over WebSocket, which also requires:

- `SERVER_WORKER_CLASS` set to `geventwebsocket.gunicorn.workers.GeventWebSocketWorker` so Gunicorn accepts WebSocket upgrades (see [Container Startup Configuration](#container-startup-configuration)).
- `NEXT_PUBLIC_SOCKET_URL` set to a WebSocket URL your browser can reach (see [CORS Configuration](#cors-configuration)).

**Operational notes:**

- **Reverse proxy**: if you terminate TLS or route traffic through your own nginx, traefik, or cloud load balancer (instead of Dify's bundled nginx), forward the `Upgrade` and `Connection` headers on the `/socket.io/` path. The bundled `docker/nginx/conf.d/default.conf.template` already includes this.
- **Multi-replica API**: the socket.io server keeps per-session state in memory. Running multiple API replicas requires sticky sessions at the load-balancer layer to avoid clients bouncing between replicas.
...
| `SERVER_WORKER_CLASS` | `gevent` | Gunicorn worker type. `gevent` provides lightweight async concurrency. Set to `geventwebsocket.gunicorn.workers.GeventWebSocketWorker` to accept WebSocket upgrades when `ENABLE_COLLABORATION_MODE` is `true`; left at `gevent`, the collaboration client fails the WebSocket handshake without a startup error. Other values break psycopg2 and gRPC patching and are not supported. |
...
| `NEXT_PUBLIC_SOCKET_URL` | `ws://localhost` | Browser-side WebSocket endpoint for real-time collaboration. When `ENABLE_COLLABORATION_MODE` is `true`, the Workflow canvas connects here for live editing and comments. Provide scheme + host + port only (the path `/socket.io/` is fixed by the client); swap `http(s)://` for `ws(s)://`. Example: `wss://dify.example.com`. The default `ws://localhost` resolves to port 80 in the browser, matching the bundled nginx. Applied via Dify's runtime env injection, so container restart picks up changes without an image rebuild. |
