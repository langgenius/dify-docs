...
| Variable | Default | Description |
|---|---|---|
| `S3_ENDPOINT` | (empty) | S3 endpoint address. Required for non-AWS S3-compatible services (MinIO, Tigris, etc.). |
| `S3_REGION` | `us-east-1` | S3 region. For Tigris, set to `auto`. |
...
| `S3_ADDRESS_STYLE` | `auto` | S3 addressing style: `auto`, `path`, or `virtual`. Set to `virtual` for services that require virtual-hosted-style requests (e.g. Tigris). |
...
To use [Tigris](https://www.tigrisdata.com) as the storage backend, create an access key at [console.tigris.dev](https://console.tigris.dev) and set:
...
S3_ACCESS_KEY=tid_YOUR_ACCESS_KEY
S3_SECRET_KEY=tsec_YOUR_SECRET_KEY
