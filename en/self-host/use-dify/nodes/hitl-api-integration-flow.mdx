...
<Steps>
  ...
  <Step title="(File inputs only) Upload local files">
    If the recipient attaches a local file to a `file` or `file-list` input, upload it first with [Upload File](/api-reference/files/upload-file), the standard Service API file endpoint. It returns an `id` you reference as `upload_file_id` in the submit payload.
