...
To create a loop variable:

1. Open the Loop node and locate the **Loop Variables** section.
2. Click **+** and set the variable **name**, **type**, and an initial **value**. The initial value applies when the loop starts.
3. Reference it from nodes inside the loop: loop variables appear in the variable picker under the Loop node.

To update a loop variable inside the loop body, add a **Variable Assigner** node, select the loop variable as the assigned variable, and choose an operation:

- **Set** / **Overwrite** replaces the value
- `+=`, `-=`, `*=`, `/=` update numeric counters
- **Append**, **Extend**, **Remove First**, **Remove Last**, **Clear** modify arrays

<Info>
  A reference inside a loop must point to a variable that exists in the loop scope, such as a loop variable or the output of a node inside the loop. If a node shows the "Invalid variable. Please select an existing variable." error, check that the loop variable is defined in the Loop Variables section and that the reference selects it.
</Info>
