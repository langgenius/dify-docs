...
<Tip>
    If there's a ready-made trigger integration for your target external system, consider using an [Integration Trigger](/en/self-host/use-dify/nodes/trigger/plugin-trigger) instead.
</Tip>
...
<Tip>
    A workflow can have multiple webhook triggers. If these trigger branches share identical downstream nodes, add a [Variable Aggregator](/en/self-host/use-dify/nodes/variable-aggregator) to converge them and avoid duplicating those nodes on each branch.
</Tip>
