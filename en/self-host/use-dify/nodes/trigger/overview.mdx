...
- [Integration Trigger](/en/self-host/use-dify/nodes/trigger/plugin-trigger)
  ...
  - Runs your workflow when a specific event occurs in an external system, via an event subscription through an integration.

  - Example: Automatically analyze and archive new messages in a specific Slack channel via a subscription to the `New Message in Channel` event through a Slack integration.
...
<Tip>
  ...
  2. Use a **webhook trigger** when no integration exists or when you need to capture events not covered by available integrations. In such cases, you'll need to set up custom webhooks in the external system.
</Tip>

## Enable or Disable Triggers

After publishing, each trigger appears on the app's **Monitoring** page under the **Triggers** card. Toggle a trigger between **In Service** and **Disabled** to control whether it initiates workflow runs.
