...
<Tabs>
  ...
  **Memory Management** balance context retention with token efficiency based on your use case requirements.
  ...
  In **Agent task**, describe what you need at this step, the way you'd brief a colleague on one job. This is separate from the agent's own role and prompt. Type `/` to pull in outputs from earlier nodes.

  ## Declare Its Outputs

  Every Agent node returns `text`, `files`, and `json` by default.

  If you want the agent to also produce something else, add your own output: type `/` in the **Agent task** and choose **New output**, then name it and pick a type in place.

  Declaring an output tells the agent exactly what to produce, and each one becomes a variable that downstream nodes can reference.
  </Tab>
</Tabs>
