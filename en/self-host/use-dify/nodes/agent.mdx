...
<Tip>
  The new Agent node is in beta: deploy the Dify [1.16.0-rc1](https://github.com/langgenius/dify/releases) release to try it out. It's on by default on Docker Compose, with its runtime bundled in.

  Before starting, set [`DIFY_AGENT_SERVER_SECRET_KEY`](/en/self-host/deploy/configuration/environments#dify_agent_server_secret_key) to a random value.
</Tip>
  ...
  **Data Security Notice**
  ...
  <Tabs>
    <Tab title="Invite an Agent">
      ...
      An invited agent is like a full-time employee: its capabilities are managed centrally in **Agent**. When you publish an update there, every workflow that uses the agent gets it.
    ...
    <Tab title="Start from Scratch">
      Create a one-time agent that lives on this node alone, and configure it right here, the same way you [build](/en/self-host/use-dify/build/new-agent/build) in **Agent**.

      It's like a temp you bring in for one job. If it proves its worth, promote it to **Agent** to reuse it elsewhere.
