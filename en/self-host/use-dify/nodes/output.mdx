...
Output nodes are only available in Workflows. Chatflows use the [Answer](/en/self-host/use-dify/nodes/answer) node instead.
</Info>
...
When you [publish a workflow as a tool](/en/self-host/use-dify/workspace/tools#workflow), the tool exposes the output variables from every Output node.

Give each output variable a name that is unique across all Output nodes. The tool exposes one output per name, so if a name repeats, only one of its definitions survives.

Avoid `text`, `files`, and `json`, which are reserved for the tool's built-in outputs.
