...
To debug an API extension that runs on your local machine while Dify is deployed elsewhere, you can use [Ngrok](https://ngrok.com) to expose the local endpoint to the public internet so your Dify instance can reach it. Steps:
...
This API endpoint can now be accessed publicly. At this point, we can configure this API endpoint in Dify for local code debugging. For configuration steps, please refer to [External Data Tool](/en/self-host/use-dify/workspace/api-extension/external-data-tool-api-extension).
...
For detailed instructions, see [Deploy Custom Endpoints Using Cloudflare Workers](/en/self-host/use-dify/workspace/api-extension/cloudflare-worker).
