...
Dify is made for agentic app building. In **Studio**, you can quickly build agentic workflows via a drag & drop interface and publish them as apps. You can access published apps via API, the web, or as an [MCP server](/en/self-host/use-dify/publish/publish-mcp). Dify offers two main app types: workflow and chatflow. You will need to choose an app type when creating a new app.
...
- **[User Input](/en/self-host/use-dify/nodes/user-input)**: Direct user interaction or API call invokes the app.

- **[Trigger](/en/self-host/use-dify/nodes/trigger/overview)**: The application runs automatically on a schedule or in response to a specific third-party event.
...
Unlike workflow, chatflow can't use [Trigger](/en/self-host/use-dify/nodes/trigger/overview) to start.
...
**Inputs**: You can specify any number of input variables at the [User Input](/en/self-host/use-dify/nodes/user-input) node for your app's end users to fill in.
