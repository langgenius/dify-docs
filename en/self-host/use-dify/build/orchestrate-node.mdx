...
- **Workspace-specific resources**, such as tools, integrations, and knowledge bases

## Iteration and Loop

For nodes that should run multiple times (once per item in a list or until a condition is met), place them inside an [Iteration](/en/self-host/use-dify/nodes/iteration) or [Loop](/en/self-host/use-dify/nodes/loop) node.