...
However the agent runs, its sessions collect in one place: the console's **Logs** page, where each row carries its source—the agent's web app or a workflow that uses it.