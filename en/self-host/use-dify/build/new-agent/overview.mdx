...
However the agent runs, its sessions collect in one place: the [**Logs**](/en/self-host/use-dify/monitor/logs) page, where each row carries its source—the agent's web app or a workflow that uses it.