...
To create an agent from a shared DSL file, choose **Import DSL file**. Review the imported agent and add skills or files as needed for this workspace.
...
To share the agent across workspaces, export it as a DSL file. Share its skill packages and the files added under **Files** alongside the DSL so the recipient can set up the agent in their workspace.
