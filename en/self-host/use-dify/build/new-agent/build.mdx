...
To create an agent from a DSL file someone shared, choose **Import DSL file** instead. Skills and files aren't included in DSL files, so add them back after importing.
...
To share the agent with another workspace, choose **Export DSL** from its card menu on **Agents**. The exported file doesn't include skills and files, so add them back after importing.
