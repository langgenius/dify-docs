...
- **Time**
    ...
    If a run gets cut off, try again or break the task into smaller steps. To allow runs up to an hour, raise `APP_MAX_EXECUTION_TIME` and `WORKFLOW_MAX_EXECUTION_TIME` to `3600`; beyond an hour, also raise `DIFY_AGENT_RUN_TIMEOUT_SECONDS`.
