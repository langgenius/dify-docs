...
<Tip>
A new kind of Agent is in beta: it works in a sandbox of its own, and you build it once to use as a chat app or inside workflows. See [New Agent](/en/self-host/use-dify/build/new-agent/overview).
</Tip>
...
<Tip>
Agents support optional features like conversation openers, follow-up suggestions, and more. See [App Toolkit](/en/self-host/use-dify/build/additional-features) for details.
</Tip>
...
Add [Dify tools](/en/self-host/use-dify/workspace/tools) to enable the model to interact with external services and APIs for tasks beyond text generation, such as fetching live data, searching the web, or querying databases.
...
For creating and managing document metadata, see [Metadata](/en/self-host/use-dify/knowledge/metadata).
...
When you're happy with the results, click **Publish** to make your app available. See [Publish](/en/self-host/use-dify/publish/README) for the full list of publishing options.