...
<Steps>
  ...
  <Step title="Start Dify">
    ...
    2. Start the containers:
        ...
        <Info>
          On first run, the `dify-compose` startup script creates `.env` and writes a generated `SECRET_KEY` to it.
        </Info>

        You will then be asked whether you need a custom deployment:

        - **Press `Enter` (or answer `n`)** to deploy with built-in defaults from `.env.default`. Containers start immediately.

        - **Answer `y`** to customize first. The startup script exits so you can edit `.env`:

            1. Refer to `.env.example` for the full environment variable list. See [environment variables](/en/self-host/configuration/environments) for details.

            2. Add the ones you want to override to `.env`.

            3. Run the start command again when ready. At runtime, your `.env` overrides are merged with defaults from `.env.default`.

        <Info>
          The prompt only appears on first run. To change configuration later, see [Customize](#customize).
        </Info>
...
Refer to `.env.example` for the full list of environment variables. Add the ones you want to override to your local `.env` file, then restart Dify to apply the changes:
