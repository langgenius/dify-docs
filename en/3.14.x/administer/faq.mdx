...
Currently, it's not possible to switch member permissions within [System Settings](/en/3.14.x/administer/members-management). To adjust permissions, you need to remove Member A and then re-add them with the Admin role.

If you need to continue adding members to the workspace, it's recommended to have the workspace administrator directly invite members to join the space. For detailed instructions, please refer to [Team Member Management](/en/3.14.x/use/workspace/team-members-management).
...
For more information about security restrictions, please read the [Security Measures](/en/3.14.x/administer/password-policy).

## 5. How can I enhance my account security?

1. **Use SSO**: If your organization supports SSO, we recommend configuring and using it. For detailed information, please refer to [Enterprise SSO Authentication](/en/3.14.x/administer/sso/introduction).
...
1. Please refer to [Authentication - User Account Creation Methods](/en/3.14.x/administer/loginsettings) and enable the following two account creation methods:

- Allow users to self-register
- Allow auto-create personal workspace for new users

2. If the above switches are confirmed to be enabled but the error still occurs, please refer to [Edit Attributes and Claims](/en/3.14.x/administer/sso/azure/saml#advanced-configuration%3A-edit-attributes-and-claims) and change the **source attribute** for the **Unique User Identifier (Name ID)** to `user.mail`.
