...
4. You wish to deploy on a Kubernetes-based container platform that requires platform-specific support, such as Red Hat OpenShift or ROSA (Red Hat OpenShift Service on AWS).
5. You wish to modify existing software structure, code, etc.
6. You wish to use certain vector databases that the Dify team cannot support.
7. You wish to use unsupported hardware protocols.
8. Other situations explicitly refused support by the Dify team.
...
**Dify Enterprise** is a software product. When you purchase the software on its own, we provide standard technical support during the license term on business days (Monday–Friday, 8 hours per day). Purchasing this software does not include Dify deployment services. After reading this document, please assess your team’s technical capacity as needed, contact our sales team to evaluate technical complexity and your actual requirements, and we can recommend the deployment approach that best fits your situation.
