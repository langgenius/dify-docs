...
<Note>
External SSO is for people **without** a Dify account. If your email already belongs to a Dify account, this sign-in is rejected; sign in as an [account user](/en/3.14.x/develop/cli/account-users/authenticate) instead.
</Note>
...
Signing in stores an OAuth bearer token, recognizable by its `dfoe_` prefix. It represents your corporate identity on this Dify host and carries only the access your company granted it. For what that access covers, see [Scope and Limitations](/en/3.14.x/develop/cli/sso-users/scope-and-limitations).
...
The config directory is `~/.config/difyctl` on macOS and Linux and `%APPDATA%\difyctl` on Windows. Set [`DIFY_CONFIG_DIR`](/en/3.14.x/develop/cli/reference/environment-variables) to override it.
...
| The browser says your email belongs to a Dify account | External SSO is only for users without a Dify account. Sign in as an [account user](/en/3.14.x/develop/cli/account-users/authenticate) instead. |
...
For everything else, see [Troubleshooting](/en/3.14.x/develop/cli/troubleshooting).
