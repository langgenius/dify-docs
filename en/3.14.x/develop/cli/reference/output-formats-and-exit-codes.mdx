...
`difyctl` is built to be scripted: data goes to stdout while everything else goes to stderr, the [`-o` global flag](/en/3.14.x/develop/cli/reference/global-flags) selects the output format, and failures exit with a predictable code.
...
For what each code means and how to fix it, see [Troubleshooting](/en/3.14.x/develop/cli/troubleshooting).
...
| `64` | Compatibility gate failed | Only [`version --check-compat`](/en/3.14.x/develop/cli/reference/version#gate-scripts-on-compatibility): the server was not confirmed compatible. |
...
For the payload shape and the resume protocol, see [When a Workflow Pauses](/en/3.14.x/develop/cli/reference/apps#when-a-workflow-pauses) on the Apps reference.
