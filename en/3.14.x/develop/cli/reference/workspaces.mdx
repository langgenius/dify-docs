---
title: Workspaces
description: List your workspaces, switch the active one, and see how difyctl resolves which workspace your commands use
---
...
- [`use workspace`](#switch-your-workspace) switches the active one

Both accept the [global flags](/en/3.14.x/develop/cli/reference/global-flags).

<Note>
Workspaces apply to account users only. If you signed in through external SSO, you're scoped to a single context with no workspace, and the commands here don't apply. See [Scope and Limitations](/en/3.14.x/develop/cli/sso-users/scope-and-limitations) for details.
</Note>
...
<Tip>
For the everyday invocation, see [Switch Your Workspace](/en/3.14.x/develop/cli/account-users/common-tasks#switch-your-workspace) in Common Tasks.
</Tip>
...
Only the [global flags](/en/3.14.x/develop/cli/reference/global-flags).
...
2. The [`DIFY_WORKSPACE_ID`](/en/3.14.x/develop/cli/reference/environment-variables) environment variable.
