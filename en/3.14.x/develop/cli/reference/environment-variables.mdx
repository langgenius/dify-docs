...
| `DIFY_WORKSPACE_ID` | The workspace commands run against when no `--workspace` flag is set. Must be a UUID. See [How difyctl Picks a Workspace](/en/3.14.x/develop/cli/reference/workspaces#how-difyctl-picks-a-workspace) for the full resolution order. | none |
| `DIFYCTL_HTTP_RETRY` | Retry attempts for idempotent requests on transient failures (`0` disables retries). [`--http-retry`](/en/3.14.x/develop/cli/reference/global-flags#http-retries) overrides it. | `3` |
...
<Note>
`env list` takes `--json`, not the [`-o` global flag](/en/3.14.x/develop/cli/reference/global-flags).
</Note>
