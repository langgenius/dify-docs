...
`difyctl` can hold sign-ins for several Dify hosts and accounts at once. Each host and account pair is a context: its metadata lives in `hosts.yml` in the [config directory](/en/3.14.x/develop/cli/reference/environment-variables), and its bearer token lives in your OS credential store (with a protected file fallback).
...
For the step-by-step sign-in walkthrough, see Authenticate for [account users](/en/3.14.x/develop/cli/account-users/authenticate) or [external SSO users](/en/3.14.x/develop/cli/sso-users/authenticate).
...
If you sign in through external SSO, the same device flow routes through your corporate identity provider and produces an `external SSO` context (a `dfoe_` token) instead of a standard account context; see [External SSO Users: Authenticate](/en/3.14.x/develop/cli/sso-users/authenticate) for the specifics.

### Flags

| Flag | Type | Default | Description |
|:---|:---|:---|:---|
| `--host <url>` | string | none | The Dify host to sign in to. Use your instance's console API URL (for example, `https://dify.example.com`), since `difyctl` operates against the console API. A URL without a scheme gets `https://`. In a terminal, omit it and enter your host; in a non-interactive session (script, CI, pipe), the flag is required. |
...
| Flag | Type | Default | Description |
|:---|:---|:---|:---|
| `--domain <host>` | string | none | The stored host to switch to, written as its bare domain (`dify.example.com`), without a scheme.<br></br><br></br>In a terminal, omit it and pick from your stored hosts. In a non-interactive session (script, CI, pipe), the flag is required. |
