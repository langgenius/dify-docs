...
Every app task maps to one command, and all of them accept the [global flags](/en/3.14.x/develop/cli/reference/global-flags).
...
<Tip>
For the everyday invocations, see Common Tasks for [account users](/en/3.14.x/develop/cli/account-users/common-tasks#find-your-apps) or [external SSO users](/en/3.14.x/develop/cli/sso-users/common-tasks).
</Tip>

### Arguments

- `[app-id]`: optional. The ID of one app to show. Omit it to list every app.
...
| `--limit <n>` | integer | `20` | Page size, 1 to 200. The flag wins, then [`DIFY_LIMIT`](/en/3.14.x/develop/cli/reference/environment-variables). |
| `--workspace <id>` | string | active workspace | Run against another workspace for this invocation only.<br></br><br></br>For how `difyctl` resolves the workspace, see [How difyctl Picks a Workspace](/en/3.14.x/develop/cli/reference/workspaces#how-difyctl-picks-a-workspace). |
...
<Note>
External SSO users see the apps an administrator permitted, not a workspace listing; `--workspace` and `-A` don't apply. See [Scope and Limitations](/en/3.14.x/develop/cli/sso-users/scope-and-limitations) for details.
</Note>
...
List apps across every workspace you can see:
...
| `--think` | boolean | `false` | Print the model's thinking to stderr when the model exposes it, whether as inline `<think>` blocks or a separate reasoning stream.<br></br><br></br>Without this flag, that thinking is hidden. |
...
Errors print to stderr. Under `-o json` they arrive as a structured JSON object with a stable `code` field. See [Output Formats and Exit Codes](/en/3.14.x/develop/cli/reference/output-formats-and-exit-codes) for the error shape.
...
| `--action <id>` | string | auto-selected | Which form action to take, by `id` from the pause payload's `actions`.<br></br><br></br>Optional when the form has exactly one action, required when it has several. |
| `--inputs <json>` | string | none | Values for the form's inputs as one JSON object, keyed by each input's `output_variable_name`.<br></br><br></br>Mutually exclusive with `--inputs-file`. |
...
| Flag | Type | Default | Description |
|:---|:---|:---|:---|
| `-o, --output <path>` | string | none | Write the DSL to this file instead of stdout.<br></br><br></br>On this command, `-o` is the output file path, not the output-format selector. |
| `--include-secret` | boolean | `false` | Include encrypted secret values in the exported DSL. |
| `--workflow-id <id>` | string | none | Export a specific published workflow version by ID, instead of the default draft.<br></br><br></br>Workflow and Chatflow apps only. |
| `--workspace <id>` | string | active workspace | Run against another workspace for this invocation only.<br></br><br></br>For how `difyctl` resolves the workspace, see [How difyctl Picks a Workspace](/en/3.14.x/develop/cli/reference/workspaces#how-difyctl-picks-a-workspace). |
...
| `--app-id <id>` | string | none | Overwrite an existing app instead of creating a new one.<br></br><br></br>Workflow and Chatflow apps only. |
| `--workspace <id>` | string | active workspace | Import into another workspace for this invocation only.<br></br><br></br>For how `difyctl` resolves the workspace, see [How difyctl Picks a Workspace](/en/3.14.x/develop/cli/reference/workspaces#how-difyctl-picks-a-workspace). |
