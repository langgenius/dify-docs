...
The overview lists every command with its one-liner, three getting-started examples, the [global flags](/en/3.14.x/develop/cli/reference/global-flags), and the [guide topics](#guide-topics). For the same inventory with links into this reference, see the [Command Index](/en/3.14.x/develop/cli/reference/command-index).
...
These topics are built into the CLI. For which variables `difyctl` actually reads, see [Environment Variables](/en/3.14.x/develop/cli/reference/environment-variables).
