---
title: Authenticate
description: Sign in to your company's Dify host from the CLI, see where your token is stored, and manage your session
---
...
<Steps>
  <Step title="Run the login command">
    ...
    Pass your Dify host URL. Use the [console API URL](/en/3.14.x/deploy/advanced-configuration/environment-variables#console_api_url).
...
The config directory is `~/.config/difyctl` on macOS and Linux (Linux honors `XDG_CONFIG_HOME`) and `%APPDATA%\difyctl` on Windows. Set [`DIFY_CONFIG_DIR`](/en/3.14.x/develop/cli/reference/environment-variables) to override it.
