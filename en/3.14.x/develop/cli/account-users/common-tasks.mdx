...
All commands below assume you're [signed in](/en/3.14.x/develop/cli/account-users/authenticate).

## Send a Message

Send a message to a Chatbot, Chatflow, Agent, or Text Generator app by passing it as a positional argument to [`difyctl run app`](/en/3.14.x/develop/cli/reference/apps#run-an-app). The reply prints to stdout.
...
Pass the inputs as a single JSON object with [`--inputs`](/en/3.14.x/develop/cli/reference/apps#run-an-app) instead of a positional message. The outputs print to stdout as JSON.
...
List the apps in your active workspace with [`difyctl get app`](/en/3.14.x/develop/cli/reference/apps#list-your-apps). Narrow the list with `--name` or `--mode`.
...
Add [`--stream`](/en/3.14.x/develop/cli/reference/apps#run-an-app) to print the response as it's generated instead of all at once at the end.
...
Copy the conversation ID from the hint that follows each Chatbot or Chatflow reply, then pass it back with [`--conversation`](/en/3.14.x/develop/cli/reference/apps#run-an-app) to continue the same conversation.
...
Add [`-o json`](/en/3.14.x/develop/cli/reference/output-formats-and-exit-codes) to any command to get the raw response as pipe-friendly JSON.
...
Change your active workspace with [`difyctl use workspace`](/en/3.14.x/develop/cli/reference/workspaces#switch-your-workspace). List available workspaces with `get workspace`.
...
Before running an unfamiliar app, check its app type and input schema with [`difyctl describe app`](/en/3.14.x/develop/cli/reference/apps#inspect-an-app): names, types, and which inputs are required.
