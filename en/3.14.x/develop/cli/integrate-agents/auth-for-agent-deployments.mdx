...
- **Your own machine**: sign in ([account](/en/3.14.x/develop/cli/account-users/authenticate) or [SSO](/en/3.14.x/develop/cli/sso-users/authenticate)) and you're set.
...
Use the [console API URL](/en/3.14.x/deploy/advanced-configuration/environment-variables#console_api_url) for `--host`.
...
<Steps>
  ...
  <Step title="Copy the config directory to the target">
    After copying, point `difyctl` at it with [`DIFY_CONFIG_DIR`](/en/3.14.x/develop/cli/reference/environment-variables). For a container, mount the directory at runtime instead of baking it into the image:
...
So it should stop and surface the failure for a human to sign in again, not retry. See [Handle Errors and Retries](/en/3.14.x/develop/cli/integrate-agents/error-handling-and-retries-for-agents#branch-on-the-exit-code) for how to branch on it.

To revoke a session you suspect is compromised, run [`auth devices revoke`](/en/3.14.x/develop/cli/reference/auth-and-contexts#revoke-sessions) from any signed-in machine.
