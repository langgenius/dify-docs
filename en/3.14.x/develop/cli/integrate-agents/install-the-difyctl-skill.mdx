...
Run [`difyctl skills install`](/en/3.14.x/develop/cli/reference/skills#install-the-skill) to write a skill file for the coding agents on your machine. The agent picks it up and onboards itself from there.

The installed `SKILL.md` is a bootstrap, not a manual. It tells the agent the one thing that matters: run [`difyctl help -o json`](/en/3.14.x/develop/cli/reference/help#machine-readable-help) and treat that output as the source of truth.
...
If your runtime doesn't read skills, your agent can still drive `difyctl` directly by reading [`difyctl help -o json`](/en/3.14.x/develop/cli/reference/help#machine-readable-help) at runtime.

## Prerequisites

- [Install](/en/3.14.x/develop/cli/install) `difyctl` and [sign in](/en/3.14.x/develop/cli/integrate-agents/auth-for-agent-deployments) on the machine the agent runs on, so it can reuse your session.
...
<Info>
See the [Skills reference page](/en/3.14.x/develop/cli/reference/skills) for detection, target paths per agent, and the `--agent`, `--stdout`, and explicit-directory forms.
</Info>
...
| Commands fail with exit code 4 | The agent has no session to reuse. [Sign in](/en/3.14.x/develop/cli/integrate-agents/auth-for-agent-deployments) on its machine first. |
...
For everything else, see the full [Troubleshooting](/en/3.14.x/develop/cli/troubleshooting) page.
