...
The agent doesn't work from a hard-coded command list. `difyctl` describes itself through [`difyctl help -o json`](/en/3.14.x/develop/cli/reference/help#machine-readable-help), and the agent reads that at runtime, so it always tracks the current CLI.
...
See [Authenticate Where Your Agent Runs](/en/3.14.x/develop/cli/integrate-agents/auth-for-agent-deployments) for details.

## Onboard Your Agent

The agent learns `difyctl` from `difyctl help -o json`. Pointing it there is the only setup, and how depends on the agent:

- **Reads skills** (Claude Code, Codex, and similar): [install the `difyctl` skill](/en/3.14.x/develop/cli/integrate-agents/install-the-difyctl-skill). One command writes a small file that points the agent at `difyctl help -o json`, and it onboards itself from there. No integration code.
