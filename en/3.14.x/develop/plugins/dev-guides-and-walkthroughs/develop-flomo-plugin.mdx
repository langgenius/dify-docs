...
A tool plugin uses two YAML files: a **provider** file that declares credentials and lists the tools, and one **tool** file per callable tool. See [General Specifications](/en/3.14.x/develop/plugins/features-and-specs/plugin-types/general-specifications) for the full schema.
