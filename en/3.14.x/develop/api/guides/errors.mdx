...
Most HTTP errors use this JSON envelope:
...
Use `code` to decide how your client handles the failure. The operation page lists the codes that call can raise. For diagnosis, read `message` alongside the HTTP status.

[Run Query](/en/3.14.x/develop/api/queries/run-query) also relays errors with `error`, `code`, and `failure` fields. Use its response reference for those errors and [Handle Errors](/en/3.14.x/develop/api/guides/knowledge-base#handle-errors) for new knowledge base error codes.
...
Once an app stream opens, the HTTP status is already `200`: failures arrive as an `error` event and end the stream. The event's `code` values are the same ones documented here—classify them with the same rules. See [Consume Streaming Responses](/en/3.14.x/develop/api/guides/streaming) for details.

New knowledge base query streams use `answer.error` instead. Follow [Query the Knowledge Base](/en/3.14.x/develop/api/guides/knowledge-base#query-the-knowledge-base) for their events and recovery steps.
...
For new knowledge base calls, follow [Handle Errors](/en/3.14.x/develop/api/guides/knowledge-base#handle-errors). A write may have taken effect even when its response is lost. Check its outcome before sending it again.