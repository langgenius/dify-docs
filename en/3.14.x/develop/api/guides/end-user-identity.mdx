...
- **Resuming runs**: [Stream Workflow Events](/en/3.14.x/develop/api/workflow-runs/stream-workflow-events) returns a 404 when `user` doesn't match the run's creator.
...
Some responses carry an end-user ID instead of full details, such as `created_by` in the Upload File response. [Get End User Info](/en/3.14.x/develop/api/end-users/get-end-user-info) resolves it.
