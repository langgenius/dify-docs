...
When a workflow reaches a [Human Input node](/en/3.14.x/use/nodes/human-input), it pauses and emits a `human_input_required` event in the streaming response. The event carries a `form_token` that your integration uses to drive the form lifecycle until the workflow resumes.

For per-endpoint reference, see the [Human Input API](/en/3.14.x/develop/api/human-input/get-human-input-form).
...
<Steps>
  <Step title="Start the app in streaming mode">
    ...
    1. Call [Run Workflow](/en/3.14.x/develop/api/workflow-runs/run-workflow) (Workflow apps) or [Send Chat Message](/en/3.14.x/develop/api/chat-messages/send-chat-message) (Chatflow apps), passing your end user's `user` identifier.
  ...
  <Step title="(File inputs only) Upload local files">
    If the recipient attaches a local file to a `file` or `file-list` input, upload it first with [Upload File](/en/3.14.x/develop/api/files/upload-file). It returns an `id` you reference as `upload_file_id` in the submit payload. Use one consistent `user` across the run, upload, and submit calls (see [End User Identity](/en/3.14.x/develop/api/guides/end-user-identity) for details).
  ...
  <Step title="Submit the response">
    Call [Submit Human Input Form](/en/3.14.x/develop/api/human-input/submit-human-input-form) with the recipient's input values, the selected `action`, and your `user`. The `action` must be one of the actions from the form definition in Step 2.
  ...
  <Step title="Resume listening to the workflow">
    If the original SSE stream closed, reopen it via [Stream Workflow Events](/en/3.14.x/develop/api/workflow-runs/stream-workflow-events) with the `workflow_run_id` from Step 1 and the same `user` that started the run—a different `user` gets a 404.
...
- **Pre-upload, then reference `upload_file_id`** (recommended)
  ...
  [Upload File](/en/3.14.x/develop/api/files/upload-file) enforces the file size limits at upload time, so the recipient gets immediate feedback and can retry before committing the whole submission.
...
The Human Input API works only with forms delivered via the Human Input node's web app method. Email-only delivery doesn't expose a `form_token`.
...
1. Call [Get Human Input Form](/en/3.14.x/develop/api/human-input/get-human-input-form) to get the form definition:
...
2. For each local file, call [Upload File](/en/3.14.x/develop/api/files/upload-file):
...
3. Call [Submit Human Input Form](/en/3.14.x/develop/api/human-input/submit-human-input-form) with the recipient's input and selected action:
...
4. Reconnect to the run's stream with [Stream Workflow Events](/en/3.14.x/develop/api/workflow-runs/stream-workflow-events) to follow it through to completion, as in Step 5 of the sequence above.
