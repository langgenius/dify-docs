...
This API endpoint can now be accessed publicly. At this point, we can configure this API endpoint in Dify for local code debugging. For configuration steps, please refer to [External Data Tool](/en/3.14.x/use/workspace/api-extension/external-data-tool-api-extension).
...
For detailed instructions, see [Deploy Custom Endpoints Using Cloudflare Workers](/en/3.14.x/use/workspace/api-extension/cloudflare-worker).
