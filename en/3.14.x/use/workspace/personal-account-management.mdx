...
Setting or resetting a password applies only when password sign-in is enabled for your workspace; with an SSO-only setup, the password option is unavailable.

If your organization uses single sign-on, you sign in through your identity provider over OIDC, SAML, or OAuth2 instead of entering a password. Administrators configure and enforce SSO from the Enterprise Admin Panel; see [SSO Authentication](/en/3.14.x/administer/sso/introduction).
