...
Build an API service that implements the [External Knowledge API specification](/en/3.14.x/use/knowledge/external-knowledge-api). Your service needs a single `POST` endpoint that accepts a search query and returns matching text chunks with similarity scores.
...
Once created, the external knowledge base is available for use in your applications just like any built-in knowledge base. See [Integrate Knowledge Within Application](/en/3.14.x/use/knowledge/integrate-knowledge-within-application) for details.
...
If retrieval fails or returns unexpected results, verify your API response against the [External Knowledge API specification](/en/3.14.x/use/knowledge/external-knowledge-api#response). 
