...
- **From a workflow**
    ...
    Their configurations are saved into the snippet. Any external variables they reference (outputs from nodes you didn't select, environment variables, and so on) become the snippet's [Input Variables](#edit-a-snippet).

    <Info>
    An exception: an LLM node that takes its model from an environment variable keeps that model configuration as a fixed setting, so the snippet uses the same model wherever you add it.
    </Info>
...
<Tip>
Administrators can reassign these permissions to custom roles. See [Roles and Permissions](/en/3.14.x/use/workspace/roles-and-permissions) for details.
</Tip>
