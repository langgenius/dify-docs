...
To create an agent from a shared DSL file, choose **Import DSL file**. [Library skills](/en/3.14.x/use/build/skills) reattach automatically when your workspace has skills with the same names; embedded skills and files aren't included, so add them after importing.

<Info>
Creating and managing agents requires the **Manage Agents** permission; the Owner, Admin, and Editor roles have it by default. See [Permission Reference](/en/3.14.x/use/workspace/permission-reference) for details.
</Info>
...
- Choose **Add from library** to add a [*library skill*](/en/3.14.x/use/build/skills) shared workspace-wide. You can add up to 20 library skills to an agent, and it follows the latest published version of each.

- Choose **Upload skill.zip** to add a `.zip` or `.skill` package as an *embedded skill*, one the agent keeps to itself.

    The package needs a `SKILL.md` inside and can be up to 50 MB by default, adjustable with [`UPLOAD_SKILL_FILE_SIZE_LIMIT`](/en/3.14.x/deploy/advanced-configuration/environment-variables#upload_skill_file_size_limit).

    To share an embedded skill with other agents, [make it a library skill](/en/3.14.x/use/build/skills#import-a-skill).
...
Size limits depend on file type; each is a default you can adjust with [environment variables](/en/3.14.x/deploy/advanced-configuration/environment-variables):
...
<Info>
Files the agent creates and saves are capped at 50 MB each by default, adjustable with `DIFY_AGENT_STUB_UPLOAD_FILE_SIZE_LIMIT`. See [Environment Variables](/en/3.14.x/deploy/advanced-configuration/environment-variables#agent-backend-configuration) for details.
</Info>

### Tools {#tools}

Give the agent [Dify tools](/en/3.14.x/use/workspace/tools) so it can act beyond the chat: plugins, custom APIs, workflows, and MCP servers from your workspace. Some tools need authentication before use.
...
<Note>
Values stored here can surface in command output the agent reads. To mask sensitive strings from it, set `DIFY_AGENT_SHELL_REDACT_PATTERNS`. See [Environment Variables](/en/3.14.x/deploy/advanced-configuration/environment-variables#agent-backend-configuration) for details.
</Note>

## Build by Chatting {#build-by-chatting}
...
Chatting in Preview never changes the agent's configuration, so use it as the final check before you publish.
...
<Tip>
To polish the chat experience, you can add extras like a welcome message, suggested questions, and voice in **Chat Features**. See [App Toolkit](/en/3.14.x/use/build/additional-features) for details.
</Tip>

From the **Access Point** tab, host it as a web app at a shareable link, embed it in your site, or call it from your code through the [service API](/en/3.14.x/develop/api/guides/agent). You can also set who can reach the web app with [Web App Access Control](/en/3.14.x/use/publish/webapp/web-app-access).
...
You can also invite the agent into a workflow's [Agent node](/en/3.14.x/use/nodes/agent#new-agent) to handle a step.
...
- **Time**
    ...
    See [Environment Variables](/en/3.14.x/deploy/advanced-configuration/environment-variables) for details on these variables.
...
- **Reply files**
    ...
    If your agent produces bigger artifacts, raise `DIFY_AGENT_STUB_UPLOAD_FILE_SIZE_LIMIT`. See [Environment Variables](/en/3.14.x/deploy/advanced-configuration/environment-variables#agent-backend-configuration) for details.

## Troubleshooting

<AccordionGroup>
  <Accordion title="Agents doesn't appear in the navigation or the node list">
    If other members see **Agents** but you don't, the cause is your workspace role: the entry needs the **Manage Agents** permission, which the Owner, Admin, and Editor roles have by default.
  ...
  <Accordion title="Agent runs fail as soon as they start">
    If every run errors immediately, the cluster may be missing the agent's services: the agent-backend service or a sandbox runtime.

    See [Agent Sandbox Deployment](/en/3.14.x/deploy/advanced-configuration/agent-sandbox) for the required components and how to check them.
  </Accordion>
  ...
  <Accordion title="The agent can't fetch files or pages from your internal network">
    The sandbox's internet access goes through a proxy that blocks private-network addresses by default. Public URLs work; for material on your intranet, add it to the agent's [Files](#files) instead.
  </Accordion>
</AccordionGroup>
