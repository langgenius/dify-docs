...
- A **Chatflow** adds a conversation layer. 
    ...
    <Tip>
        Chatflows support optional features like content moderation, text to speech, and more. See [App Toolkit](/en/3.14.x/use/build/additional-features) for details.
    </Tip>
...
| | Workflow | Chatflow |
|:---|:---|:---|
| Starts with | [User Input](/en/3.14.x/use/nodes/user-input) or [Trigger](/en/3.14.x/use/nodes/trigger/overview) | User Input |
| Ends with | [Output](/en/3.14.x/use/nodes/output) node (optional) | [Answer](/en/3.14.x/use/nodes/answer) node (required) |
...
For how to compose either type, see [Orchestration Logic](/en/3.14.x/use/build/orchestrate-node).
