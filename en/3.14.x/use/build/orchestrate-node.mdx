...
For nodes that should run multiple times (once per item in a list or until a condition is met), place them inside an [Iteration](/en/3.14.x/use/nodes/iteration) or [Loop](/en/3.14.x/use/nodes/loop) node.