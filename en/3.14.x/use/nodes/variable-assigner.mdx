...
The Variable Assigner node manages persistent data in Chatflow applications by writing to conversation variables (Understand the different types of variables [here](/en/3.14.x/learn/key-concepts#variables)). Unlike regular workflow variables that reset with each execution, conversation variables persist throughout an entire chat session.
