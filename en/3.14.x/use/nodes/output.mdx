...
Output nodes are only available in Workflows. Chatflows use the [Answer](/en/3.14.x/use/nodes/answer) node instead.
</Info>
...
When you [publish a workflow as a tool](/en/3.14.x/use/workspace/tools#workflow), the Output node defines the tool's return schema. Each output variable name becomes a key in the tool's result, accessible to the parent workflow that invokes the tool.
