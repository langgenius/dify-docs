...
### Share a Model Configuration Across Nodes

When several LLM nodes should run on the same model, store the model selection in an environment variable instead of configuring each node separately. Editing the variable, such as pointing it at a different model, updates every node bound to it.

Open the **Environment Variables** panel, add a variable of the **LLM** type, and choose a model with its parameters.

Then, in each LLM node, switch the **Model** field to variable mode, and pick the variable. The variable carries the provider, model, and model parameters; prompts and other settings stay with each node.
...
<Frame>
  ![LLM Memory](/images/use-dify/workflow/llm-memory.png)
</Frame>
...
Some reasoning models wrap their thinking in `<think>...</think>` tags inside their response. By default, those tags are included in the `text` output, so the reasoning flows downstream together with the answer.
...
In API calls, this toggle appears as the `reasoning_format` parameter. When the toggle is on, `reasoning_format` is `separated`, and streaming API clients receive the reasoning as dedicated `reasoning_chunk` events, outside the answer stream.
