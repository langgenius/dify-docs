...
<Tabs>
  ...
  **Memory Management** balance context retention with token efficiency based on your use case requirements.
  ...
  <Tab title="New Agent" id="new-agent">
  ...
  **Security Notice: Agent Environment Variable Exfiltration Risk**
  ...
  <Note>
  The new Agent node is in beta and available in Workflow apps only. It requires the agent-backend service and a sandbox environment in your cluster; see [Agent Sandbox Deployment](/en/3.14.x/deploy/advanced-configuration/agent-sandbox) to enable them.

  For production, replace `agentBackend.serverSecretKey` in your Helm values with your own random value.
  </Note>

  The new Agent node runs an [agent](/en/3.14.x/use/build/new-agent/overview) as one step of a workflow. Unlike the classic node's model-with-tools setup, the agent arrives as a complete worker with its own capabilities and sandbox.
  ...
  <Tabs>
    <Tab title="Invite an Agent">
      ...
      - To edit its capabilities, click **Edit in Agents**.
    ...
    <Tab title="Start from Scratch">
      Create a one-time agent that lives on this node alone, and configure it right here, the same way you [build](/en/3.14.x/use/build/new-agent/build) in **Agents**.

      It's like a temp you bring in for one job. If it proves its worth, promote it to **Agents** to reuse it elsewhere.

      <Frame>
        ![Save a One-Time Agent](/images/use-dify/workflow/save-onetime-agent-to-roster.png)
      </Frame>
    </Tab>
  </Tabs>
  ...
  <Info>
  Two nodes that invite the same agent still work separately: each starts from the agent's saved setup in its own sandbox, and nothing from one node's run (files it wrote, tools it installed) carries into another.

  To pass results between nodes, declare them as outputs and reference them downstream.
  </Info>

  <a id="give-it-a-task"></a>
  ...
  <Note>
  Variables you pull in reach the agent as text and are truncated at 2,000 characters.

  To hand over longer content in full, pass it as a file: the agent receives the whole file and reads it in its sandbox.
  </Note>
  ...
  Each file the agent returns is capped at 50 MB by default, adjustable with `DIFY_AGENT_STUB_UPLOAD_FILE_SIZE_LIMIT`. See [Environment Variables](/en/3.14.x/deploy/advanced-configuration/environment-variables#agent-backend-configuration) for details.
