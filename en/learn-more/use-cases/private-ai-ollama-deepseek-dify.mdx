...
After running the command, you should see all containers running with proper port mappings. For detailed instructions, refer to [Deploy with Docker Compose](/getting-started/install-self-hosted/docker-compose).
...
• **Base URL**: Set the Ollama client’s local service URL, typically `http://your_server_ip:11434`. If you encounter connection issues, please refer to the [FAQ](/learn-more/use-cases/private-ai-ollama-deepseek-dify#id-1.-connection-errors-when-using-docker);
...
> Chatflow / Workflow applications enable the creation of more complex AI solutions, such as document recognition, image processing, and speech recognition. For more details, please check the [Workflow Documentation](/guides/workflow).
...
2. Add an LLM Node, select the `deepseek-r1:7b` model under Ollama, and use the `{{#sys.query#}}` variable into the system prompt to connect to the initial node. If you encounter any API issues, you can handle them via [Load Balancing](/guides/model-configuration/load-balancing) or the [Error Handling](/guides/workflow/error-handling) node.
