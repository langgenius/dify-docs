...
Some unrelated fields in the manifest file are omitted. For detailed Manifest format, refer to [Manifest](/en/manifest).
...
The code format is similar to the [Tool](/en/tool) standard format and defines four parameters: `model`, `tools`, `query`, and `max_iterations` to implement the most basic Agent strategy. This means that users can:
