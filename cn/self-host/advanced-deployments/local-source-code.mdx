...
* 对于 Windows
  ...
  ```
  uv run celery -A app.celery worker -P solo --without-gossip --without-mingle -Q dataset,priority_dataset,priority_pipeline,pipeline,mail,ops_trace,app_deletion,plugin,workflow_storage,conversation,workflow,schedule_poller,schedule_executor,triggered_workflow_dispatcher,trigger_refresh_executor --loglevel INFO
  ```
