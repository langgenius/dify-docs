...
* 对于 macOS 或 Linux
  ...
  ```
  uv run celery -A app.celery worker -P gevent -c 1 --loglevel INFO -Q dataset,generation,mail,ops_trace,app_deletion,plugin,workflow_storage,conversation,priority_pipeline,pipeline,priority_dataset
  ```
...
* 对于 Windows
  ...
  ```
  uv run celery -A app.celery worker -P solo --without-gossip --without-mingle -Q dataset,generation,mail,ops_trace,app_deletion,plugin,workflow_storage,conversation,priority_pipeline,pipeline,priority_dataset --loglevel INFO
  ```
