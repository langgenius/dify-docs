...
<Note> ⚠️ 本文档由 AI 自动翻译。如有任何不准确之处，请参考[英文原版](/en/documentation/pages/debug/history-and-logs)。</Note>


Dify 会在每次工作流运行时记录详细的运行历史。你可以查看应用级别和各个节点的运行情况。

<Info>
对于发布后来自真实用户的运行历史，请参阅[运行历史和标注](/cn/documentation/pages/monitor/logs)。
</Info>
