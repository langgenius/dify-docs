...
<Note> ⚠️ 本文档由 AI 自动翻译。如有任何不准确之处，请参考[英文原版](/en/documentation/pages/debug/variable-inspect)。</Note>


变量检查器显示所有流经您工作流的数据。它会在每个节点运行后捕获输入和输出，因此您可以查看正在发生的情况并测试不同的场景。
...
您可以通过点击大多数变量值来编辑它们。当您运行下游节点时，它们将使用您编辑的值而不是原始值。这让您可以测试不同场景，而无需重新运行整个工作流。

<Info>
在此处编辑变量不会更改最初创建它们的节点的"最后运行"记录。
</Info>

例如，如果大型语言模型节点生成了像 `SELECT * FROM users` 这样的 SQL，您可以在检查器中将其编辑为 `SELECT username FROM users`，然后只重新运行数据库节点以查看不同的结果。
