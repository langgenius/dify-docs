...
Dify 专为智能体应用构建而设计。在 **Studio** 中，你可以通过拖拽界面快速构建智能体工作流并将其发布为应用。你可以通过 API、Web 或作为 [MCP 服务器](/cn/documentation/pages/publish/publish-mcp)访问已发布的应用。Dify 提供两种主要的应用类型：工作流和对话流。在创建新应用时，你需要选择一种应用类型。
...
- **[用户输入](/cn/documentation/pages/nodes/user-input)**：通过直接用户交互或 API 调用触发应用程序。

- **[触发器](/cn/documentation/pages/nodes/trigger/overview)**：应用程序按计划自动运行或响应特定的第三方事件。
...
与工作流不同，对话流不能使用[触发器](/cn/documentation/pages/nodes/trigger/overview)来启动。
...
**输入**：你可以在[用户输入](/cn/documentation/pages/nodes/user-input)节点为你的应用最终用户指定任意数量的输入变量来填写。
