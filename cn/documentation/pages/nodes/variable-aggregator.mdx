...
条件工作流创建并行执行路径，其中每次只有一个分支运行。没有聚合的情况下，您需要为每个可能的分支结果设置重复的下游节点，这会创建复杂且难以维护的工作流。
...
连接来自不同工作流分支的变量，您希望将这些变量组合。每个连接的变量都成为聚合输出的潜在输入。

### 类型约束

**相同类型规则** - 所有聚合的变量必须是相同的数据类型。一旦您连接第一个变量（例如字符串），节点只接受来自其他分支的相同类型变量。
...
高级工作流（v0.6.10+）可以同时聚合多组变量。每组都保持自己的类型约束，允许您在同一节点内并行聚合不同的数据类型。
