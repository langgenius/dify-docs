...
- [定时触发器](/cn/documentation/pages/nodes/trigger/schedule-trigger)

  - 在指定的时间点自动运行 Workflow。

  - 示例：每天早上 9 点自动生成销售报告，并通过电子邮件发送给团队成员。

- [插件触发器](/cn/documentation/pages/nodes/trigger/plugin-trigger)

  - 通过触发器插件订阅外部系统的特定事件，当订阅事件发生时自动运行 Workflow。

  - 示例：通过订阅 Slack 触发器插件中的`频道中的新消息`事件，自动分析并归档特定频道中的新消息。

- [Webhook 触发器](/cn/documentation/pages/nodes/trigger/webhook-trigger)
