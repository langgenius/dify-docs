...
<Note> ⚠️ 本文档由 AI 自动翻译。如有任何不准确之处，请参考[英文原版](/en/documentation/pages/nodes/http-request)。</Note>


HTTP 请求节点将您的工作流连接到外部 API 和 Web 服务。使用它来获取数据、发送 webhooks、上传文件，或与任何接受 HTTP 请求的服务集成。
...
<Tabs>
  ...
  <Tab title="数据提交">
    ...
    **PUT** 创建或完全替换资源。当您想要设置资源的整个状态时使用。
    
    **PATCH** 对现有资源进行部分更新。当您只需要修改特定字段时使用。
  </Tab>
...
使用双花括号引用工作流变量：`{{variable_name}}`。Dify 支持深度对象访问，因此您可以从先前的 HTTP 响应中提取嵌套值，如 `{{api_response.data.items[0].id}}`。
...
**错误处理** 定义 HTTP 请求失败时的替代工作流路径，确保即使在外部 API 不可用时您的工作流也能继续执行。
...
**API 数据获取** - 检索用户资料、产品信息或外部数据以丰富您的工作流处理。
