...
<Note> ⚠️ 本文档由 AI 自动翻译。如有任何不准确之处，请参考[英文原版](/en/documentation/pages/nodes/code)。</Note>


代码节点执行自定义 Python 或 JavaScript 来处理工作流中复杂的数据转换、计算和逻辑。当预设节点无法满足您的特定处理需求时可以使用它。
...
定义**输入变量**以访问工作流中其他节点的数据，然后在代码中引用这些变量。您的函数必须返回一个包含您已声明的**输出变量**的字典。
...
根据您的需求和熟悉程度在 **Python** 和 **JavaScript** 之间进行选择。两种语言都在安全沙箱中运行，并可访问用于数据处理的常用库。
...
**错误处理**让您在代码执行失败时定义回退路径，使您的工作流即使在代码遇到问题时也能继续运行。
...
如果您的代码无法保存，请检查浏览器的网络选项卡 - 安全过滤器可能正在阻止潜在危险的操作。
...
依赖已预装在沙箱环境中。请检查您的 Dify 安装中的可用包列表。
...
沙箱服务需要 Docker，并将代码执行与您的主系统隔离以确保安全。
