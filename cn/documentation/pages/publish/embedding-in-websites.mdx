...
将 Dify 应用嵌入到网站中，可以直接为您的访问者带来 AI 功能。无论您想要一个浮动聊天按钮、全屏聊天界面，还是嵌入式小组件，Dify 都提供了适用于任何网站技术的灵活选项。

## 嵌入选项

<CardGroup cols={2}>
  <Card title="聊天气泡按钮" icon="comment">
    浮动聊天按钮，打开您的 AI 助手——非常适合客户支持
  </Card>
...
浮动聊天按钮是最受欢迎的嵌入方法。它会在您的网站上添加一个可自定义的按钮，点击时打开您的 AI 助手。
...
window.difyChatbotConfig = {
    ...
    // 可选：预填充用户上下文
    inputs: {
        name: "John Doe",          // 来自您的 Dify 应用的变量名
        department: "Support"
    },
...
<Steps>
  <Step title="获取您的嵌入令牌">
    在您的 Dify 应用中，前往 **发布 → 嵌入** 找到您的唯一令牌。
  </Step>
  
  <Step title="添加脚本">
    在您网站的 HTML 中包含配置和 Dify 的嵌入脚本。
  </Step>
  
  <Step title="自定义外观">
    调整 `containerProps` 以匹配您网站的设计。
  </Step>
  
  <Step title="测试功能">
    打开您的网站并尝试聊天按钮，确保一切正常工作。
  </Step>
</Steps>

## Iframe 嵌入

为了获得更集成的体验，可以将您的 Dify 应用直接嵌入到网页内容中：
...
<AccordionGroup>
  ...
  <Accordion title="产品演示">
    通过嵌入式演示展示 AI 功能。让访问者在不离开您网站的情况下试用您的应用。
  </Accordion>
...
**Iframe 无法加载：**
- 确认 iframe URL 包含您正确的应用令牌
- 检查您的网站是否允许 iframe 内容
- 如果您的 Dify 应用需要，确保父页面通过 HTTPS 提供服务

<Warning>
在不同的浏览器和设备上测试您的嵌入以确保兼容性。某些企业防火墙可能会阻止 iframe 内容。
</Warning>

您可以使用 CSS 变量或 `containerProps` 选项覆盖默认按钮样式。根据 CSS 特异性应用这些方法以实现您想要的自定义。
