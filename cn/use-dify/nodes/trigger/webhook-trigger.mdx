...
<Tip>
    若目标外部系统有可用的触发器插件，推荐使用 [插件触发器](/cn/use-dify/nodes/trigger/plugin-trigger)。
</Tip>
...
<Tip>
    一个 Workflow 可同时拥有多个并行的 Webhook 触发器。当并行的分支连续包含相同节点时，可在相同部分之前添加 [变量聚合节点](/cn/use-dify/nodes/variable-aggregator) 以合并分支，而无需在每个分支中分别重复添加相同的节点。
</Tip>
