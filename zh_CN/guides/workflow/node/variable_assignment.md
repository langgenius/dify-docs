...
**自动判断提取并存储对话中的信息**，在会话内通过会话变量数组记录用户的重要信息，并在后续对话中使用这些信息来个性化回复。

示例：开始对话后，LLM会自动判断用户输入中是否包含需要记住的事实、偏好或历史记录。如果有，LLM会先提取并存储这些信息，然后再用这些信息作为上下文来回答。如果没有新的信息需要保存，LLM会直接使用之前相关的记忆来回答问题。
