...
description: .difypkg を準備、検証してマーケットプレイスに提出し、公開後のパフォーマンスとフィードバックを確認します
---
...
<Tabs>
  ...
  <Tab title="依存関係">
    依存関係は最小限にし、再現可能なインストールのためにバージョン制約を指定してください。URL や Git から依存関係を直接インストールすることはできません。
...
パッケージには、プラグインが収集、保存、記録するユーザーデータや、第三者へ送信するデータを説明した `PRIVACY.md` が必要です。`manifest.yaml` から公開済みのプライバシーポリシーへリンクすることもできます。ユーザーデータを収集しない場合も、その旨を明記してください。
...
## PR チェックとレビューへの対応 {#after-you-submit}
...
## クリエイターセンターでプラグインのパフォーマンスとフィードバックを確認 {#manage-published-plugins}
...
クリエイターセンターが組織による **ユニークハンドル** の所有を確認すると、そのハンドルで公開されたプラグインが組織に表示されます。チームメンバーを招待すると、それぞれが GitHub アカウントを連携しなくても同じプラグインを確認できます。
...
申請状況は **申請履歴** で確認できます。承認されると、そのハンドルで公開されたすべてのプラグインが対応する組織に表示されます。却下された場合は理由を確認し、**新規申請** を選択して証拠を更新し、再提出してください。
