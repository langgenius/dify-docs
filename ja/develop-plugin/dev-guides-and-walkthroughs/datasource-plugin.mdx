...
- [ステップ2：ナレッジパイプラインオーケストレーション](/en/guides/knowledge-base/knowledge-pipeline/knowledge-pipeline-orchestration)
- [Difyプラグイン開発：Hello Worldガイド](/en/develop-plugin/dev-guides-and-walkthroughs/tool-plugin)
...
<Info>
  親クラスを継承してプラグイン機能を実装する方法については、[Difyプラグイン開発：Hello Worldガイド - 4.4 ツールロジックの実装](/en/develop-plugin/dev-guides-and-walkthroughs/tool-plugin#4-4-implementing-tool-logic)を参照してください。
</Info>
...
<Info>
  プロバイダーYAMLファイルの作成について詳しくは、[Difyプラグイン開発：Hello Worldガイド-4.3 プロバイダー認証情報の設定](/en/develop-plugin/dev-guides-and-walkthroughs/tool-plugin#4-3-configuring-provider-credentials)を参照してください。
</Info>

<Info>
  データソースプラグインはOAuth 2.0またはAPIキーによる認証をサポートしています。

  OAuthの設定については、[ツールプラグインにOAuthサポートを追加する](/en/develop-plugin/dev-guides-and-walkthroughs/tool-oauth)を参照してください。
</Info>
...
<Info>
  プラグイン公開プロセスについては、[プラグインの公開](/en/develop-plugin/publishing/marketplace-listing/release-overview)を参照してください。
</Info>
