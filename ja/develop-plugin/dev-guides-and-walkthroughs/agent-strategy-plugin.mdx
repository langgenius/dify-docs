...
プラグインパッケージを [Dify プラグインリポジトリ](https://github.com/langgenius/dify-plugins) に提出できます。提出前に、[マーケットプレイス向けの準備と検証要件](/ja/develop-plugin/publishing/marketplace-listing/submit-plugin-to-marketplace#パッケージの準備) を満たしていることを確認してください。承認されると、パッケージがメインブランチにマージされ、マーケットプレイスのパイプラインから公開されます。
