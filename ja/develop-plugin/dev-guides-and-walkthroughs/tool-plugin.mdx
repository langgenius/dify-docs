...
プラグイン開発スキャフォールディングツールの準備方法の詳細については、[開発ツールの初期化](/plugin-dev-en/0221-initialize-development-tools)を参照してください。初めてプラグインを開発する場合は、まず[Difyプラグイン開発：Hello Worldガイド](/plugin-dev-en/0211-getting-started-dify-tool)を読むことをお勧めします。
...
> すでにプラグイン開発に精通しており、テンプレートに依存する必要がない場合は、[一般仕様](/plugin-dev-en/0411-general-specifications)ガイドを参照して、さまざまなタイプのプラグインの開発を完了できます。
...
-   `credentials_for_provider`のサブレベル構造は、[一般仕様](/plugin-dev-en/0411-general-specifications)の要件を満たす必要があります。
...
この例は、`serpapi`にリクエストを送信し、`self.create_json_message`を使用してフォーマットされた`json`データ文字列を返すことを意味します。返却データタイプの詳細については、[プラグインのリモートデバッグ](/plugin-dev-en/0411-remote-debug-a-plugin)と[永続ストレージKV](/plugin-dev-en/0411-persistent-storage-kv)ドキュメントを参照してください。
...
プラグインをDify Marketplaceに公開したい場合は、プラグインが[Dify Marketplaceへの公開](/plugin-dev-en/0322-release-to-dify-marketplace)の仕様に従っていることを確認してください。レビューに合格すると、コードはメインブランチにマージされ、自動的に[Dify Marketplace](https://marketplace.dify.ai/)で公開されます。

[公開の概要](/plugin-dev-en/0321-release-overview)
...
-   [一般仕様](/plugin-dev-en/0411-general-specifications) - マニフェスト構造とツール仕様
-   [Endpoint](/plugin-dev-en/0432-endpoint) - 詳細なEndpointの定義
-   [リバース呼び出し](/plugin-dev-en/9241-reverse-invocation) - Dify機能のリバース呼び出し
-   [モデルスキーマ](/plugin-dev-en/0412-model-schema) - モデル
-   [エージェントプラグイン](/plugin-dev-en/9232-agent) - エージェント戦略の拡張

## 次の学習ステップ

- [プラグインのリモートデバッグ](/plugin-dev-en/0411-remote-debug-a-plugin) - より高度なデバッグテクニックを学ぶ
- [永続ストレージ](/plugin-dev-en/0411-persistent-storage-kv) - プラグインでのデータストレージの使用方法を学ぶ
- [Slack Botプラグイン開発例](/plugin-dev-en/0432-develop-a-slack-bot-plugin) - より複雑なプラグイン開発ケースを見る
- [ツールプラグイン](/plugin-dev-en/0411-tool) - ツールプラグインの高度な機能を探索
