...
- **`parameters`**：パラメータリスト。
  ...
  - **`type`**（必須）：パラメータタイプ。値は次の 3 種類に分かれます。
    ...
    - **セレクターとアップロード**：
        - `secret-input` は機密情報に使い、暗号化入力ボックスとして表示されます。
        - `file` と `files` はファイルのアップロードに使います。
        - `app-selector` と `model-selector` はワークスペースのアプリやモデルの選択に使います。
        - `date` は単一の日付に使い、日付ピッカーとして表示され、値は `YYYY-MM-DD` 形式の文字列として渡されます。
        - `date-range` は開始日と終了日に使い、日付範囲ピッカーとして表示され、値は `start` と `end` の日付文字列を含むオブジェクトとして渡されます。
  ...
  - **`multiple`**（任意）：`select` と `dynamic-select` パラメータで `true` にすると、複数のオプションを選択できます。値はオプション値のリストとして渡され、`default` を設定する場合はリストにする必要があります。
