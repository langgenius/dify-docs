...
<Steps>
  <Step title="アプリをストリーミングモードで起動">
    ...
    1. Workflow アプリは [ワークフローを実行](/ja/api-reference/workflows/run-workflow) を呼び出します。Chatflow アプリは [チャットメッセージを送信](/ja/api-reference/chats/send-chat-message) を呼び出します。
  ...
  <Step title="ワークフローの監視を継続">
    元の SSE ストリームが切断されている場合は、手順 1 の `workflow_run_id` と、実行を開始したときと同じ `user` を使って、[Workflow](/ja/api-reference/workflows/stream-workflow-events) または [Chatflow](/ja/api-reference/workflows/stream-workflow-events) のワークフローイベントをストリームエンドポイントに再接続します。`user` が異なると 404 が返されます。
