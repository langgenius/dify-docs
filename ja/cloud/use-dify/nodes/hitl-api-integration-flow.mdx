...
<Steps>
  ...
  <Step title="フォーム定義を取得">
    `form_token` を指定して [人間の入力フォームを取得](/ja/api-reference/human-input/get-human-input-form) エンドポイントを呼び出します。レスポンスにはレンダリング済みの Markdown、入力フィールド定義、利用可能なアクション、事前入力済みのデフォルト値、そして `expiration_time`（この時刻を過ぎるとフォームを送信できなくなります）が含まれます。フォームを受信者に表示します。
  </Step>
