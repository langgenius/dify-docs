...
<Steps>
  ...
  <Tip>
    ...
    | 変数 | 説明 | 例 |
    | --- | --- | --- |
    | `DIFY_VERSION` | `difyctl` を取得する Dify リリースタグ。[Dify Releases](https://github.com/langgenius/dify/releases) のタグと一致させる（`v` プレフィックスなし）。既定は最新リリース。サーバーが最新でない場合は、そのリリースタグを指定する。 | `1.15.0` |
    | `DIFYCTL_VERSION` | 特定の `difyctl` ビルドに固定する（`v` プレフィックスなし）。`DIFY_VERSION` が未設定のときのみ使用される。 | `0.1.0-alpha` |
