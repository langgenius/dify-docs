...
- エージェントが動作するマシンで `difyctl` を [インストール](/ja/cli/install) し、[サインイン](/ja/cli/integrate-agents/auth-for-agent-deployments) して、セッションを再利用できるようにします。
...
| コマンドが終了コード 4 で失敗する | エージェントに再利用できるセッションがありません。まずそのマシンで [サインイン](/ja/cli/integrate-agents/auth-for-agent-deployments) してください。 |
