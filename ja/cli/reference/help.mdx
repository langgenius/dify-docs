...
すべてのヘルプ領域が `-o json` と `-o yaml` を受け付けます。`difyctl` を駆動するコーディングエージェントは、主に次の 2 つを使います。[コンパクトなコマンド一覧](#コンパクトなコマンド一覧) でコマンドを選び、そのコマンドの [ディスクリプタ](#コマンドごとのディスクリプタ) で引数とフラグを読み取ります。[完全なドキュメント](#完全なドキュメント) は、コマンド横断の規約とトピック一覧も 1 回でまとめて返します。

### コンパクトなコマンド一覧

トップレベルのヘルプに `--compact` を付けると、各コマンドが 3 つのフィールドだけで出力されます。`command`（コマンドのパス）、`description`（1 行の説明）、`effect` ラベル（`read`、`write`、`destructive`）です。それ以外は含まれないため、一覧全体が小さく、コマンドを選ぶ前に一度で読み切れます。
...
`--compact` はトップレベルのヘルプ専用で、`-o json` または `-o yaml` が必要です。コマンドごとのヘルプに付けた場合や、構造化形式を指定しない場合は、使用法エラーとして終了コード `2` で失敗します。

### コマンドごとのディスクリプタ

コマンドを選んだら、そのディスクリプタが一覧に含まれない情報をすべて補います。コマンドごとのヘルプは、どちらの書き方でもディスクリプタを返します。
...
各ディスクリプタには、型とデフォルト値を含むコマンドの引数とフラグ、例、`effect` ラベルが含まれます。存在する場合は、エージェント向けのコマンド固有のガイダンスである `agentGuide` 文字列も含まれます。

### 完全なドキュメント

`--compact` を付けない場合、トップレベルのヘルプはコマンドの全体像を 1 つのドキュメントとして出力します。
...
- `bin`：実行ファイル名（`difyctl`）
- `contract`：終了コード、出力形式、エラー出力の構造、コマンド横断のルール
- `commands`：コマンドごとに 1 つの完全なディスクリプタ
- `topics`：各ガイドトピックの名前と 1 行の概要。本文は `difyctl help <topic>` で読み取る
...
| `2` | トップレベルの `-o json` または `-o yaml` ヘルプ以外での `--compact` |
