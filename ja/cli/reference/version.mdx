...
`difyctl version` は判定を報告するだけです。他のすべてのコマンドは実行前に判定に従って動作するため、`too_old` のサーバーでは、バージョンの不一致を解消するまでこれらのコマンドは実行できません。

- **`too_old`**：コマンドはリクエストを送信する前に終了コード [`6`](/ja/cli/reference/output-formats-and-exit-codes) で停止し、Dify サーバーをこのビルドがサポートする最小バージョン以上にアップグレードする（または、[サーバーに合う `difyctl` をインストールする](/ja/cli/install)）よう促します。
- **`too_new`**：コマンドはそのまま実行され、stderr に 1 行の警告を表示します。
- **`unknown`**：判定できないため、コマンドはそのまま実行されます。

判定が通った結果はホストごとに約 1 時間キャッシュされるため、`difyctl` はコマンドごとに再探査しません。サーバーをアップグレードすると、キャッシュは直ちにクリアされます。
...
`difyctl` と Dify サーバーはバージョンを合わせ、常に同時にアップグレードしてください。

## スクリプトを互換性でゲートする

`--check-compat` は判定をスクリプトで扱えるようにします。`compatible` 以外はすべて、`unknown` のあらゆるケースを含め、`64` で終了します。この確認は毎回サーバーをライブで探査するため、スクリプトのゲートは常にサーバーの現在のバージョンを反映し、キャッシュされた結果は使いません。
