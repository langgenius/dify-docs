...
> このドキュメントは AI によって自動翻訳されています。不正確な部分がある場合は、[英語版](/en/cli/reference/skills) を参照してください。

[`skills install`](#スキルのインストール) は、`difyctl` スキル（`SKILL.md` ファイル）を、お使いのマシン上にある各コーディングエージェントのスキルディレクトリへ書き込みます。

このスキルには、あえてコマンドを一切記載していません。代わりにエージェントを `difyctl help -o json --compact` とコマンドごとのヘルプに誘導し、利用可能なコマンド群をエージェント自身に発見させます。

これらの発見の仕組みは [help リファレンス](/ja/cli/reference/help#機械可読ヘルプ) を、統合の全体像は [difyctl スキルをインストールする](/ja/cli/integrate-agents/install-the-difyctl-skill) を参照してください。
...
| `--agent` | string、繰り返し可能 | なし | インストール対象を、検出済みの特定エージェントに名前で限定します。名前は [エージェント検出](#エージェント検出) を参照してください。<br></br><br></br>`--agent` を指定しない場合、スキルは検出されたすべてのエージェントにインストールされます。すべてではなく複数のエージェントにインストールするには、フラグを繰り返すか、カンマ区切りのリストを渡します。<br></br><br></br>検出されていないエージェントを指定すると、使用方法のエラーになります。 |
...
複数の特定のエージェントにインストールする（フラグを繰り返すか、カンマで区切る）：
