...
<Note>
ワークスペースはアカウントユーザーにのみ適用されます。SSO でサインインした場合は、ワークスペースを持たない単一のコンテキストに限定されるため、ここで説明するコマンドは適用されません。詳細は [権限範囲と制限](/ja/cli/sso-users/scope-and-limitations) を参照してください。
</Note>
...
<Tip>
日常的な呼び出し方法については、よく使うタスクの [ワークスペースの切り替え](/ja/cli/account-users/common-tasks#ワークスペースの切り替え) を参照してください。
</Tip>
