...
| `--insecure` | boolean | false | `http://` ホストを許可し、TLS 証明書の検証をスキップします（ローカル開発のみ）。 |
...
| `6` | サーバーがこの `difyctl` には古すぎる（セッションを保存する前に確認）、またはサーバーにデバイスフローのサインインエンドポイントがない（`unsupported_endpoint`） |
| `7` | レート制限（HTTP 429） |
