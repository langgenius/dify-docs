...
| フラグ | 型 | デフォルト | 説明 |
|:---|:---|:---|:---|
| `--host <url>` | string | none | サインイン先の Dify ホスト。Dify Cloud の場合は `https://cloud.dify.ai` を使用します。`difyctl` はコンソール API に対して動作するため、セルフホストの Dify では [コンソール API URL](/ja/self-host/deploy/configuration/environments#console_api_url) を指定します。スキームのない URL には `https://` が付与されます。ターミナルでは省略でき、その場合はホストの入力を求められます。非対話セッション（スクリプト、CI、パイプ）ではこのフラグは必須です。 |
...
| フラグ | 型 | デフォルト | 説明 |
|:---|:---|:---|:---|
| `--domain <host>` | string | none | 切り替え先の保存済みホスト。スキームを付けず、素のドメイン（`cloud.dify.ai`）で指定します。ターミナルでは省略して、保存済みのホストから選択できます。非対話セッション（スクリプト、CI、パイプ）ではこのフラグは必須です。 |
