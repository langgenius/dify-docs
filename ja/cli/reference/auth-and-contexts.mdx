...
| フラグ | 型 | デフォルト | 説明 |
|:---|:---|:---|:---|
| `--host <url>` | string | none | サインイン先の Dify ホスト。`difyctl` はコンソール API に対して動作するため、セルフホストの Dify では [コンソール API URL](/ja/self-host/deploy/configuration/environments#console_api_url) を指定します。スキームのない URL には `https://` が付与されます。ターミナルでは省略でき、その場合はホストの入力を求められます。非対話セッション（スクリプト、CI、パイプ）ではこのフラグは必須です。 |
