...
- **自分のマシン**：サインイン（[アカウント](/ja/3.14.x/develop/cli/account-users/authenticate) または [SSO](/ja/3.14.x/develop/cli/sso-users/authenticate)）すれば完了です。
...
`--host` に [コンソール API URL](/ja/3.14.x/deploy/advanced-configuration/environment-variables#console_api_url) を指定します。
...
<Steps>
  ...
  <Step title="設定ディレクトリを対象マシンへコピー">
    コピー後、[`DIFY_CONFIG_DIR`](/ja/3.14.x/develop/cli/reference/environment-variables) で `difyctl` をそのディレクトリに向けます。コンテナの場合は、イメージに焼き込むのではなく、実行時にディレクトリをマウントします。
...
そのため、リトライするのではなく、処理を停止して失敗を表面化させ、人が再サインインできるようにします。これに基づく分岐方法は、[エラーとリトライの処理](/ja/3.14.x/develop/cli/integrate-agents/error-handling-and-retries-for-agents#終了コードで分岐する) を参照してください。

漏洩が疑われるセッションを取り消すには、サインイン済みの任意のマシンから [`auth devices revoke`](/ja/3.14.x/develop/cli/reference/auth-and-contexts#セッションを取り消す) を実行します。
