...
<Note>
`difyctl` の各ビルドは特定の Dify バージョンに対応しています。サーバーに合ったビルドをインストールしてください（後述の `DIFY_VERSION` を参照）。
</Note>
...
<Steps>
  <Step title="インストーラーの実行">
    `difyctl` のビルドは Dify の各リリースに同梱されています。Dify Enterprise 3.13.x は Dify `1.17.0` で動作するため、対応するビルドをインストールします：
  ...
  <Tip>
    ...
    | 変数 | 説明 | 例 |
    | --- | --- | --- |
    | `DIFY_VERSION` | `difyctl` を取得する Dify リリースタグ。[Dify Releases](https://github.com/langgenius/dify/releases) のタグと一致させる。<br></br><br></br>既定は最新リリース。サーバーが最新でない場合は、そのリリースタグを指定する。 | `1.17.0` |
...
アンインストール前にサインアウトするには、[`difyctl auth logout`](/ja/3.14.x/develop/cli/reference/auth-and-contexts#サインアウト) を実行します。
