...
| `DIFY_WORKSPACE_ID` | `--workspace` フラグが設定されていない場合に `difyctl` コマンドの対象となるワークスペース。UUID である必要があります。完全な解決順序は [difyctl がワークスペースを選択する仕組み](/ja/3.14.x/develop/cli/reference/workspaces#difyctl-がワークスペースを選択する仕組み) を参照してください。 | なし |
| `DIFYCTL_HTTP_RETRY` | 一時的な障害が発生した際の、冪等なリクエストの再試行回数（`0` で再試行を無効化）。[`--http-retry`](/ja/3.14.x/develop/cli/reference/global-flags#http-リトライ) が上書きします。 | `3` |
...
<Note>
`env list` が受け取るのは `--json` であり、[`-o` グローバルフラグ](/ja/3.14.x/develop/cli/reference/global-flags) ではありません。
</Note>
...
完全な体系は [出力フォーマットと終了コード](/ja/3.14.x/develop/cli/reference/output-formats-and-exit-codes) を参照してください。
