...
- [`use workspace`](#ワークスペースの切り替え) はアクティブなワークスペースを切り替えます

どちらも [グローバルフラグ](/ja/3.14.x/develop/cli/reference/global-flags) を受け付けます。

<Note>
ワークスペースはアカウントユーザーにのみ適用されます。外部 SSO でサインインした場合は、ワークスペースを持たない単一のコンテキストに限定されるため、ここで説明するコマンドは適用されません。詳細は [権限範囲と制限](/ja/3.14.x/develop/cli/sso-users/scope-and-limitations) を参照してください。
</Note>
...
全体の体系については [出力形式と終了コード](/ja/3.14.x/develop/cli/reference/output-formats-and-exit-codes) を参照してください。
...
<Tip>
日常的な呼び出し方法については、よく使うタスクの [ワークスペースの切り替え](/ja/3.14.x/develop/cli/account-users/common-tasks#ワークスペースの切り替え) を参照してください。
</Tip>
...
[グローバルフラグ](/ja/3.14.x/develop/cli/reference/global-flags) のみです。
...
2. [`DIFY_WORKSPACE_ID`](/ja/3.14.x/develop/cli/reference/environment-variables) 環境変数。
