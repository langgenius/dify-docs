...
`difyctl` は、複数の Dify ホストとアカウントへのサインインを同時に保持できます。各ホストとアカウントの組み合わせが 1 つのコンテキストです。そのメタデータは [設定ディレクトリ](/ja/3.14.x/develop/cli/reference/environment-variables) 内の `hosts.yml` に保存され、bearer token は OS の認証情報ストアに保存されます（保護されたファイルへのフォールバックあり）。
...
サインインの手順については、[アカウントユーザー](/ja/3.14.x/develop/cli/account-users/authenticate) または [外部 SSO ユーザー](/ja/3.14.x/develop/cli/sso-users/authenticate) の認証ドキュメントを参照してください。
...
外部 SSO でサインインすると、同じデバイスフローが会社の ID プロバイダー経由でルーティングされ、標準のアカウントコンテキストではなく `external SSO` コンテキスト（`dfoe_` token）が生成されます。詳細は [外部 SSO ユーザー：認証](/ja/3.14.x/develop/cli/sso-users/authenticate) を参照してください。
...
すべての終了コードについては、[出力形式と終了コード](/ja/3.14.x/develop/cli/reference/output-formats-and-exit-codes) を参照してください。
...
| フラグ | 型 | デフォルト | 説明 |
|:---|:---|:---|:---|
| `--domain <host>` | string | none | 切り替え先の保存済みホスト。スキームを付けず、素のドメイン（`dify.example.com`）で指定します。<br></br><br></br>ターミナルでは省略して、保存済みのホストから選択できます。非対話セッション（スクリプト、CI、パイプ）ではこのフラグは必須です。 |
...
| フラグ | 型 | デフォルト | 説明 |
|:---|:---|:---|:---|
| `--email <email>` | string | none | 切り替え先のアカウント。<br></br><br></br>ターミナルでは省略して、保存済みのアカウントから選択できます。非対話セッション（スクリプト、CI、パイプ）ではこのフラグは必須です。 |
