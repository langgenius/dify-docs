...
このガイドでは、Slack 内でユーザーの質問に直接回答する、AI を活用した Slack Bot を構築します。プラグインを開発したことがない場合は、まず [プラグイン開発クイックスタートガイド](/ja/3.14.x/develop/plugins/dev-guides-and-walkthroughs/tool-plugin) を読んでください。
...
- **Dify プラグイン開発ツール**：[開発ツールの初期化](/ja/3.14.x/develop/plugins/getting-started/cli) を参照してください。
...
コーディングを始める前に、[クイックスタート：拡張プラグインの開発](/ja/3.14.x/develop/plugins/dev-guides-and-walkthroughs/endpoint) を読んでいるか、または Dify プラグインを構築した経験があることを確認してください。
...
プラグイン内で Dify サービスを逆呼び出しする方法の詳細は、[逆呼び出し：App](/ja/3.14.x/develop/plugins/features-and-specs/advanced-development/reverse-invocation-app) を参照してください。
...
プラグインが正しく動作することを確認したら、以下のコマンドでパッケージ化します。このコマンドは、現在のディレクトリに `slack_bot.difypkg` ファイル、つまり最終的なプラグインパッケージを生成します。詳細なパッケージ化手順は、[ローカルファイルとしてパッケージ化して共有](/ja/3.14.x/develop/plugins/publishing/marketplace-listing/release-by-file) を参照してください。
...
これで、[Dify マーケットプレイスリポジトリ](https://github.com/langgenius/dify-plugins) にアップロードして一般公開できます。公開前に、プラグインが [Dify マーケットプレイス公開ガイドライン](/ja/3.14.x/develop/plugins/publishing/marketplace-listing/release-to-dify-marketplace) を満たしていることを確認してください。承認されると、コードはメインブランチにマージされ、プラグインは [Dify マーケットプレイス](https://marketplace.dify.ai/) で公開されます。

---

## 関連リソース

- [プラグイン開発の基礎](/ja/3.14.x/develop/plugins/getting-started/getting-started-dify-plugin)：Dify プラグイン開発の包括的な概要
- [プラグイン開発クイックスタートガイド](/ja/3.14.x/develop/plugins/dev-guides-and-walkthroughs/tool-plugin)：ゼロからプラグイン開発を始める
- [拡張プラグインの開発](/ja/3.14.x/develop/plugins/dev-guides-and-walkthroughs/endpoint)：拡張プラグインの開発
- [Dify サービスの逆呼び出し](/ja/3.14.x/develop/plugins/features-and-specs/advanced-development/reverse-invocation)：Dify プラットフォームの機能を呼び出す方法
- [逆呼び出し：App](/ja/3.14.x/develop/plugins/features-and-specs/advanced-development/reverse-invocation-app)：プラットフォーム内でアプリを呼び出す方法
- [プラグインの公開](/ja/3.14.x/develop/plugins/publishing/marketplace-listing/release-overview)：公開プロセス
- [Dify マーケットプレイスへの公開](/ja/3.14.x/develop/plugins/publishing/marketplace-listing/release-to-dify-marketplace)：マーケットプレイス公開ガイド
- [エンドポイントの詳細定義](/ja/3.14.x/develop/plugins/dev-guides-and-walkthroughs/endpoint)：エンドポイントのリファレンス
...
- [バンドルプラグイン：複数のプラグインのパッケージ化](/ja/3.14.x/develop/plugins/features-and-specs/advanced-development/bundle)

**プラグインインターフェースドキュメント**：

- [Manifest ファイルによるプラグイン情報の定義](/ja/3.14.x/develop/plugins/features-and-specs/plugin-types/plugin-info-by-manifest)：Manifest 構造
- [エンドポイント](/ja/3.14.x/develop/plugins/dev-guides-and-walkthroughs/endpoint)：エンドポイントのリファレンス
- [逆呼び出し](/ja/3.14.x/develop/plugins/features-and-specs/advanced-development/reverse-invocation)：プラグインから Dify 機能を呼び出す
- [一般仕様](/ja/3.14.x/develop/plugins/features-and-specs/plugin-types/general-specifications)：ツール仕様
- [モデルスキーマ](/ja/3.14.x/develop/plugins/features-and-specs/plugin-types/model-schema)：モデルスキーマのリファレンス
