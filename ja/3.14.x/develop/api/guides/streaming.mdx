...
`streaming`（ストリーミングモード）は、返信を SSE イベントとして届けます。ユーザーに見せる処理、長時間の実行、[人間の入力](/ja/3.14.x/develop/api/guides/human-input-flow) で一時停止するすべてのフローには、こちらを使ってください。
...
`ping` を除き、各イベントは 1 行の `data: ` として届き、1 つの JSON オブジェクトを含みます。イベントの終わりは空行です。

`event` フィールドを読んで処理内容を判断し、`data: ` で始まらない行はスキップします。キープアライブの `ping` は、`data:` ペイロードのない `event: ping` 行だけで構成されます。

Chatflow アプリと Workflow アプリのストリームでは、最初のフレームが必ず `ping` になります。その後も実行中はおよそ 10 秒ごとに `ping` が届きます。実行が受け付けられたことは、最初のフレームではなく、最初の `data:` イベント（`workflow_started`）で判断してください。
...
届くイベントはアプリタイプによって異なります。イベントの一覧は [チャットメッセージを送信](/ja/3.14.x/develop/api/chat-messages/send-chat-message)、[ワークフローを実行](/ja/3.14.x/develop/api/workflow-runs/run-workflow)、[完了メッセージを送信](/ja/3.14.x/develop/api/completion-messages/send-completion-message) の各イベント表を参照してください。
...
- `task_id` は進行中の生成を制御します。停止エンドポイント（[生成を停止](/ja/3.14.x/develop/api/chat-messages/stop-chat-message-generation)、[ワークフロータスクを停止](/ja/3.14.x/develop/api/workflow-runs/stop-workflow-task)）が受け取るのはこの値です。
...
ワークフローが背後にある実行（Workflow アプリと Chatflow アプリ）では、接続が切れても致命的ではありません。[ワークフローイベントをストリーム](/ja/3.14.x/develop/api/workflow-runs/stream-workflow-events) でストリームを開き直し、`workflow_run_id` と、実行を開始したのと同じ `user` を渡します。`user` が一致しない場合は 404 が返ります。
...
実行中のワークフローに再接続した後は、[ワークフロー実行詳細を取得](/ja/3.14.x/develop/api/workflow-runs/get-workflow-run-detail) で完了を確認してください。再接続したストリームの最後のイベントだけに頼るのは避けてください。

それ以外の返信には再開用のエンドポイントがありません。返信の途中で接続が切れた場合は、新しいリクエストを送ってください。チャット系アプリでは、[会話履歴メッセージ一覧を取得](/ja/3.14.x/develop/api/conversations/list-conversation-messages) で会話に保存された内容を確認できます。
