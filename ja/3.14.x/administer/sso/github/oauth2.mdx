...
<Tabs>
  <Tab title="Workspace">
   **管理バックエンド** → **認証** → **メンバー** → **SSO アイデンティティプロバイダー** → **New Identity Provider** → **New OAuth2 Provider** をクリックし、底部で Workspace Callback URL を確認します。
  ...
  <Tab title="WebApp External Users（外部ユーザー向け）">
   Dify エンタープライズ版の「認証」ページをクリックし、Web 外部ユーザー認証内の SSO オプション内の **"+ New Identity Provider → New OAuth2 Provider"** をクリックし、底部で Web App Callback URL を確認します。
