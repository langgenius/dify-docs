...
有効化する必要がある SSO 範囲に応じて、**コールバック URL は異なります**。システム管理者は、Dify エンタープライズのコールバック URL を対応する Okta アプリケーション内に入力して、作成プロセスを完了する必要があります。

<Tabs>
  <Tab title="Workspace">
   **管理バックエンド** → **認証** → **メンバー** → **SSO アイデンティティプロバイダー** → **New Identity Provider** → **New OAuth2 Provider** をクリックし、一番下に記載のある Workspace Callback URL を確認します。
  ...
  <Tab title="WebApp External Users（外部ユーザー向け）">
      **管理コンソール** → **認証** → **ウェブアプリ** → **SSO IDプロバイダー** → **New Identity Provider** → **New OAuth2 Provider** をクリックし、下部でWeb App Callback URLを確認します。
...
3. "Sign On" ページに切り替え、**Issuer** を固定リンクに設定します。
...
> `your-okta-issuer-url` の取得方法：**Security  → API  → Authorization Servers → Issuer URI**。Okta App を通じてより多くのフィールド情報を取得する方法については、[Use of the Issuer](https://support.okta.com/help/s/article/What-is-theIssuerlocated-under-the-OpenID-Connect-ID-Token-app-settings-used-for?language=en_US)を参照してください。
