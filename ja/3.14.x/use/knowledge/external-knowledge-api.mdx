...
このページでは、Dify が外部ナレッジサービスからコンテンツを取得するために実装が必要な API 仕様を定義します。API の準備ができたら、[外部ナレッジベースとの接続](/ja/3.14.x/use/knowledge/connect-external-knowledge-base) を参照して Dify に登録してください。
