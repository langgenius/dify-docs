...
この API エンドポイントは公開ネットワークでアクセス可能になります。これで、Dify でこの API エンドポイントを設定してローカルコードをデバッグできます。設定手順については、[外部データツール](/ja/3.14.x/use/workspace/api-extension/external-data-tool-api-extension) を参照してください。
...
詳細な説明については、[Cloudflare Workers を使用したカスタム API のデプロイ](/ja/3.14.x/use/workspace/api-extension/cloudflare-worker) を参照してください。
