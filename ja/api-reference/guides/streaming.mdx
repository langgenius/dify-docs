...
`blocking`（ブロッキングモード）は、生成完了後に JSON ボディを 1 回で返します。短い非対話型の呼び出しでは統合が簡単です。ただし長い生成には中断のリスクがあります。プロキシは長いリクエストを切断することがあり、Dify Cloud では、エッジプロキシがタイムアウト時間内に上流からのレスポンスを受け取れない場合、接続が中断されることがあります。
...
<Info>
レガシー Agent と Agent アプリはストリーミングモードのみ対応です。
</Info>
...
1. 返信チャンクを順番に連結します。
    ...
    - レガシー Agent と Agent アプリでは `agent_message` イベント

    Agent アプリでは、最後に完全な回答を繰り返す `message` イベントが 1 つ届きます。追加のテキストではなく、最終回答として扱ってください。

2. 正しい終端イベントで締めくくります。
    - チャットボット、レガシー Agent、Agent アプリは `message_end`
