...
2. **[移行スクリプト](https://github.com/langgenius/dify-docs/blob/main/assets/migrate_weaviate_collections.py)を実行**：
...
[ステップ A5](#step-a5-migrate-the-schema) と同じコマンドに従います。必要に応じて virtualenv を作成し、`weaviate-client` 4.x をインストールし、`migrate_weaviate_collections.py` を実行してから、`api`、`worker`、`worker_beat` を再起動します。
