...
[ステップ A5](#ステップ-a5：スキーマを移行) と同じコマンドに従います。必要に応じて virtualenv を作成し、`weaviate-client` 4.x をインストールし、`migrate_weaviate_collections.py` を実行してから、`api`、`worker`、`worker_beat` を再起動します。
