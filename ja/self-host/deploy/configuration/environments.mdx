...
新しい Agent（ベータ版）を Web UI に表示します。メインナビゲーションに **エージェント名簿** が表示され、ワークフローのノード一覧に新しい Agent ノードが加わります。オフの場合、エージェント名簿ページは 404 を返し、ワークフローではクラシック Agent ノードのみ利用できます。オンにしても表示されるのは UI のみで、Agent の実行には Agent ランタイムサービスのデプロイも必要です。ソースコードからのデプロイでは、代わりに `NEXT_PUBLIC_ENABLE_AGENT_V2` を設定してください。
