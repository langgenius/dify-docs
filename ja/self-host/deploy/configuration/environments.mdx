...
| `MARKETPLACE_ENABLED` | `true` | Dify Marketplace のプラグイン機能を有効または無効にします。無効にすると、Marketplace プラグインの閲覧・インストール画面が非表示になります。API は Marketplace からのプラグインのインストールとアップグレードを拒否します。定期アップグレードチェック（`ENABLE_CHECK_UPGRADABLE_PLUGIN_TASK`）も停止します。<br/><br/>テンプレートのインポート処理自体には影響しません。ただし、インポートしたテンプレートが未インストールの Marketplace プラグインに依存する場合があります。Marketplace が無効な間は、その依存プラグインをインストールできません。インストール済みのプラグインはそのまま動作し、GitHub やローカルパッケージからのインストールも引き続き利用できます。 |
| `MARKETPLACE_API_URL` | `https://marketplace.dify.ai` | プラグインとテンプレートの閲覧・ダウンロード、およびプラグインのアップグレード確認に使う Marketplace API エンドポイント。<br/><br/>Dify API サービス、Web サービス、ユーザーのブラウザでこの URL を使用するため、すべてからアクセスできる必要があります。 |
| `MARKETPLACE_URL` | （空。Compose 構成では `https://marketplace.dify.ai` にフォールバック） | Marketplace リンクのベース URL。プラグイン詳細ページや「統合」ページの Marketplace 入口に使用します。Dify は Creator Center のガイドリンク生成にもこの値を使用します。<br/><br/>`MARKETPLACE_API_URL` とは用途が異なります。コンソールとプラグインデーモンが Marketplace データの取得に使います。 |
...
| `CREATORS_PLATFORM_OAUTH_CLIENT_ID` | （空） | Creator Center へのパスワードレス SSO に使用する OAuth クライアント ID。セルフホスト環境では空のままにしてください。空でも公開できますが、Creator Center への手動ログインが必要です。デプロイの OAuth サーバーに未登録の ID を設定すると、**マーケットプレイスに公開** が失敗します。 |
