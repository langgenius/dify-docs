...
Web アプリフロントエンド（公開アプリ）のバックエンド API URL。この変数は Web フロントエンドコンテナでのみ使用され、Python バックエンドには影響しません。空の場合、Docker イメージのデフォルトは `http://127.0.0.1:5001`です。
...
公開 Web アプリのアクセス URL。ワークフローの **人間の入力ノード** に必要です。メール通知のフォームリンクは `{APP_WEB_URL}/form/{token}` 形式で構築されます。空の場合、人間の入力メールに有効なフォームリンクが含まれません。
...
Webhook およびインテグレーショントリガーエンドポイントのアクセス URL。外部システムがこのアドレスを使用してワークフローを呼び出します。Dify はトリガーコールバック URL を `{TRIGGER_URL}/triggers/webhook/{id}` の形式で構築し、コンソールに表示します。
...
大きなワークフローの共同編集中にエディタが切断と再接続を繰り返す場合は、`WEBSOCKET_MAX_HTTP_BUFFER_SIZE` を上げてください。
...
| `DEPLOYMENT_EDITION` | `COMMUNITY` | デプロイのエディション（`COMMUNITY`、`ENTERPRISE`、`CLOUD`）。デフォルトのまま使用してください。<br/><br/>従来の `EDITION`、`ENTERPRISE_ENABLED`、`BILLING_ENABLED` を置き換えるものです。これらの旧変数は 1.17.0 以降無視されます。 |
...
保存された資格情報（モデルプロバイダーの API キー、ツール資格情報などワークスペースの機密情報）を暗号化する、ワークスペースごとの鍵の保管場所を選択します。

- `local`：ワークスペースごとに RSA 鍵ペアを生成し、秘密鍵を `STORAGE_TYPE` のストレージバックエンドに保存します。

- `azure-keyvault`：各ワークスペースの RSA 鍵を Azure Key Vault に保管し、秘密鍵が Key Vault の外に出ることはありません。認証には `DefaultAzureCredential`（マネージド ID、環境変数、または Azure CLI ログイン）を使用します。

<Warning>
プロバイダーは最初のワークスペースを作成する前に決めてください。一方のプロバイダーで暗号化した資格情報はもう一方では復号できません。資格情報が保存済みのデプロイで切り替えると、再入力するまで読み取れなくなります。
</Warning>

| 変数 | デフォルト値 | 説明 |
|---|---|---|
| `AZURE_KEYVAULT_VAULT_URL` | （空） | Key Vault インスタンスの URL。`KEY_PROVIDER_TYPE` が `azure-keyvault` の場合は必須で、未設定だと API サービスが起動しません。例：`https://your-vault-name.vault.azure.net` |
| `AZURE_KEYVAULT_KEY_SIZE` | `2048` | Dify が Key Vault にワークスペース用の鍵を新規作成する際の RSA 鍵長（ビット）。新規作成時のみ適用され、既存の鍵のサイズは変わりません。 |
| `AZURE_KEYVAULT_ROTATION_INTERVAL_DAYS` | （空） | 設定すると（最小 7）、新規作成されるワークスペース鍵に N 日ごとの Key Vault 自動ローテーションポリシーが付与されます。古いバージョンの鍵は使用可能なまま残るため、ローテーション前に暗号化した資格情報も再暗号化なしで復号できます。空のままにすると、ローテーションは Azure 側で手動管理します。 |
...
| `APP_MAX_EXECUTION_TIME` | `3600` | アプリ実行の最大実行時間（秒）。超過すると終了されます。`WORKFLOW_MAX_EXECUTION_TIME` と連動する外側の制限で、両方ともデフォルトは 1 時間です。これはアプリキューレベル、もう一方はワークフローエンジンレベルで適用されます。ワークフローにより多くの時間が必要な場合は、両方を増やしてください。<br/><br/>Agent を利用する場合は、この 2 つの外側の制限を Agent の実行期限（`DIFY_AGENT_RUN_TIMEOUT_SECONDS`）以上に保ってください。下回ると Agent の実行が途中で打ち切られます。 |
...
| `WEBSOCKET_MAX_HTTP_BUFFER_SIZE` | `10485760` | 共同編集用 WebSocket が受け付ける 1 メッセージの最大バイト数（デフォルトは 10 MiB）。大きなワークフローの保存時にグラフが上限を超え、切断が繰り返される場合は値を上げる。<br/><br/>`/socket.io/` を処理する API プロセスに設定する。Docker Compose では `api_websocket` コンテナが該当する。 |
...
| `HUAWEI_OBS_ACCESS_KEY` | （空） | OBS access key。`HUAWEI_OBS_USE_OIDC` が `false` の場合のみ必須です。 |
| `HUAWEI_OBS_SECRET_KEY` | （空） | OBS secret key。`HUAWEI_OBS_USE_OIDC` が `false` の場合のみ必須です。 |
...
| `HUAWEI_OBS_USE_OIDC` | `false` | 静的な AK/SK の代わりに、OBS SDK の組み込み OIDC credential provider を使用します。Huawei Cloud IAM の ID プロバイダーと Kubernetes/CCE service account token が必要です。 |
| `HUAWEI_CLOUD_IDP_ID` | （空） | Huawei Cloud IAM の ID プロバイダー ID。`HUAWEI_OBS_USE_OIDC` が `true` の場合は必須です。 |
| `OIDC_TOKEN_FILE` | `/var/run/secrets/tokens/oidc-token` | OBS SDK が読み取る Kubernetes/CCE の projected service account token ファイル。デフォルト以外のパスを使用する場合のみ設定します。 |
...
ワークフロー実行ログをアーカイブするための独立した S3 互換ストレージ。`archive-workflow-runs` メンテナンスコマンドが、指定した期限より古いワークフロー実行を月単位の Parquet バンドルにまとめます。アーカイブから生成される CSV エクスポートはエクスポート用バケットに一時保存されます。
...
| `ARCHIVE_STORAGE_EXPORT_BUCKET` | （空） | アーカイブされたログから生成される一時的な CSV エクスポート ZIP 用バケット。バケットのライフサイクルルールで自動削除できます。 |
...
| `TIDB_VECTOR_ENABLE_FULLTEXT_SEARCH` | `false` | TiDB Vector 上のナレッジベースで全文検索とハイブリッド検索を利用できるようにします。<br/><br/>新規のナレッジベーステーブルには多言語対応の全文インデックスが付与され、既存テーブルには次回ドキュメントをインデックスする際に追加されます。<br/><br/>全文インデックスをサポートする TiDB デプロイが必要です。 |
...
| 変数 | デフォルト値 | 説明 |
|---|---|---|
| `ENABLE_LEARN_APP` | `true` | Learn Dify セクションの表示を切り替えます。対象は探索ページのチュートリアルテンプレート、スタジオにアプリがないときの入門パネル、ヘルプメニューの表示切り替えです。<br/><br/>`false` で 3 か所とも非表示になります。 |
| `ENABLE_SKILL` | `true` | メインナビゲーションに [Skills](/ja/self-host/use-dify/build/skills) を表示します。<br/><br/>`false` にすると、ワークスペースの UI からスキルが非表示になります。 |

#### 認証とアカウント

デフォルトでは、サインインはメールアドレスとパスワードで行い、新しいアカウントはワークスペースへの招待または初期セットアップウィザードで作成します。以下のトグルでこの方針を調整できます。

| 変数 | デフォルト値 | 説明 |
|---|---|---|
| `ENABLE_EMAIL_PASSWORD_LOGIN` | `true` | メールアドレスとパスワードでコンソールにサインインします。無効にすると、サインインページからパスワードフォームが消え、パスワードログイン、パスワードリセット、セルフサービス登録の各エンドポイントが 403 を返します。サインイン方法は必ず 1 つ以上有効にしてください。すべて無効にすると、認証方式が未設定である旨が表示され、設定を戻すまで誰もログインできません。 |
| `ENABLE_EMAIL_CODE_LOGIN` | `false` | パスワードの代わりに、メールで届く 6 桁のワンタイムコードでサインインします。メール送信の設定が必要です（[メール設定](#メール設定) を参照）。未設定ではコードが届きません。`ALLOW_REGISTER=true` の場合、未登録のメールアドレスでコードを入力するとアカウントも作成されるため、セルフサービス登録の経路にもなります。 |
| `EMAIL_CODE_LOGIN_TOKEN_EXPIRY_MINUTES` | `5` | サインインコードの有効時間（分）。期限切れのコードは拒否され、再取得が必要です。 |
| `EMAIL_CODE_LOGIN_MAX_ATTEMPTS` | `5` | 1 つのサインインコードで許容される入力ミスの最大回数。超えるとコードが無効になり、再取得が必要です。 |
| `ENABLE_SOCIAL_OAUTH_LOGIN` | `false` | コンソールのサインインページに GitHub と Google のサインインボタンを表示します。ボタンが機能するのは、対応する GitHub または Google の OAuth アプリ認証情報を API サービスに設定した場合だけです。フラグだけ有効にして認証情報が未設定だと、サインインは失敗します。初回の OAuth サインインでアカウントが作成されるのは `ALLOW_REGISTER=true` のときだけです。 |
| `ALLOW_REGISTER` | `false` | アカウントの自己作成を許可します。サインインページに新規登録リンクを表示し（パスワードログインが有効な場合）、初回のメールコードまたは OAuth サインインでのアカウント作成を許可します。招待制にするには `false` のままにします。ワークスペースへの招待と初期セットアップウィザードは常に利用できます。 |
| `ALLOW_CREATE_WORKSPACE` | `false` | どのワークスペースにも属さないアカウントに対して、セルフサービス登録時やメールコード、OAuth サインイン時にワークスペースを自動作成します。`false` の場合、こうしたアカウントはワークスペースへの招待を待つ必要があります。セットアップウィザードが作成する管理者ワークスペースには影響しません。 |
| `ENABLE_CHANGE_EMAIL` | `true` | アカウント設定ページからメールアドレスを変更できるようにします。変更はメールの確認コードで検証されるため、メール送信の設定が必要です。外部ディレクトリと一致させる必要がある場合など、メールアドレスを固定したいときは `false` に設定します。 |
...
| 変数 | デフォルト値 | 説明 |
|---|---|---|
| `OPS_TRACE_RETRYABLE_DISPATCH_MAX_RETRIES` | `780` | 一時的な失敗後にトレース配信をリトライする最大回数。超えるとそのトレースは破棄されます。<br/><br/>デフォルトのリトライ時間（回数 × `OPS_TRACE_RETRYABLE_DISPATCH_DELAY_SECONDS`）は、1 時間のワークフロー上限とエクスポート猶予期間を上回るよう設定されています。<br/><br/>現在この経路に入るのは、Arize/Phoenix でネストしたワークフローをトレースする場合のみです。子トレースが親のコンテキストより先に届くと発生します。`0` は指定できません。 |
| `OPS_TRACE_RETRYABLE_DISPATCH_DELAY_SECONDS` | `5` | リトライの間隔（秒）。許容ウィンドウは間隔 × 最大回数（デフォルトで約 65 分）です。<br/><br/>小さくするとネストしたトレースが早く届く一方、リトライ回数を早く使い切ります。`0` は指定できません。 |
| `OPS_TRACE_PARENT_CONTEXT_TTL_SECONDS` | `3900` | 親ワークフローのトレースコンテキストを保持する時間（秒）。保持中は、後から届くネストした子トレースが親に接続できます。<br/><br/>リトライ時間（回数 × 間隔）以上に設定してください。 |
...
| `WORKFLOW_MAX_EXECUTION_TIME` | `3600` | ワークフロー実行ごとの最大実行時間（秒）。超過するとワークフローが終了されます。<br/><br/>`APP_MAX_EXECUTION_TIME` と対になる外側の制限です（両方ともデフォルトは 1 時間）。両方を一緒に増やしてください。Agent を利用する場合は、Agent の実行期限（`DIFY_AGENT_RUN_TIMEOUT_SECONDS`）以上に保ちます。 |
...
| `GRAPH_ENGINE_SCALE_UP_THRESHOLD` | `0` | 空きワーカーを待つタスクをいくつまで許容するか。超えると `GRAPH_ENGINE_MAX_WORKERS` を上限にワーカーが追加されます。<br/><br/>デフォルトの `0` では、待ちタスクが発生した時点で即座に追加されます。値を上げると、短い滞留を許容してからスケールアップします。 |
...
| `MARKDOWN_FORM_FIELD_NAME_EXTRA_CHARS` | （空） | チャット応答内の Markdown からアプリがレンダリングするフォームで、フィールドの `name` 属性に追加で許可する文字。<br/><br/>デフォルトでは、フィールド名は文字で始まり、文字・数字・アンダースコア・ハイフンのみ使用できます。それ以外の文字を名前に含むフィールドはレンダリングされません。<br/><br/>追加する文字は 1 つの文字列として設定します。半角と全角のどちらも使用できます。例：`()！？` |
| `MARKDOWN_FORM_FIELD_NAME_MAX_LENGTH` | `128` | チャット応答内の Markdown からアプリがレンダリングするフォームで、フィールドの `name` 属性に許可する最大長。これより長い名前のフィールドはレンダリングされません。名前に使える文字は `MARKDOWN_FORM_FIELD_NAME_EXTRA_CHARS` で決まり、この変数は長さを制限します。<br/><br/>生成されたフィールド名が上限を超え、レンダリングされたフォームにフィールドが欠ける場合は、この値を上げてください。正の整数を指定します。 |
...
新しい Agent は、Docker Compose デプロイで追加サービス（Agent バックエンド、サンドボックス、サンドボックス専用の外向きプロキシ）として動作します。いずれも `docker compose up` で自動的に起動します。この機能はデフォルトで有効で、設定の大部分は既存の Compose の値から導出されるため、通常は以下の変数だけを確認すれば十分です。
...
デフォルト値：（`.env.example` に開発用トークンを設定済み。本番環境では必ず置き換えてください）

Dify API が Agent バックエンドの実行 API を呼び出す際に提示する Bearer トークンです。Compose は同じ値を両方のサービスに渡すため、一度設定するだけで一致が保たれます。

デフォルトは公開された開発用トークンのため、デプロイはそのまま起動します。ただし、Agent バックエンドに到達でき、トークンを知っていれば誰でも実行を開始できます。本番環境で使用する前に、独自のランダムな値に置き換えてください。
...
| `DIFY_AGENT_RUN_RETENTION_SECONDS` | `7200` | 各 Agent 実行のレコードとイベントストリームを Redis に保持する秒数。最後の書き込みから数える（デフォルトは 2 時間）。値を下げると Redis メモリを早く解放できるが、切断後にイベントを再生できる期間も短くなる。 |
| `DIFY_AGENT_RUN_EVENT_STREAM_MAX_LENGTH` | `5000` | 各 Agent 実行の Redis ストリームに保持するイベント数の上限（概算）。値を下げると、1 回の実行が使う Redis メモリを減らせる。<br/><br/>上限に達すると、新しいイベントの書き込み時に古いイベントから削除する。削除したイベントは切断後に再生できない。 |
| `DIFY_AGENT_STREAM_TEXT_DELTA_COALESCING_ENABLED` | `true` | モデルが連続して出力するテキスト差分をまとめて、Redis に書き込み、配信する。有効のままにすると、Redis への書き込み回数とイベント数を減らせる。<br/><br/>差分を個別に配信するには `false` に設定する。ツール呼び出しなど、テキスト以外のイベントの内容と順序は変わらない。 |
| `DIFY_AGENT_STREAM_TEXT_DELTA_FLUSH_INTERVAL_MS` | `100` | まとめたテキスト差分を配信する間隔の目安（ミリ秒）。値を下げるとテキストの更新頻度が上がる。上げると、1 回の配信量が増え、配信回数が減る。<br/><br/>厳密な時間制限ではない。`DIFY_AGENT_STREAM_TEXT_DELTA_MAX_CHARS` に達すると早めに配信する。最小値は 1。結合を無効にするには `DIFY_AGENT_STREAM_TEXT_DELTA_COALESCING_ENABLED` を `false` にする。 |
| `DIFY_AGENT_STREAM_TEXT_DELTA_MAX_CHARS` | `4096` | まとめたテキスト差分を、フラッシュ間隔を待たずに配信する文字数のしきい値。値を下げると、小さなまとまりで早めに配信できる。<br/><br/>差分は分割せずに追加するため、文字数がこの値を超える場合がある。最小値は 1。 |
| `DIFY_AGENT_RUN_TIMEOUT_SECONDS` | `3600` | Agent 実行 1 回のモデルとツール処理に適用される制限時間。超過した実行は `agent_run_limit_exceeded` で失敗する。旧 `AGENT_BACKEND_RUN_TIMEOUT_SECONDS` を置き換える。<br/><br/>API 側の `APP_MAX_EXECUTION_TIME` と `WORKFLOW_MAX_EXECUTION_TIME`（デフォルト `3600`）をこの値より低く設定すると、実行が先に停止される。<br/><br/>`e2b` バックエンドではサンドボックス側の `DIFY_AGENT_E2B_ACTIVE_TIMEOUT_SECONDS`（上限 3600）も適用されるため、1 時間を超える値が効くのは `local` バックエンドのみ。 |
...
| `DIFY_AGENT_OUTBOUND_HTTP_CONNECT_TIMEOUT` | `10` | Agent サービスの外向き HTTP 呼び出し（ツールやプラグインの通信など）の接続タイムアウト（秒）。 |
| `DIFY_AGENT_OUTBOUND_HTTP_READ_TIMEOUT` | `600` | 読み取りタイムアウト（秒）。遅いツール応答や大きなダウンロードが完了できるように、デフォルトは長めに設定されている。 |
| `DIFY_AGENT_OUTBOUND_HTTP_WRITE_TIMEOUT` | `30` | 外向きリクエストの書き込みタイムアウト（秒）。 |
| `DIFY_AGENT_OUTBOUND_HTTP_POOL_TIMEOUT` | `10` | 接続プールの空き接続を待つ時間（秒）。 |
| `DIFY_AGENT_OUTBOUND_HTTP_MAX_CONNECTIONS` | `100` | Agent サービスの外向き HTTP の接続プールサイズ。 |
| `DIFY_AGENT_OUTBOUND_HTTP_MAX_KEEPALIVE_CONNECTIONS` | `20` | 再利用のために保持するアイドル接続数。 |
| `DIFY_AGENT_OUTBOUND_HTTP_KEEPALIVE_EXPIRY` | `30` | アイドル接続を閉じるまで保持する秒数。 |
...
| `DIFY_AGENT_LOCAL_SANDBOX_ENDPOINT` | `http://local_sandbox:5004` | 同梱のサンドボックスサービスのアドレス。旧 `DIFY_AGENT_SHELLCTL_ENTRYPOINT` を置き換える。旧名も互換エイリアスとして引き続き使える。 |
| `DIFY_AGENT_LOCAL_SANDBOX_AUTH_TOKEN` | （空） | Agent バックエンドからサンドボックスへの呼び出しを保護する任意の Bearer トークン。Compose は同じ値を両サービスに渡す。<br/><br/>旧 `DIFY_AGENT_SHELLCTL_AUTH_TOKEN` を置き換える。旧名も互換エイリアスとして引き続き使える。 |
| `EXPOSE_LOCAL_SANDBOX_PORT` | `5004` | [ソースコードからのデプロイ](/ja/self-host/deploy/advanced-deployments/local-source-code) で使うミドルウェアスタックが Agent サンドボックスを公開するホスト側のポート。ソースから起動した Agent バックエンドは `http://localhost:5004` でサンドボックスに到達できる。設定はそのスタックの `middleware.env` で行う。変更した場合は、Agent バックエンドの `.env` にある `DIFY_AGENT_LOCAL_SANDBOX_ENDPOINT` を新しいポートに合わせる。<br/><br/>ミドルウェアスタック専用。フル構成の Compose スタックでは、サンドボックスは内部ネットワークに留まる。 |
| `DIFY_AGENT_STUB_API_BASE_URL` | `http://agent_backend:5050/agent-stub` | サンドボックス内の Agent が Agent バックエンドを呼び出すためのアドレス。 |
| `DIFY_AGENT_SANDBOX_FILES_BASE_URL` | `http://api:5001` | サンドボックス内の Agent がファイル転送に使う Dify API アドレス。Docker Compose ではデフォルトのまま使う。リモートサンドボックスバックエンドでは、公開されている Dify アドレスを指定する。 |
| `AGENT_BACKEND_BINDING_FILE_DOWNLOAD_TIMEOUT_SECONDS` | `240` | API が Agent のサンドボックスからファイルを取得する際の待ち時間。超過すると転送は失敗する。Agent 側の `DIFY_AGENT_BINDING_FILE_DOWNLOAD_COMMAND_TIMEOUT_SECONDS` より高く保つこと。 |
| `AGENT_BACKEND_HOME_SNAPSHOT_TIMEOUT_SECONDS` | `45` | home snapshot の転送を伴う Agent バックエンド呼び出し（スナップショットの取得・削除、スナップショットを復元する実行の開始）を API が待つ時間。<br/><br/>home 環境が大きい Agent でビルドの適用や実行開始が失敗する場合は、値を大きくしてください。 |
| `DIFY_AGENT_BINDING_FILE_DOWNLOAD_COMMAND_TIMEOUT_SECONDS` | `210` | そのファイルを出力するサンドボックスコマンドの実行時間の上限。デフォルトは API 側のタイムアウトより低く、Agent 自身のエラーが先に表面化するようにしている。 |
| `DIFY_AGENT_STUB_UPLOAD_FILE_SIZE_LIMIT` | `50` | サンドボックス内の Agent が Dify にアップロードする 1 ファイルの上限サイズ（MiB）。返信のために生成したファイルなどが対象。 |
...
| `DIFY_AGENT_SHELL_REDACT_PATTERNS` | （空） | シェル出力を Agent が読む前にマスクする正規表現の JSON 配列。API キーなどの機密を保護する。Agent の内部トークンは常にマスクされる。 |

### DIFY_AGENT_RUNTIME_BACKEND

デフォルト値：`local`

Agent のサンドボックスをどこで実行するかを選択します。デフォルトの `local` は同梱のサンドボックスコンテナを使います。`e2b` に設定すると、各 Agent のサンドボックスを [E2B](https://e2b.dev) のクラウド VM で実行でき、同梱コンテナより強い分離が得られます。

`e2b` バックエンドには専用の変数グループがあり、`docker/docker-compose.e2b.yaml` オーバーレイが Compose 構成を調整します：
...
| `MARKETPLACE_ENABLED` | `true` | Dify Marketplace のプラグイン機能を有効または無効にします。無効にすると、Marketplace プラグインの閲覧・インストール画面への導線が非表示になります。API は Marketplace からのプラグインのインストールとアップグレードを拒否します。定期アップグレードチェック（`ENABLE_CHECK_UPGRADABLE_PLUGIN_TASK`）も停止します。<br/><br/>テンプレートのインポート処理自体には影響しません。ただし、インポートしたテンプレートが未インストールの Marketplace プラグインに依存する場合があります。Marketplace が無効な間は、その依存プラグインをインストールできません。インストール済みのプラグインはそのまま動作します。インストールポリシーで許可されている場合は、GitHub やローカルパッケージからも引き続きインストールできます。 |
| `MARKETPLACE_API_URL` | `https://marketplace.dify.ai` | プラグインとテンプレートの閲覧・ダウンロード、プラグインのアップグレード確認に使う Marketplace API エンドポイント。<br/><br/>Dify API サービス、バックグラウンドワーカー、Web サービス、ユーザーのブラウザでこの URL を使用します。各実行環境からアクセスできる必要があります。 |
| `MARKETPLACE_URL` | （空。Compose 構成では `https://marketplace.dify.ai` にフォールバック） | Marketplace リンクのベース URL。プラグイン詳細ページや「統合」ページの Marketplace 入口に使用します。Dify は Creator Center のガイドリンク生成にもこの値を使用します。<br/><br/>`MARKETPLACE_API_URL` とは用途が異なり、後者は Marketplace のデータとパッケージを提供する API エンドポイントです。 |
...
Creator Center（`creators.dify.ai`）は、Dify アプリをテンプレートとして提出する窓口です。提出されたテンプレートはレビューを経て Dify マーケットプレイスに掲載されます。Dify コンソールから Creator Center を開くと、新しいブラウザタブに表示されます。

コンソールの **マーケットプレイスに公開** アクションは、アプリの DSL（YAML 形式のアプリ定義）を Creator Center にアップロードします。ユーザーは Creator Center で提出を完了します。手順は [アプリをマーケットプレイスに公開](/ja/self-host/use-dify/publish/publish-to-marketplace) を参照してください。

| 変数 | デフォルト値 | 説明 |
|---|---|---|
| `CREATORS_PLATFORM_FEATURES_ENABLED` | `true` | アプリの「公開する」パネルに **マーケットプレイスに公開** アクションを表示するかどうかを制御します。`false` に設定するとアクションが非表示になり、対応するコンソール API は 403 を返します。テンプレートを外部に提出する一発経路をユーザーに提供したくないデプロイで有用です。 |
| `CREATORS_PLATFORM_API_URL` | `https://creators.dify.ai` | Creator Center サービスのベース URL。**マーケットプレイスに公開** は、API サービスからこのアドレスへアプリの DSL をアップロードします。ユーザーのブラウザも同じアドレスにアクセスして提出を完了するため、どちらからも到達できる必要があります。 |
| `CREATORS_PLATFORM_OAUTH_CLIENT_ID` | （空） | Creator Center へのパスワードレス SSO に使用する OAuth クライアント ID。セルフホスト環境では空のままにしてください。空でも公開できますが、ログインしていないユーザーは Creator Center に手動でログインする必要があります。デプロイの OAuth サーバーに未登録の ID を設定すると、**マーケットプレイスに公開** が失敗します。 |
...
| `ENABLE_CLEAN_MESSAGES` | `false` | 毎日午前 4:00 に `SANDBOX_EXPIRED_RECORDS_RETENTION_DAYS` の保持期間を超えた会話メッセージを削除します。 |
...
| `ENABLE_CONVERSATION_CLEANUP_TASK` | `true` | 会話削除のセーフティネット。削除された会話は即座に見えなくなり、メッセージと会話中にツールが生成したファイルはバックグラウンドジョブが削除します。エンドユーザーがアップロードしたファイルは保持されます。<br/><br/>このタスクは、クリーンアップが実行されなかった会話を定期的に再処理し、データの残留を防ぎます。 |
| `CONVERSATION_CLEANUP_TASK_INTERVAL` | `5` | タスクの実行頻度（分）。 |
| `CONVERSATION_CLEANUP_BATCH_SIZE` | `100` | 1 回のスイープで再処理する会話の最大数。 |

#### レコード保持とクリーンアップ

古いメッセージとワークフロー実行レコードの定期クリーンアップを制御します。保持期間を超えたレコードはバッチ単位で削除されます。
