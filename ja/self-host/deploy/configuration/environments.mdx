...
| `MARKETPLACE_ENABLED` | `true` | Dify Marketplace のプラグイン機能を有効または無効にします。無効にすると、Marketplace プラグインの閲覧・インストール画面が非表示になります。API は Marketplace からのプラグインのインストールとアップグレードを拒否します。定期アップグレードチェック（`ENABLE_CHECK_UPGRADABLE_PLUGIN_TASK`）も停止します。<br/><br/>テンプレートのインポートには影響しません。インストール済みのプラグインはそのまま動作し、GitHub やローカルパッケージからのインストールも引き続き利用できます。 |
| `MARKETPLACE_API_URL` | `https://marketplace.dify.ai` | プラグインとテンプレートの閲覧・ダウンロード、およびプラグインのアップグレード確認に使う Marketplace API エンドポイント。<br/><br/>Web サービスとユーザーのブラウザの両方でこの URL を使用するため、どちらからもアクセスできる必要があります。 |
| `MARKETPLACE_URL` | （空。Compose 構成では `https://marketplace.dify.ai` にフォールバック） | Marketplace リンクのベース URL。プラグイン詳細ページや「統合」ページの Marketplace 入口に使用します。Dify は Creator Center のガイドリンク生成にもこの値を使用します。<br/><br/>`MARKETPLACE_API_URL` とは用途が異なります。そちらは Marketplace データの取得に使います。 |
...
Creator Center（`creators.dify.ai`）は、Dify アプリをテンプレートとして提出する窓口です。提出されたテンプレートはレビューを経て Dify マーケットプレイスに掲載されます。Creator Center の関連ページは、ブラウザの新しいタブで開きます。
...
| `CREATORS_PLATFORM_OAUTH_CLIENT_ID` | （空） | Creator Center へのパスワードなし SSO に使用する OAuth クライアント ID。セルフホスト環境では空のままにしてください。空でも公開に影響しません。デプロイの OAuth サーバーに未登録の ID を設定すると、**マーケットプレイスに公開** が失敗します。 |
