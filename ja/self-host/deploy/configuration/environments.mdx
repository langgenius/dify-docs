...
| `MARKETPLACE_ENABLED` | `true` | Dify Marketplace をプラグインとテンプレートの入手元として有効化します。無効にすると、コンソールの Marketplace 閲覧・インストール画面が非表示になります。API は Marketplace からのインストールとアップグレードを拒否します。定期アップグレードチェック（`ENABLE_CHECK_UPGRADABLE_PLUGIN_TASK`）も停止します。<br/><br/>インストール済みのプラグインはそのまま動作し、GitHub やローカルパッケージからのインストールは引き続き利用できます。 |
| `MARKETPLACE_API_URL` | `https://marketplace.dify.ai` | プラグインとテンプレートの閲覧、ダウンロード、アップグレード確認に使う Marketplace API エンドポイント。<br/><br/>コンソールは Marketplace の一覧をブラウザ側で取得します。そのため、API サービスとユーザーのブラウザの両方から到達できる必要があります。 |
| `MARKETPLACE_URL` | （空。Compose 構成では `https://marketplace.dify.ai` にフォールバック） | Marketplace サイトへ遷移するリンクのベース URL。プラグイン詳細ページと「統合」ページの Marketplace 入口が対象です。<br/><br/>`MARKETPLACE_API_URL` とは別物で、そちらはコンソールとプラグインデーモンが Marketplace データの取得に使います。 |
...
Creator Center（`creators.dify.ai`）は、Dify アプリをテンプレートとして提出する窓口です。提出されたテンプレートはレビューを経て Dify マーケットプレイスに掲載されます。

コンソールの **マーケットプレイスに公開** アクションは、アプリの DSL（YAML 形式のアプリ定義）を Creator Center にアップロードします。その後、ユーザーのブラウザを Creator Center へ移動させて提出を完了させます。ユーザー向けフローは [アプリをマーケットプレイスに公開](/ja/self-host/use-dify/publish/publish-to-marketplace) を参照してください。
...
| `CREATORS_PLATFORM_API_URL` | `https://creators.dify.ai` | Creator Center サービスのベース URL。**マーケットプレイスに公開** は、API サービスからこのアドレスへアプリの DSL をアップロードします。その後、ユーザーのブラウザを同じアドレスへリダイレクトし、提出を完了させます。そのため、API サービスとユーザーのブラウザの両方から到達できる必要があります。 |
| `CREATORS_PLATFORM_OAUTH_CLIENT_ID` | （空） | Creator Center がテンプレートの提出を対応する Dify アカウントに紐付けるための OAuth クライアント ID。空のままにしてください。空でも公開は問題なく動作します。デプロイの OAuth サーバーに未登録の ID を設定すると、**マーケットプレイスに公開** が失敗します。 |
