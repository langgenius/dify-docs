...
コラボレーション通信を担う専用の `api_websocket` コンテナは、[コンテナ起動設定](#コンテナ起動設定) の `API_WEBSOCKET_*` 変数でチューニングできます。
...
| `API_WEBSOCKET_WORKER_AMOUNT` | `1` | Docker Compose のみ。リアルタイムコラボレーション用の WebSocket を担う専用 `api_websocket` コンテナの Gunicorn ワーカー数です。コラボレーションイベントは Redis 経由でワーカー間に同期されるため、増やしても問題ありません。同時編集が多く 1 ワーカーが CPU コアを使い切る場合に増やしてください。 |
...
| `API_WEBSOCKET_WORKER_CONNECTIONS` | `1000` | Docker Compose のみ。`api_websocket` ワーカーごとに同時に受け付ける WebSocket 接続の最大数です。開いているワークフローエディタタブごとに 1 接続を使用するため、デフォルトではワーカーあたり最大 1000 の同時編集セッションを処理できます。それ以上が見込まれる場合は、この値かワーカー数を増やしてください。 |
...
| `NEXT_PUBLIC_ENABLE_FEATURE_PREVIEW` | `true` | 開発中のプレビュー機能をコンソールに表示します。現在は、コマンドパレットの `/create` および `/refine` スラッシュコマンド（AI によるワークフロー／チャットフロー生成）を制御します。`false` に設定すると、これらのプレビュー機能を非表示にできます。 |
