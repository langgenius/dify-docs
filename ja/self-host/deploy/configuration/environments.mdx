...
新しい Agent は、Docker Compose デプロイで 2 つの追加サービス（Agent バックエンドとサンドボックス）として動作し、`docker compose up` で自動的に起動します。この機能はデフォルトで有効で、設定の大部分は既存の Compose の値から導出されるため、通常は以下の変数だけを確認すれば十分です。
