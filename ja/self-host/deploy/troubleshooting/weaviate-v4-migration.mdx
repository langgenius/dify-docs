...
> このドキュメントは AI によって自動翻訳されています。不正確な部分がある場合は、[英語版](/en/self-host/deploy/troubleshooting/weaviate-v4-migration) を参照してください。

> このガイドでは、Weaviate クライアント v3 から weaviate-client v4（v4.17.0 以降）への移行方法を説明します。また、Weaviate サーバーをバージョン 1.19.0 から 1.27.0 以降にアップグレードする方法も説明します。この移行は、weaviate-client v4 アップグレードを含む Dify バージョンに必要です。

<Info>
  すでに 1.27 を使用していて、Dify 1.17.1 にアップグレードする場合、同梱される Weaviate イメージは `1.39.2` です。この差を越えるには、マイナーリリースを順に進める必要があります。手順の一覧は [Weaviate サーバーのアップグレード経路](/ja/self-host/deploy/troubleshooting/weaviate-server-migration-path) を参照してください。
</Info>
...
<Warning>
  **破壊的変更**：新しい weaviate-client v4 は、Weaviate サーバーバージョン 1.27.0 未満との後方互換性がありません。バージョン 1.19.0 以前のセルフホスト Weaviate インスタンスを実行している場合は、Dify をアップグレードする前に Weaviate サーバーをアップグレードする必要があります。
</Warning>
...
- 1.27.0 未満のバージョンでセルフホスト Weaviate インスタンスを実行しているセルフホスト Dify ユーザー
...
| 1.17.0 - 1.17.1 | v4.22.0              | 1.27.0 以降                         |
...
- [スキーマ移行スクリプト](https://github.com/langgenius/dify-docs/blob/main/assets/migrate_weaviate_collections.py)用に Python 3.12
...
2. **[移行スクリプト](https://github.com/langgenius/dify-docs/blob/main/assets/migrate_weaviate_collections.py)を実行**：ローカルまたは Worker コンテナ内で実行できます。\
   **オプション A: ローカルで実行（Python 3.12 と依存関係がインストールされている場合）**：
...
- Docker + Docker Compose がインストール済み
- [移行スクリプト](https://github.com/langgenius/dify-docs/blob/main/assets/migrate_weaviate_collections.py)用に Python 3.12
...
Docker インストールの場合は、正しい Dify バージョンを実行していることを確認します。`docker` ディレクトリで実行してください：
...
ソースインストールの場合は、チェックアウトが固定しているクライアントバージョンをインストールします。`docker` ではなくリポジトリのルートで実行してください：
...
- [Dify セルフホスティングガイド](/ja/self-host/deploy/quick-start/docker-compose)
- [Dify 環境変数](/ja/self-host/deploy/configuration/environments)
