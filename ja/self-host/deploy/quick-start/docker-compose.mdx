...
<Steps>
  ...
  <Step title="Dify を起動">
    ...
    3. コンテナを起動します：
        ...
        - 8 つの依存コンポーネント：`weaviate`、`db_postgres`、`redis`、`nginx`、`ssrf_proxy`、`agent_ssrf_proxy`、`sandbox`、`local_sandbox`
...
<Note>
  ...
  `docker-compose.yaml` をカスタマイズしている場合は、古いファイルを使い続けるのではなく、新しいリリースのファイルにカスタマイズを適用し直してください。リリースによっては、Dify の動作に必要なサービスが追加されることがあります。
</Note>
