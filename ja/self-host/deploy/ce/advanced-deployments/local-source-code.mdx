...
<Note> ⚠️ このドキュメントは AI によって自動翻訳されています。不正確な部分がある場合は、[英語版](/en/self-host/advanced-deployments/local-source-code) を参照してください。</Note>
...
> OpenAI TTS を使用する必要がある場合は、正常に機能するためにシステムに `FFmpeg` がインストールされている必要があります。詳細については、[リンク](https://docs.dify.ai/en/self-host/troubleshooting/integrations#text-to-speech-tts)を参照してください。
