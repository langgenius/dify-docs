...
> このドキュメントは AI によって自動翻訳されています。不正確な部分がある場合は、[英語版](/en/self-host/deploy/advanced-deployments/local-source-code) を参照してください。
...
> OpenAI TTS を使用する必要がある場合は、正常に機能するためにシステムに `FFmpeg` がインストールされている必要があります。詳細については、[リンク](https://docs.dify.ai/en/self-host/deploy/troubleshooting/integrations#text-to-speech-tts)を参照してください。
...
バックエンドを起動する前に、Docker Compose でデータベース、[サンドボックス](https://github.com/langgenius/dify-sandbox)、[プラグインデーモン](https://github.com/langgenius/dify-plugin-daemon)を起動します。
...
- NodeJS のインストール
  ...
  [https://nodejs.org/en/download](https://nodejs.org/en/download) にアクセスします。お使いのオペレーティングシステム用の v24 インストールパッケージ（24.20.0 以降）を選択してください。
