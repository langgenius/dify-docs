...
# postgresql を使用しない場合は、profile を mysql に変更してください
# weaviate を使用しない場合は、profile を他のベクターデータベースに変更してください
...
さらに、celery のスケジュールタスクをデバッグしたり、Schedule Trigger ノードを実行したりする場合は、別のターミナルで以下のコマンドを実行して beat サービスを起動できます：
