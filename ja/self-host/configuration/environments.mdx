...
ワークフローキャンバスのリアルタイム共同編集（同時編集、キャンバス上のコメント、@メンション）を制御するマスタースイッチです。`true` に設定すると、フロントエンドが共同編集 UI を有効化します。

この変数を設定するだけでは不十分です。共同編集は WebSocket 経由で動作するため、以下も必要です：

- `SERVER_WORKER_CLASS` を `geventwebsocket.gunicorn.workers.GeventWebSocketWorker` に設定し、Gunicorn が WebSocket のアップグレードを受け付けられるようにします（[コンテナ起動設定](#コンテナ起動設定)を参照）。
- `NEXT_PUBLIC_SOCKET_URL` をブラウザからアクセス可能な WebSocket URL に設定します（[CORS設定](#cors設定)を参照）。

**運用上の注意：**

- **リバースプロキシ**：Dify 同梱の nginx ではなく、独自の nginx、traefik、クラウドロードバランサーで TLS 終端やトラフィック転送を行う場合、`/socket.io/` パスで `Upgrade` および `Connection` ヘッダーを転送してください。同梱の `docker/nginx/conf.d/default.conf.template` にはこの設定が含まれています。
- **API の複数レプリカ構成**：socket.io サーバーはセッションごとの状態をメモリ上に保持します。API を複数レプリカで運用する場合、ロードバランサー層でスティッキーセッションを有効化し、クライアントがレプリカ間を行き来して状態を失わないようにしてください。
...
| `SERVER_WORKER_CLASS` | `gevent` | Gunicorn ワーカータイプ。`gevent` は軽量な非同期並行処理を提供します。`ENABLE_COLLABORATION_MODE` が `true` の場合、WebSocket のアップグレードを受け付けるために `geventwebsocket.gunicorn.workers.GeventWebSocketWorker` に設定します。`gevent` のままだと、共同編集クライアントは WebSocket ハンドシェイクで失敗しますが起動エラーは発生しません。他の値は psycopg2 と gRPC のパッチを壊すためサポートされていません。 |
...
| `NEXT_PUBLIC_SOCKET_URL` | `ws://localhost` | リアルタイム共同編集のためのブラウザ側 WebSocket エンドポイント。`ENABLE_COLLABORATION_MODE` が `true` の場合、ワークフローキャンバスはこの URL に接続してリアルタイム編集とコメントを行います。スキーム + ホスト + ポートのみを指定します（パス `/socket.io/` はクライアント側で固定）。`http(s)://` を `ws(s)://` に置き換えてください。例：`wss://dify.example.com`。デフォルトの `ws://localhost` はブラウザで 80 番ポートに解決され、同梱の nginx と一致します。Dify のランタイム環境変数注入により適用されるため、変更はコンテナ再起動で反映され、イメージの再ビルドは不要です。 |
...
| `S3_ADDRESS_STYLE` | `auto` | S3アドレッシングスタイル：`auto`、`path`、または `virtual`。バケット名をURLパスに含める（`path`）か、サブドメインとして使用する（`virtual`）かを制御します。`S3_USE_AWS_MANAGED_IAM` が `false` の場合のみ有効です。 |
| `S3_USE_AWS_MANAGED_IAM` | `false` | 明示的なaccess key/secret keyの代わりにAWS IAMロール（EC2インスタンスプロファイル、ECSタスクロール）を使用します。有効時、インスタンスメタデータから資格情報を自動検出します。 |
