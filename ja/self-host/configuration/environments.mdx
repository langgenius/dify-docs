...
コンソールAPIのバックエンドURL。認証コールバックの構築に使用されます。空白のままにすると、アプリケーションと同じドメインがデフォルトになります。例：`https://api.console.dify.ai`

### CONSOLE_WEB_URL

コンソールWebインターフェースのフロントエンドURL。フロントエンドアドレスの構築とCORS設定に使用されます。空白のままにすると、アプリケーションと同じドメインがデフォルトになります。例：`https://console.dify.ai`

### SERVICE_API_URL

サービスAPI URL。フロントエンドでサービスAPIベースURLを表示するために使用されます。空白のままにすると、アプリケーションと同じドメインがデフォルトになります。例：`https://api.dify.ai`

### TRIGGER_URL

webhookトリガーとプラグイントリガーの両方でwebhookコールバックURLに使用されるベースURLプレフィックス。

外部システムからアクセス可能なパブリックドメインまたはIPアドレスを指していることを確認してください。

### APP_API_URL

WebApp APIバックエンドURL。フロントエンドAPIのバックエンドURLを指定するために使用されます。空白のままにすると、アプリケーションと同じドメインがデフォルトになります。例：`https://app.dify.ai`

### APP_WEB_URL

WebApp URL。フロントエンドへのファイルプレビューまたはダウンロードURL、またはマルチモデル入力として表示するために使用されます。空白のままにすると、アプリケーションと同じドメインがデフォルトになります。例：`https://udify.app/`

### FILES_URL

ファイルプレビューまたはダウンロードURLのプレフィックス。フロントエンドでこれらのURLを表示し、マルチモーダルモデルの入力として提供するために使用されます。偽造を防ぐため、画像プレビューURLは署名され、5分後に期限切れになります。
...
起動モード：Dockerを使用して起動する場合にのみ使用可能です。ソースコードから実行する場合は適用されません。

- api：APIサーバーを起動します。
- worker：非同期キューワーカーを起動します。

### DEBUG

デバッグモード：デフォルトで無効。ローカル開発中はモンキーパッチによる問題を防ぐためにこの設定を有効にすることをお勧めします。
...
セッションクッキーの安全な署名とデータベース内の機密情報の暗号化に使用されるシークレットキー。

この変数は最初の起動前に設定する必要があります。

`openssl rand -base64 42` を実行して強力なキーを生成してください。
...
- TESTING：テスト環境。フロントエンドページにこの環境がテスト環境であることを示す色付きラベルが表示されます。
...
trueに設定すると、コンテナ起動時にデータベースマイグレーションが自動的に実行されます。Dockerを使用して起動する場合にのみ使用可能で、ソースコードから実行する場合は適用されません。
...
デフォルト値：60000（ミリ秒）。テキスト生成とワークフロープロセスのタイムアウトを指定します。この設定は、個々のプロセスが割り当てられた時間を超えることによるシステム全体のサービス中断を防ぎます。

### OPENAI_API_BASE

OpenAIベースアドレスを変更するために使用されます。デフォルトは[https://api.openai.com/v1](https://api.openai.com/v1)です。

中国でOpenAIにアクセスできない場合は国内ミラーアドレスに置き換えるか、ローカルモデルがOpenAI互換APIを提供している場合は置き換えることができます。
...
- GUNICORN_TIMEOUT：リクエスト処理タイムアウト。デフォルトは200。より長いSSE（Server-Sent Events）接続時間をサポートするには360を推奨。
...
- DB_DATABASE：データベース名
- SQLALCHEMY_POOL_SIZE：データベース接続プールのサイズ。デフォルトは30接続で、適宜増やすことができます。
- SQLALCHEMY_POOL_RECYCLE：データベース接続プールのリサイクル時間、デフォルトは3600秒。
- SQLALCHEMY_ECHO：SQLを出力するかどうか、デフォルトはfalse。

### Redis設定

このRedis設定はキャッシングと会話中のpub/subに使用されます。
...
- CELERY_USE_SENTINEL：trueに設定すると、Sentinelモードが有効になる、デフォルトはfalse
...
- COOKIE_DOMAIN\
  フロントエンドとバックエンドが異なるサブドメインで実行されている場合、`COOKIE_DOMAIN`をサイトのトップレベルドメイン（例：`example.com`）に設定します。\
  認証クッキーを共有するには、フロントエンドとバックエンドが同じトップレベルドメイン下にある必要があります。

- NEXT_PUBLIC_COOKIE_DOMAIN\
  フロントエンドとバックエンドが異なるサブドメインで実行されている場合、`NEXT_PUBLIC_COOKIE_DOMAIN`を`1`に設定します。\
  認証クッキーを共有するには、フロントエンドとバックエンドが同じトップレベルドメイン下にある必要があります。
...
- STORAGE_TYPE
  ...
  - s3：S3オブジェクトストレージ。このオプションを選択した場合、以下のS3_プレフィックス設定を設定する必要があります。

  - azure-blob：Azure Blobオブジェクトストレージ。このオプションを選択した場合、以下のAZURE_BLOB_プレフィックス設定を設定する必要があります。

  - aliyun-oss：Alibaba Cloud OSSオブジェクトストレージ。このオプションを選択した場合、以下のALIYUN_OSS_プレフィックス設定を設定する必要があります。

  - huawei-obs：Huawei OBSオブジェクトストレージ。このオプションを選択した場合、以下のHUAWEI_OBS_プレフィックス設定を設定する必要があります。

  - volcengine-tos：Volcengine TOSオブジェクトストレージ。このオプションを選択した場合、以下のVOLCENGINE_TOS_プレフィックス設定を設定する必要があります。

  - tencent-cos：Tencent Cloud COSオブジェクトストレージ。このオプションを選択した場合、以下のTENCENT_COS_プレフィックス設定を設定する必要があります。

- STORAGE_LOCAL_PATH

  デフォルトはstorage、つまり現在のディレクトリのstorageディレクトリに保存されます。

  dockerまたはdocker-composeでデプロイする場合、両方のコンテナで`/app/api/storage`ディレクトリを同じローカルディレクトリにマウントしてください。そうしないと、ファイルが見つからないエラーが発生する可能性があります。
...
- ALIYUN_OSS_PATH：your-path # '/'で始めないでください。OSSはオブジェクト名の先頭スラッシュをサポートしていません。参考：https://www.alibabacloud.com/help/en/oss/support/0016-00000005
...
- TENCENT_COS_SCHEME：COSにアクセスするためのhttp/httpsプロトコルを指定
...
- WEAVIATE_API_KEY
  ...
  Weaviateへの接続に使用されるapi-key認証情報。

- WEAVIATE_BATCH_SIZE

  Weaviateでバッチ作成されるインデックスオブジェクト数、デフォルトは100。
...
- QDRANT_API_KEY
  ...
  Qdrantへの接続に使用されるapi-key認証情報。

- PINECONE_API_KEY

  Pineconeへの接続に使用されるapi-key認証情報。
...
- OCEANBASE_CLUSTER_NAME
  ...
  OceanBaseベクトルデータベースのクラスター名、Dockerデプロイでのみ使用可能。

- OCEANBASE_MEMORY_LIMIT

  OceanBaseベクトルデータベースのメモリ制限、Dockerデプロイでのみ使用可能。

- SEEKDB_MEMORY_LIMIT

  seekdbのメモリ制限、Dockerデプロイでのみ使用可能。
...
- TENCENT_VECTOR_DB_DATABASE
  ...
  データを保存するためのデータベースを設定します。[データベース作成](https://cloud.tencent.com/document/product/1709/95822)。
...
- UPLOAD_FILE_SIZE_LIMIT：アップロードファイルサイズ制限、デフォルト15M。
- UPLOAD_FILE_BATCH_LIMIT：一度にアップロードできるファイルの最大数、デフォルト5。
- ETL_TYPE
...
- UNSTRUCTURED_API_URL
  ...
  Unstructured APIパス。ETL_TYPEがUnstructuredの場合に設定が必要です。
...
- MULTIMODAL_SEND_IMAGE_FORMAT
  ...
  マルチモーダルモデル入力時に送信される画像のフォーマット。デフォルトは`base64`、オプションは`url`。`url`モードの呼び出し遅延は`base64`モードよりも低くなります。一般的には、より互換性の高い`base64`モードを使用することをお勧めします。`url`として設定する場合、マルチモーダルモデルが画像にアクセスできるように`FILES_URL`を外部からアクセス可能なアドレスに設定する必要があります。

- UPLOAD_IMAGE_FILE_SIZE_LIMIT：画像ファイルアップロードサイズ制限、デフォルト10M。
...
- NOTION_INTEGRATION_TYPE："public"または"internal"として設定します。NotionのOAuthリダイレクトURLはHTTPSのみをサポートしているため、ローカルでデプロイする場合はNotionの内部統合を使用してください。
- NOTION_CLIENT_SECRET：Notion OAuthクライアントシークレット（パブリック統合タイプ用）
- NOTION_CLIENT_ID：OAuthクライアントID（パブリック統合タイプ用）
...
- MAIL_TYPE
  ...
  - resend
    ...
    - RESEND_API_KEY：Resendメールプロバイダーのapi-key、api-keyから取得できます。
  ...
  - sendgrid
    ...
    - MAIL_DEFAULT_SEND_FROM：送信者のメールアドレス、例：your_email@sendgrid.com。このフィールドはSendGrid認証に必須です。

SendGridメールプロバイダーの詳細については、[SendGridドキュメント](https://sendgrid.com/docs/for-developers/sending-email/)を参照してください。

### モデルプロバイダーとツール位置設定

アプリで使用できるモデルプロバイダーとツールを指定するために使用されます。これらの設定により、アプリのインターフェースで利用可能なツールとモデルプロバイダー、およびそれらの順序と包含/除外をカスタマイズできます。
...
- POSITION_TOOL_PINS
  ...
  特定のツールをリストの上部にピン留めし、インターフェースで最初に表示されるようにします。（項目間に**スペースなし**のカンマ区切り値を使用してください。）

  例：`POSITION_TOOL_PINS=bing,google`

- POSITION_TOOL_INCLUDES

  アプリに含めるツールを指定します。ここにリストされたツールのみが使用可能になります。設定されていない場合、POSITION_TOOL_EXCLUDESで指定されていない限り、すべてのツールが含まれます。（項目間に**スペースなし**のカンマ区切り値を使用してください。）

  例：`POSITION_TOOL_INCLUDES=bing,google`

- POSITION_TOOL_EXCLUDES

  特定のツールをアプリで表示または使用することから除外します。ここにリストされたツールは、ピン留めされたツールを除いて、利用可能なオプションから省略されます。（項目間に**スペースなし**のカンマ区切り値を使用してください。）

  例：`POSITION_TOOL_EXCLUDES=yahoo,wolframalpha`

- POSITION_PROVIDER_PINS

  特定のモデルプロバイダーをリストの上部にピン留めし、インターフェースで最初に表示されるようにします。（項目間に**スペースなし**のカンマ区切り値を使用してください。）

  例：`POSITION_PROVIDER_PINS=openai,openllm`

- POSITION_PROVIDER_INCLUDES

  アプリに含めるモデルプロバイダーを指定します。ここにリストされたプロバイダーのみが使用可能になります。設定されていない場合、POSITION_PROVIDER_EXCLUDESで指定されていない限り、すべてのプロバイダーが含まれます。（項目間に**スペースなし**のカンマ区切り値を使用してください。）

  例：`POSITION_PROVIDER_INCLUDES=cohere,upstage`

- POSITION_PROVIDER_EXCLUDES

  特定のモデルプロバイダーをアプリで表示または使用することから除外します。ここにリストされたプロバイダーは、ピン留めされたプロバイダーを除いて、利用可能なオプションから省略されます。（項目間に**スペースなし**のカンマ区切り値を使用してください。）
...
- ENABLE_CLEAN_EMBEDDING_CACHE_TASK
  ...
  埋め込みキャッシュクリーンアップタスクを有効にするかどうか。デフォルトはfalse。有効にすると、毎日午前2時に期限切れの埋め込みキャッシュをクリーンアップしてストレージ使用量を削減します。

- ENABLE_CLEAN_UNUSED_DATASETS_TASK

  未使用データセットクリーンアップタスクを有効にするかどうか。デフォルトはfalse。有効にすると、毎日午前3時に長期間未使用のデータセットをクリーンアップしてストレージスペースを解放します。
...
- ENABLE_MAIL_CLEAN_DOCUMENT_NOTIFY_TASK
  ...
  メールドキュメントクリーンアップ通知タスクを有効にするかどうか。デフォルトはfalse。有効にすると、毎週月曜日午前10時にドキュメントクリーンアップ通知メールを送信します。

- ENABLE_DATASETS_QUEUE_MONITOR

  データセットキュー監視タスクを有効にするかどうか。デフォルトはfalse。有効にすると、データセット処理キューのステータスを監視し、キューバックログがしきい値を超えるとアラートを送信します。
...
- WORKFLOW_LOG_CLEANUP_ENABLED
  ...
  ワークフローログ自動クリーンアップタスクを有効にするかどうか。デフォルトはfalse。有効にすると、毎日午前2時に保持期間を超えたワークフロー実行ログをクリーンアップします。
...
> ⚠️ 0.3.8で変更、0.4.9で非推奨予定。`CONSOLE_API_URL`と`CONSOLE_WEB_URL`に置き換えられます。

コンソールURL。認証コールバック、コンソールフロントエンドアドレス、およびCORS設定の連結に使用されます。空の場合、同じドメインです。例：`https://console.dify.ai`。

### API_URL

> ⚠️ 0.3.8で変更、0.4.9で非推奨予定。`SERVICE_API_URL`に置き換えられます。

API URL。フロントエンドにサービスAPIベースURLを表示するために使用されます。空の場合、同じドメインです。例：`https://api.dify.ai`

### APP_URL

> ⚠️ 0.3.8で変更、0.4.9で非推奨予定。`APP_API_URL`と`APP_WEB_URL`に置き換えられます。

WebApp URL。フロントエンドにWebAPP APIベースURLを表示するために使用されます。空の場合、同じドメインです。例：`https://udify.app/`

### セッション設定

> ⚠️ この設定はv0.3.24以降無効です。

APIサービスによるインターフェースID検証にのみ使用されます。

- SESSION_TYPE：セッションコンポーネントタイプ

  - redis（デフォルト）：これを選択した場合、以下のSESSION_REDIS\_で始まる環境変数を設定する必要があります。
  - sqlalchemy：これを選択した場合、現在のデータベース接続が使用され、sessionsテーブルがセッションレコードの読み書きに使用されます。
...
ID検証に使用されるセッションクッキーのブラウザポリシーを設定するために使用されます。
...
**大きなチャンク**
- 各チャンク内でより多くのコンテキストを保持し、テキストのより広い理解を必要とするタスクに理想的です。
- 総チャンク数を減らし、処理時間とストレージオーバーヘッドを削減します。

**小さなチャンク**
- より細かい粒度を提供し、抽出や要約などのタスクの精度を向上させます。
- モデルトークン制限を超えるリスクを減らし、より厳しい制約を持つモデルに対してより安全です。

**設定の推奨事項**
- 感情分析やドキュメント要約などのコンテキスト重視のタスクには大きなチャンクを選択します。
- キーワード抽出や段落レベルの処理などの細粒度タスクには小さなチャンクを選択します。