...
Dify はデフォルト設定でそのまま動作します。カスタマイズするには、`docker/.env.example` で利用可能な環境変数を確認し、上書きしたい変数を `docker/.env` に追加してください。

<Info>
Dify をアップグレードした後も、`.env` のオーバーライドは引き続き適用されます。
</Info>
...
- `NEXT_PUBLIC_SOCKET_URL` をブラウザからアクセス可能な WebSocket URL に設定します（[CORS 設定](#cors-設定) を参照）。
...
デフォルト値：Docker Compose 起動スクリプトによって `.env` に生成されます。

セッション Cookie 署名、JWT 認証トークンに使用されます。さらに、ファイル URL 署名（HMAC-SHA256）とサードパーティ OAuth 資格情報の暗号化（AES-256）にも使用されます。Docker Compose 起動スクリプトが初回起動前に強力なキーを生成します。手動デプロイの場合は、次のコマンドで生成してください：
...
| 変数 | デフォルト値 | 説明 |
|---|---|---|
| `MAIL_TYPE` | （空） | メールプロバイダー：`resend`、`smtp`、または `sendgrid`。空にするとメール送信が無効化されます。 |
| `MAIL_DEFAULT_SEND_FROM` | （空） | すべての送信メールのデフォルト「From」アドレス。`MAIL_TYPE` を設定する場合は必須です。 |
...
Creator Center（`creators.dify.ai`）は、Dify アプリをテンプレートとして提出する窓口です。提出されたテンプレートはレビューを経て Dify マーケットプレイスに掲載されます。コンソールの **マーケットプレイスに公開** アクションは、アプリの DSL をエクスポートして Creator Center にアップロードします。ユーザー向けフローは [アプリをマーケットプレイスに公開](/ja/use-dify/publish/publish-to-marketplace) を参照してください。
