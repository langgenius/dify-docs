...
Creator Center（`creators.dify.ai`）は、Dify アプリをテンプレートとして提出する窓口です。提出されたテンプレートはレビューを経て Dify マーケットプレイスに掲載されます。コンソールの **マーケットプレイスに公開** アクションは、アプリの DSL をエクスポートして Creator Center にアップロードします。ユーザー向けフローは [アプリをマーケットプレイスに公開](/use-dify/publish/publish-to-marketplace) を参照してください。
