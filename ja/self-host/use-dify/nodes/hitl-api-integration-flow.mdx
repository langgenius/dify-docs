...
<Steps>
  ...
  <Step title="（ファイル入力時のみ）ローカルファイルをアップロード">
    受信者が `file` または `file-list` 入力にローカルファイルを添付する場合は、標準の Service API ファイルエンドポイント [ファイルをアップロード](/api-reference/ファイル操作/ファイルをアップロード) で、送信時と同じ `user` を使って先にアップロードします。このエンドポイントは `id` を返すので、送信時に `upload_file_id` として参照します。アップロードしたファイルはその `user` にスコープされます。

    リモートファイルにアップロードは不要です。送信時に `remote_url` マッピングでインライン添付します。
  </Step>
  <Step title="応答を送信">
    [人間の入力フォームを送信](/api-reference/人間の入力/人間の入力フォームを送信) エンドポイントを呼び出し、受信者の入力値、選択されたアクション、そして `user` を送ります。
