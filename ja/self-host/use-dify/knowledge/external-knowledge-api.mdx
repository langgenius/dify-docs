...
<Note> ⚠️ このドキュメントは AI によって自動翻訳されています。不正確な部分がある場合は、[英語版](/en/self-host/use-dify/knowledge/external-knowledge-api) を参照してください。</Note>

このページでは、Dify が外部ナレッジサービスからコンテンツを取得するために実装が必要な API 仕様を定義します。API の準備ができたら、[外部ナレッジベースとの接続](/ja/self-host/use-dify/knowledge/connect-external-knowledge-base)を参照して Dify に登録してください。
