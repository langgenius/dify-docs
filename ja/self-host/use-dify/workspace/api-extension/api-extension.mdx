...
> このドキュメントは AI によって自動翻訳されています。不正確な部分がある場合は、[英語版](/en/self-host/use-dify/workspace/api-extension/api-extension) を参照してください。
...
Dify を別の場所にデプロイしたまま、ローカルマシンで動作するカスタム API をデバッグするには、[Ngrok](https://ngrok.com) を使用してローカルエンドポイントを公開インターネットに公開し、Dify インスタンスから到達できるようにします。操作手順：
...
この API エンドポイントは公開ネットワークでアクセス可能になります。これで、Dify でこの API エンドポイントを設定してローカルコードをデバッグできます。設定手順については、[外部データツール](/ja/self-host/use-dify/workspace/api-extension/external-data-tool-api-extension) を参照してください。
...
詳細な説明については、[Cloudflare Workers を使用したカスタム API のデプロイ](/ja/self-host/use-dify/workspace/api-extension/cloudflare-worker) を参照してください。
