---
title: "30 分間クイックスタート"
description: "サンプルアプリを通じて Dify を深く理解"
---

<Note> ⚠️ このドキュメントは AI によって自動翻訳されています。不正確な部分がある場合は、[英語版](/en/self-host/use-dify/getting-started/quick-start) を参照してください。</Note>
...
<Frame>
  ![ワークフロー概要](/images/use-dify/get-started/deeper-dive-workflow-overview.png)
</Frame>

## 始める前に

<Steps>
  <Step title="Dify のデプロイ">

    まだ稼働中のインスタンスがない場合は、[Docker Compose で Dify をデプロイ](/ja/self-host/deploy/ce/quick-start/docker-compose) に従ってローカルに構築します。ソースコードからのインストールや AMI ベースのデプロイもサポートしています。代替手段については [デプロイ](/ja/self-host/deploy/ce/quick-start/docker-compose) セクションを参照してください。

  </Step>

  <Step title="サインイン">

    ブラウザで Dify インスタンスを開き、インストール時に作成した管理者アカウントでサインインします。サインインすると、デフォルトのワークスペースに移動します。
  ...
  <Step title="モデルプロバイダーの設定">
    ...
    **設定 ** > ** モデルプロバイダー** に移動し、OpenAI プラグインをインストールします。このチュートリアルでは `gpt-5.2` を使用します。
    ...
    プラグインの設定で独自の OpenAI API キーを追加します。有効な認証情報を 1 つ以上追加するまで、プラグインは使用できません。

    <Tip>
      [マーケットプレイス](https://marketplace.dify.ai/) から任意の LLM プラグインをインストールし、独自の API キーを使用できます。Ollama などのローカルモデルプロバイダーも利用可能で、対応するプラグインでエンドポイントを設定します。
    </Tip>
  ...
  <Step title="デフォルトモデルの設定">
    ...
    2. **システム推論モデル** を `gpt-5.2`、または同等の性能を持つ別のモデルに設定します。これがワークフローのデフォルトモデルになります。
  </Step>
...
<Frame>
  ![ユーザー入力](/images/use-dify/get-started/deeper-dive-start.png)
</Frame>
...
<Frame>
  ![パラメータ抽出器](/images/use-dify/get-started/deeper-dive-parameter-extractor.png)
</Frame>
...
<Frame>
  ![IF ブランチ](/images/use-dify/get-started/deeper-dive-if.png)
</Frame>
...
<Frame>
  ![リスト演算子](/images/use-dify/get-started/deeper-dive-list-operator.png)
</Frame>
...
1. ドキュメント抽出器ノードの後に、LLM ノードを追加します。
...
3. LLM ノードをクリックして設定します：
    ...
    5. **メッセージを追加** をクリックしてユーザーメッセージを追加し、以下を貼り付けます。`{`または`/`を入力して、`Doc Extractor/text`と`User Input/draft`をリスト内の対応する変数に置き換えます。
        ...
        <Frame>
          ![ユーザーメッセージ](/images/use-dify/get-started/deeper-dive-user-message.png)
        </Frame>
...
<Frame>
  ![イテレーションノード](/images/use-dify/get-started/deeper-dive-iteration.png)
</Frame>
...
3. スタイル識別ノードの後に、別の LLM ノードを追加して設定します：
    ...
    4. 構造化出力を有効にします。
        ...
        <Frame>
          ![構造化出力](/images/use-dify/get-started/deeper-dive-structured-output.png)
        </Frame>
        ...
        2. ポップアップスキーマエディタで、右上隅の **JSON からインポート** をクリックし、以下を貼り付けます：
            ...
            ```
            <Frame>
              ![JSON からインポート](/images/use-dify/get-started/deeper-dive-import-from-json.png)
            </Frame>
...
4. イテレーションノードをクリックして設定します：
    ...
    <Frame>
      ![イテレーション設定](/images/use-dify/get-started/deeper-dive-iteration-config.png)
    </Frame>
...
<Frame>
  ![テンプレートノード](/images/use-dify/get-started/deeper-dive-template.png)
</Frame>
...
1. チェックリストがクリアされていることを確認します。
    ...
    <Frame>
      ![チェックリストを確認](/images/use-dify/get-started/deeper-dive-checklist-clear.png)
    </Frame>
...
<Frame>
  ![テスト出力](/images/use-dify/get-started/deeper-dive-test-output.png)
</Frame>
