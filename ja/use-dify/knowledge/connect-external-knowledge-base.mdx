...
2. 以下のフィールドを入力します：
    ...
    - **APIエンドポイント**：外部ナレッジサービスの検索URL。必要なエンドポイント形式については、[外部ナレッジベースAPI仕様](/ja/use-dify/knowledge/external-knowledge-api)を参照してください。
...
2. 以下のフィールドを入力します：
    ...
    - **検索設定**：
      ...
      - **スコアしきい値**：返されるチャンクの最小類似度スコア。これを有効にすると、関連性の低い結果をフィルタリングできます。厳密な関連性にはより高いしきい値を、より広いマッチを含めるにはより低いしきい値を使用します。

          無効にすると、スコアに関係なくTop K制限までのすべての結果が返されます。

作成後、外部ナレッジベースは組み込みナレッジベースと同様にアプリケーションで使用できます。次のステップについては、[アプリケーション内でナレッジを統合する](/ja/use-dify/knowledge/integrate-knowledge-within-application)を参照してください。

## 接続例

### LlamaCloud

DifyマーケットプレイスにLlamaCloud](https://marketplace.dify.ai/plugin/langgenius/llamacloud)は、カスタムAPIを構築することなくLlamaCloudナレッジベースに接続できます。プラグインをインストールし、設定ウィザードに従ってLlamaCloud APIキーを入力してください。
...
検索が失敗したり、予期しない結果が返される場合は、[外部ナレッジベースAPI仕様](/ja/use-dify/knowledge/external-knowledge-api#response-elements)に対してAPIレスポンスを検証してください。
