...
* モデルプロバイダーのルールは、[Provider](/en/model-designing-rules#provider) エンティティに基づいています。
* モデルルールは、[AIModelEntity](/en/model-designing-rules#aimodelentity) エンティティに基づいています。
...
* `help` (object) \[optional] ヘルプ情報
  ...
  * `url` (object) ヘルプリンク。多言語対応。
    ...
    * `en_US` (string) 英語のリンク
* `supported_model_types` (array\[[ModelType](/en/model-designing-rules#modeltype)]) サポートされているモデルタイプ
* `configurate_methods` (array\[[ConfigurateMethod](/en/model-designing-rules#configuratemethod)]) 設定方法
* `provider_credential_schema` (\[[ProviderCredentialSchema](/en/model-designing-rules#providercredentialschema)]) プロバイダーの認証情報スキーマ（プロバイダーの資格情報仕様）
* `model_credential_schema` (\[[ModelCredentialSchema](/en/model-designing-rules#modelcredentialschema)]) モデルの認証情報スキーマ（モデルの資格情報仕様）
...
* `parameter_rules` (array\[[ParameterRule](/en/model-designing-rules#parameterrule)]) \[optional] モデル呼び出しパラメータのルール
* `pricing` (\[[PriceConfig](/en/model-designing-rules#priceconfig)]) \[optional] 価格情報
...
* `credential_form_schemas` (array\[[CredentialFormSchema](/en/model-designing-rules#credentialformschema)]) 資格情報フォームの仕様
...
* `options` (array\[[FormOption](/en/model-designing-rules#formoption)]) フォーム項目が `select` または `radio` の場合の専用属性。ドロップダウンの内容を定義します。
...
* `show_on` (array\[[FormShowOnObject](/en/model-designing-rules#formshowonobject)]) 他のフォーム項目の値が条件を満たす場合に表示します。空の場合は常に表示します。
