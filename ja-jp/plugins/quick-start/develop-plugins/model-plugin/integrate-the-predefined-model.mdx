...
[モデルプロバイダーの作成](/en/create-model-providers)が完了していることを確認してください。事前定義済みモデルを組み込むには、以下の手順に従います。
...
3.  **事前定義済みモデル設定の追加**
    ...
    プロバイダーが事前定義済みモデルを提供している場合、各モデルに対してモデル名と同名の`YAML`ファイル（例：`claude-3.5.yaml`）を作成します。[AIModelEntity](./schema-definition/model/model-designing-rules#aimodelentity)の仕様に従ってファイルの内容を記述し、モデルのパラメータと機能を定義します。
4.  **プラグインのテスト**
...
すべてのモデル設定を準備してから、モデルコードの実装を開始することをお勧めします。完全なYAMLの記述規則については、[モデル設計規則](./schema-definition/model/model-designing-rules)を参照してください。詳細なコードについては、[Githubコードリポジトリ](https://github.com/langgenius/dify-official-plugins/tree/main/models)の例を参照してください。
...
作成したプラグインを[Dify Pluginsコードリポジトリ](https://github.com/langgenius/dify-plugins)にアップロードして公開できます。アップロードする前に、プラグインが[プラグイン公開仕様](/zh-hans/plugins/publish-plugins/publish-to-dify-marketplace)に準拠していることを確認してください。審査に合格すると、コードがメインブランチにマージされ、[Dify Marketplace](https://marketplace.dify.ai/)に自動的に公開されます。
...
* [モデルタイププラグインの開発](/en/README)
* [バンドルタイププラグイン：複数のプラグインをパッケージ化](/en/../bundle)

**プラグインインターフェースドキュメント:**

* [マニフェスト](./schema-definition/manifest) 構造
* [エンドポイント](./schema-definition/endpoint) 詳細定義
* [Difyサービスの逆呼び出し](./schema-definition/reverse-invocation-of-the-dify-service/)
* [ツール](./schema-definition/tool)
* [モデル](./schema-definition/model/)
* [拡張エージェント戦略](./schema-definition/agent)