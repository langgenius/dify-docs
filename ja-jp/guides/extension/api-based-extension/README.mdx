...
これで、このAPIエンドポイントはパブリックアクセス可能となります。これで、DifyでこのAPIエンドポイントを設定してローカルデバッグコードを実行できます。設定手順については、[外部データツール](/ja-jp/guides/extension/api-based-extension/external-data-tool)を参照してください。
