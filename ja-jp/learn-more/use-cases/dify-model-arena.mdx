...
Difyのチャットボットタイプのアプリケーションにプリインストールされている[複数モデルのデバッグ](/en/../../guides/application-orchestrate/multiple-llms-debugging)機能を使うと、同じ質問に対する異なるLLMの回答効果を同時に比較できます。この記事では、DeepSeek R1とo1を例に、Dify内でさまざまな大規模言語モデルの回答品質を直感的に比較する方法をご紹介します。
...
テストを開始する前に、**「右上」→[「新しいプロバイダーを追加」](/en/../../guides/model-configuration/README)** をクリックし、表示される指示に従って、複数のモデルのAPIキーを手動で追加してください。
...
詳細や利用に関するご質問は、[複数モデルのデバッグ](/en/../../guides/application-orchestrate/multiple-llms-debugging)をご覧ください。
