...
class MappingValidator:
    def __init__(self, json_file: str = "plugin_mappings.json"):
        ...
        self.json_file = self.base_dir / "plugin_dev_ja" / "sync" / json_file
        self.plugin_dir = self.base_dir / "ja-jp" / "plugins"
        self.dev_dir = self.base_dir / "plugin_dev_ja"
        self.mappings = []
        self.load_mappings()
    ...
    def calculate_mapping_stats(self) -> Dict:
        ...
        plugin_only = sum(1 for m in self.mappings if m.get(
            'plugin_path') and not m.get('dev_path'))
        dev_only = sum(1 for m in self.mappings if m.get(
            'dev_path') and not m.get('plugin_path'))
        complete = sum(1 for m in self.mappings if m.get(
            'plugin_path') and m.get('dev_path'))
    ...
    def validate(self):
        ...
        print(
            f"  插件预期数 = 总映射数({stats['total_mappings']}) - 仅开发数({stats['dev_only']}) = {stats['expected_plugin_files']}")
        print(
            f"  开发预期数 = 总映射数({stats['total_mappings']}) - 仅插件数({stats['plugin_only']}) = {stats['expected_dev_files']}")
    ...
    def find_discrepancies(self):
        ...
        # 获取映射中的文件路径
        mapped_plugin_files = set(m['plugin_path']
                                  for m in self.mappings if m.get('plugin_path'))
        mapped_dev_files = set(m['dev_path']
