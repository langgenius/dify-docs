...
把消息作为位置参数传给 [`difyctl run app`](/zh/cli/reference/apps#运行应用)，即可向聊天助手、Chatflow、Agent 或文本生成应用发送消息。回复会打印到 stdout。
...
从每条聊天助手或 Chatflow 回复后的提示中复制对话 ID，再用 [`--conversation`](/zh/cli/reference/apps#运行应用) 把它传回，即可继续同一段对话。
