...
- 在 Agent 运行的机器上 [安装](/zh/cli/install) `difyctl` 并 [登录](/zh/cli/integrate-agents/auth-for-agent-deployments)，使其能复用你的会话。
...
| 命令失败并返回退出码 4 | Agent 没有可复用的会话。先在其机器上 [登录](/zh/cli/integrate-agents/auth-for-agent-deployments)。 |
