...
以 Agent 要扮演的账户，在目标机器上登录。Agent 会继承该账户能访问的一切。

使用 Dify Cloud 时，`--host` 填写 `https://cloud.dify.ai`；自部署时，使用 [控制台 API URL](/zh/self-host/deploy/configuration/environments#console_api_url)。机器没有浏览器时，加上 `--no-browser`：
