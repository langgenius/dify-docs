...
| `--mode <mode>` | string | 无 | 按应用类型筛选，以其 API mode 命名：<ul><li>`chat`（聊天助手）</li><li>`advanced-chat`（Chatflow）</li><li>`agent-chat`（Agent）</li><li>`workflow`（Workflow）</li><li>`completion`（文本生成应用）</li></ul> |
| `--page <n>` | integer | `1` | 页码。 |
| `--limit <n>` | integer | `20` | 每页数量，1 到 200。该标志优先，其次是 [`DIFY_LIMIT`](/zh/cli/reference/environment-variables)。 |
| `--workspace <id>` <Badge color="blue" size="sm">Cloud</Badge> | string | 当前活跃工作空间 | 仅本次调用针对另一个工作空间运行。<br></br><br></br>`difyctl` 如何解析工作空间，参见 [difyctl 如何选择工作空间](/zh/cli/reference/workspaces#difyctl-如何选择工作空间)。 |
| `-A, --all-workspaces` <Badge color="blue" size="sm">Cloud</Badge> | boolean | `false` | 列出你的令牌可见的所有工作空间中的应用。 |
...
| 标志 | 类型 | 默认值 | 说明 |
|:---|:---|:---|:---|
| `--inputs <json>` | string | 无 | 以单个 JSON 对象形式提供的输入变量，例如 `--inputs '{"topic":"Q3"}'`。Workflow 应用必填。与 `--inputs-file` 互斥。 |
...
| `--workflow-id <id>` | string | 无 | 将本次运行固定到某个已发布的工作流版本。仅适用于 Workflow 和 Chatflow 应用。 |
| `--stream` | boolean | `false` | 在生成过程中实时打印输出，而非在结束时一次性打印。 |
| `--think` | boolean | `false` | 当模型暴露其思考过程时，将其打印到 stderr，可以是内联的 `<think>` 块，也可以是独立的推理流。<br></br><br></br>不加该标志时，这些内容会被隐藏。 |
...
| 格式 | stdout 内容 |
|:---|:---|
| 默认（`text`） | 回复（聊天助手、Chatflow、Agent、文本生成应用）或工作流的输出，以纯文本形式呈现。 |
...
| `1` | 网络或服务器错误，包括应用未找到 |
| `2` | 用法错误：`--inputs` JSON 无效，或向 Workflow 应用传入了位置参数形式的消息 |
| `4` | 认证失败 |
| `7` | 触发限流（HTTP 429） |
...
- `<form-token>`：必填。来自暂停载荷的 `form_token`。令牌仅可使用一次，因此用已消费的令牌恢复会返回错误。
...
| `--action <id>` | string | 自动选择 | 要执行哪个表单操作，由暂停载荷 `actions` 中的 `id` 指定。<br></br><br></br>表单恰好只有一个操作时可选，有多个操作时必填。 |
| `--inputs <json>` | string | 无 | 以单个 JSON 对象形式提供的表单输入值，以每个输入的 `output_variable_name` 为键。<br></br><br></br>与 `--inputs-file` 互斥。 |
...
恢复后的工作流可能在后续的人工介入节点再次暂停。届时你会得到一个新的暂停载荷，并用新令牌再次恢复。
...
|:---|:---|
| `0` | 成功，包括运行在后续节点再次暂停 |
| `1` | 错误，包括令牌已被消费，或在有多个操作的表单上省略了 `--action` |
| `2` | 用法错误 |
| `4` | 认证失败 |
| `7` | 触发限流（HTTP 429） |
...
对于 Workflow 和 Chatflow 应用，导出返回的是当前草稿，而非 `run app` 所执行的已发布版本。改用 `--workflow-id` 可导出指定的已发布版本。聊天助手、Agent 和文本生成应用导出的是已发布版本。
...
| 标志 | 类型 | 默认值 | 说明 |
|:---|:---|:---|:---|
| `-o, --output <path>` | string | 无 | 将 DSL 写入该文件，而非 stdout。<br></br><br></br>在本命令中，`-o` 是输出文件路径，而非输出格式选择器。 |
| `--include-secret` | boolean | `false` | 在导出的 DSL 中包含加密的密文值。 |
| `--workflow-id <id>` | string | 无 | 按 ID 导出指定的已发布工作流版本，而非默认的草稿。<br></br><br></br>仅适用于 Workflow 和 Chatflow 应用。 |
| `--workspace <id>` <Badge color="blue" size="sm">Cloud</Badge> | string | 当前活跃工作空间 | 仅本次调用针对另一个工作空间运行。<br></br><br></br>`difyctl` 如何解析工作空间，参见 [difyctl 如何选择工作空间](/zh/cli/reference/workspaces#difyctl-如何选择工作空间)。 |
...
对于 Workflow 和 Chatflow 应用，它会将定义写入应用的草稿。`run app` 使用的是已发布版本，因此导入后请在 Dify 中发布应用，更改才会生效。
...
| `--app-id <id>` | string | 无 | 覆盖一个已有应用，而非创建新应用。<br></br><br></br>仅适用于 Workflow 和 Chatflow 应用。 |
| `--icon-type <type>` | string | 取自 DSL | 覆盖图标类型。 |
| `--icon <icon>` | string | 取自 DSL | 覆盖图标。 |
| `--icon-background <color>` | string | 取自 DSL | 覆盖图标背景色。 |
| `--workspace <id>` <Badge color="blue" size="sm">Cloud</Badge> | string | 当前活跃工作空间 | 仅本次调用导入到另一个工作空间。<br></br><br></br>`difyctl` 如何解析工作空间，参见 [difyctl 如何选择工作空间](/zh/cli/reference/workspaces#difyctl-如何选择工作空间)。 |
