...
<Tip>
日常调用方式参见常见任务，[账号用户](/zh/cli/account-users/common-tasks#查找应用) 或 [SSO 用户](/zh/cli/sso-users/common-tasks)。
</Tip>
...
| `--workspace <id>` | string | 当前活跃工作空间 | 仅本次调用针对另一个工作空间运行。`difyctl` 如何解析工作空间，参见 [difyctl 如何选择工作空间](/zh/cli/reference/workspaces#difyctl-如何选择工作空间)。 |
| `-A, --all-workspaces` | boolean | `false` | 列出你的 token 可见的所有工作空间中的应用。 |
| `-o <format>` | string | 无 | 输出格式：`json`、`yaml`、`name` 或 `wide`。省略该标志则输出默认表格。 |

<Note>
SSO 用户看到的是管理员授权的应用，而非工作空间列表；`--workspace` 和 `-A` 不适用。详见 [权限范围与限制](/zh/cli/sso-users/scope-and-limitations)。
</Note>
...
| `--workspace <id>` | string | 当前活跃工作空间 | 仅本次调用导入到另一个工作空间。`difyctl` 如何解析工作空间，参见 [difyctl 如何选择工作空间](/zh/cli/reference/workspaces#difyctl-如何选择工作空间)。 |
