...
`difyctl` 可同时保持对多个 Dify 主机和账户的登录状态。每一对主机和账户构成一个上下文：其元数据保存在 [配置目录](/zh/cli/reference/environment-variables) 下的 `hosts.yml` 中，其 Bearer 令牌则保存在操作系统的凭据存储中（并以受保护文件作为回退方案）。
...
| 标志 | 类型 | 默认值 | 说明 |
|:---|:---|:---|:---|
| `--host <url>` | string | none | 要登录的 Dify 主机：<ul><li>使用 Dify Cloud 时，填写 `https://cloud.dify.ai`。</li><li>在自部署的 Dify 上，使用 [控制台 API URL](/zh/self-host/deploy/configuration/environments#console_api_url)。</li></ul>不带协议的 URL 会自动补上 `https://`。<br></br><br></br>在终端中可省略此标志，直接输入主机；在非交互式会话（脚本、CI、管道）中，此标志为必填。 |
| `--no-browser` | boolean | false | 不自动打开浏览器，仅打印验证 URL。 |
| `--insecure` | boolean | false | 允许 `http://` 主机，并接受 TLS 证书为自签名或无效的 `https://` 主机（仅限本地开发）。 |
...
每次登录还会注册一个服务端会话，标签为 `difyctl on <hostname>`。运行 [`auth devices list`](#列出活跃会话) 可查看你的所有会话。登录到一个已登录的主机，会刷新该上下文中存储的令牌。
...
| `6` | 版本不匹配：服务器版本对此 `difyctl` 而言过旧（在保存会话之前进行检查），或此 `difyctl` 对服务器而言过旧（HTTP `426`）。服务器没有设备流程登录端点（`unsupported_endpoint`）时也会如此 |
| `7` | 触发限流（HTTP 429） |
...
`auth whoami` 打印当前上下文所属的身份。它读取存储的上下文，不会联系服务器，因此回答的是「当前活跃的是哪个身份」，而不是「令牌是否仍然有效」。即使会话已在服务端过期，它也会以 `0` 退出。
...
`auth logout` 会在服务端撤销当前上下文的会话，然后从本机删除其令牌和 `hosts.yml` 条目。其他已保存的上下文不受影响。

本地清理始终会执行。如果服务端撤销失败，`difyctl` 会在 stderr 打印一条警告，并照常清除你的凭据。

令牌存放在操作系统钥匙串中、而退出登录时又无法读取（已锁定，或没有桌面会话）时也是如此：跳过撤销，本地清理仍会执行。
...
| 标志 | 类型 | 默认值 | 说明 |
|:---|:---|:---|:---|
| `--domain <host>` | string | none | 要切换到的已存储主机，写作其裸域名（`cloud.dify.ai`），不带协议。<br></br><br></br>在终端中可省略此标志，从已存储的主机中选取。在非交互式会话（脚本、CI、管道）中，此标志为必填。 |
...
| 标志 | 类型 | 默认值 | 说明 |
|:---|:---|:---|:---|
| `--email <email>` | string | none | 要切换到的账户。<br></br><br></br>在终端中可省略此标志，从已存储的账户中选取；在非交互式会话（脚本、CI、管道）中，此标志为必填。 |
...
如果某邮箱未存储在当前主机上，则会以用法错误失败，并列出已知的账户。如果某账户已存储的令牌不复存在（已撤销，或被 [会话撤销](#撤销会话) 清除），则会以未登录错误失败，并提示你重新登录。
