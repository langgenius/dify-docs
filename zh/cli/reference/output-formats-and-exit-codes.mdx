...
| `raw_response` | 仅在 `-v` 下 | 服务器原始响应体，Bearer 令牌已脱敏。 |
...
| `6` | 版本或兼容性错误 | `version_skew`（服务器版本低于此 `difyctl` 所支持的版本，或服务器以 HTTP 426 拒绝了过旧的 `difyctl`）、`unsupported_endpoint`（服务器缺少此版本所需的端点），或无法读取的配置文件（`config_schema_unsupported`）。 |
