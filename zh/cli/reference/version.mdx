...
- **`too_old`**：命令会在发送任何请求之前以退出码 [`6`](/zh/cli/reference/output-formats-and-exit-codes) 停止，并提示你将 Dify 服务器升级到该版本所支持的最低版本（或[安装与服务器相匹配的 `difyctl`](/zh/cli/install)）。
- **`too_new`**：命令照常运行，并向 stderr 打印一行警告。
- **`unknown`**：命令照常运行；判定为 unknown 时不进行拦截。

通过检查的结果会按主机缓存约一小时，因此 `difyctl` 不会在每次执行命令时重新探测；升级服务器后，缓存会立即失效。
...
`--check-compat` 让判定结果可用于脚本：只要不是 `compatible`，包括所有 `unknown` 情况，都会以 `64` 退出。该检查每次都会实时探测服务器，因此脚本中的校验始终反映服务器当前的版本，而不是缓存的结果。
