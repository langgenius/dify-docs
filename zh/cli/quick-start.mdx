...
<Note>
自部署环境需先将 `OPENAPI_ENABLED` 和 `ENABLE_OAUTH_BEARER` 都设为 `true`，`difyctl` 才能登录。完整变量说明详见 [Dify CLI 与编程 API](/zh/self-host/deploy/configuration/environments#dify-cli-与编程-api)。
</Note>
