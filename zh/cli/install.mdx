...
<Steps>
  ...
  <Tip>
    ...
    | 变量 | 说明 | 示例 |
    | --- | --- | --- |
    | `DIFY_VERSION` | 用于安装 `difyctl` 的 Dify 发布标签，取自 [Dify Releases](https://github.com/langgenius/dify/releases)（不含 `v` 前缀）。默认使用最新发布版本；当服务器不是最新版本时，将其设置为服务器的发布标签。 | `1.15.0` |
    | `DIFYCTL_VERSION` | 固定使用特定的 `difyctl` 构建版本，不含 `v` 前缀。仅在未设置 `DIFY_VERSION` 时生效。 | `0.1.0-alpha` |
