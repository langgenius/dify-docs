...
<Steps>
  ...
  <Tip>
    ...
    | 变量 | 说明 |
    | --- | --- |
    | `DIFY_VERSION` | 用于安装 `difyctl` 的 Dify 发布标签。默认使用最新发布版本；当服务器不是最新版本时，将其设置为服务器的发布标签。 |
    | `DIFYCTL_VERSION` | 固定使用特定的 `difyctl` 构建版本。仅在未设置 `DIFY_VERSION` 时生效。 |
    | `DIFYCTL_PREFIX` | 安装目录（默认 `~/.local`）。二进制文件会放在 `<prefix>/bin` 下。 |
...
<CardGroup cols={2}>
  <Card title="账户用户" icon="user" href="/zh/cli/account-users/quick-start">
    使用直接在 Dify 中注册的邮箱和密码登录。
  </Card>
  <Card title="SSO 用户" icon="id-card" href="/zh/cli/sso-users/quick-start">
    通过你所在组织的身份提供商登录（Google Workspace、Okta、Azure AD 等）。
  </Card>
</CardGroup>
