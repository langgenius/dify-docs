...
不经过 Dify 控制台，直接在自己的代码中构建和维护知识库：创建知识库、导入文档和分段、用元数据和标签组织内容，并可直接调用检索接口用于搜索或 RAG。
...
- 复制 API 端点，这是所有知识库 API 请求的基础 URL。
- 点击 **API 密钥** 创建和管理密钥。
...
<Steps>
  ...
  <Step title="添加文档">
    调用 [从文本创建文档](/zh/api-reference/documents/create-document-by-text) 或 [从文件创建文档](/zh/api-reference/documents/create-document-by-file)，两者都会返回一个 `batch` ID。如果创建知识库时未设置 `indexing_technique`，在第一个文档上设置即可，后续文档会自动继承。
  </Step>
  <Step title="轮询索引状态">
