...
`blocking`（阻塞式返回）在生成结束后一次性返回一个 JSON 响应体。对简短、非交互的调用来说，它集成起来更简单，但长时间生成有被中断的风险：代理会切断长请求；在 Dify Cloud 上，如果边缘代理在超时时间内未收到上游响应，就可能中断连接。

`streaming`（流式返回）以 SSE 事件的形式传送回复。面向用户的场景、长时间运行的任务，以及所有会因 [人工介入](/zh/api-reference/guides/human-input-flow) 暂停的流程，都应使用它。

<Info>
旧版 Agent 和 Agent 应用仅支持流式返回。
</Info>
...
1. 按顺序拼接回复片段：
    ...
    - 旧版 Agent 和 Agent 应用为 `agent_message` 事件

    Agent 应用在收尾时会额外返回一个重复完整回答的 `message` 事件，将其视为最终回答即可，不要再追加拼接。

2. 在正确的终止事件上收尾：
    - Chatbot、旧版 Agent 和 Agent 应用为 `message_end`
