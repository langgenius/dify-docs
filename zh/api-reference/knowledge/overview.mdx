...
知识库 API 直接管理知识库及其内容，不依赖任何应用：创建知识库、添加文档和分段、附加元数据和标签，还可独立调用检索接口进行生产检索或召回测试。
...
- **[创建空知识库](/zh/api-reference/knowledge-bases/create-an-empty-knowledge-base)**：创建一个还没有文档的知识库。`permission` 控制访问权限，`indexing_technique` 决定使用 `high_quality`（基于嵌入模型的检索）还是 `economy`（基于关键词的检索）。
...
- **[从知识库检索分段 / 测试检索](/zh/api-reference/knowledge-bases/retrieve-chunks-from-a-knowledge-base-test-retrieval)**：搜索知识库并返回最相关的分段，生产检索和召回测试共用同一个接口。传入 `retrieval_model` 可覆盖默认检索设置，传入 `attachment_ids` 可将已上传的附件纳入检索上下文。
...
<Steps>
  ...
  <Step title="添加文档">
    调用 [从文本创建文档](/zh/api-reference/documents/create-document-by-text) 或 [从文件创建文档](/zh/api-reference/documents/create-document-by-file)，两者都会返回一个 `batch` ID。`doc_form` 决定分段模式：`text_model` 对应通用分段，`hierarchical_model` 对应父子分段，`qa_model` 用于提取问答对。如果创建知识库时未设置 `indexing_technique`，在第一个文档上设置即可，后续文档会自动继承。
  </Step>
  <Step title="轮询索引状态">
...
- **[向文档添加分段](/zh/api-reference/chunks/create-chunks)**：为文档添加一个或多个分段，每个分段都需要 `content`，问答模式（`qa_model`）文档还需要 `answer`。
...
- **[获取内置元数据字段](/zh/api-reference/metadata/get-built-in-metadata-fields)**：返回系统内置字段，如文档类型、来源 URL。
