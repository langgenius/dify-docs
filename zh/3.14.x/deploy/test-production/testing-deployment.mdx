...
- Redis 数据库
    - 版本 6+
    - 1 GB RAM
- [向量数据库虚拟机 - Qdrant 单节点](/zh/3.14.x/deploy/advanced-configuration/qdrant/single-node)
    - 1 个虚拟机
    - 4 CPU，8 GB RAM
    - 100 GB 存储
...
<Note>
Dify 3.12 起默认启用 Agent Sandbox（`agentBackend` 与 `sandboxGateway`），需要在集群中预装 agent-sandbox controller；无法安装时可切换为共享沙箱模式。详见 [Agent Sandbox 部署](/zh/3.14.x/deploy/advanced-configuration/agent-sandbox)。
</Note>
...
- 更多信息，请参考 [External Qdrant Single Node](/zh/3.14.x/deploy/advanced-configuration/qdrant/single-node)。
