...
根据所需要开启的 SSO 范围，**Callback URL 有所差异**。系统管理员需要将 Dify 企业版的 Callback URL 填写至对应的 Okta 应用程序内以完成创建流程。

<Tabs>
  <Tab title="Workspace">
   1. 点击 **管理后台** → **身份认证** → **成员认证** → **SSO 身份提供商** → **新建身份提供商** → **新建 OAuth2 提供商**，在底部获取 Callback URL。
  ...
  <Tab title="WebApp External Users（面向外部用户）">
   1. 点击 **管理后台** → **身份认证** → **Web 应用** → **SSO 身份提供商** → **新建身份提供商** → **新建 OAuth2 提供商**，在底部获取 Callback URL。
...
3. 切换至 "Sign On" 页，将 **Issuer** 切换为固定链接。
...
> 如何获取 `your-okta-issuer-url`：**Security → API → Authorization Servers → Issuer URI**。如需了解如何通过 Okta App 获取更多字段信息，请参考 [Use of the Issuer](https://support.okta.com/help/s/article/What-is-theIssuerlocated-under-the-OpenID-Connect-ID-Token-app-settings-used-for?language=en_US)。
