...
- 启用后，内部用户访问任何由 Dify 企业版平台创建的 Web 应用之前都需要通过身份提供商（IdP）的身份验证，鉴权后才能够被允许访问。该设置为企业级全局选项；单个 Web 应用的访问权限可在其访问控制设置中单独管理，详见 [应用访问权限管理](/zh/3.14.x/use/publish/webapp/web-app-settings)。
...
出于安全性考虑，部分身份提供商（IDP）会禁用在 iframe 页面中启用 SSO 鉴权。这可能会影响 WebApp 嵌入至其它网页的情况，你需要检查身份提供商（IDP）的 CSP: frame-ancestors 配置以确保正常启用该功能，详细说明请参考[此文档](/zh/3.14.x/administer/faq)。
...
详细说明请参考 [常见问题](/zh/3.14.x/administer/faq)。
