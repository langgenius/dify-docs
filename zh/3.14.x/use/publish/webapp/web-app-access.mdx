---
title: 访问控制
description: 通过四个访问级别，控制谁可以访问你发布的 Web 应用，从特定成员到完全公开。
---
...
<Info>
如果此选项被禁用，请联系企业控制面板管理员配置 [Web 应用外部用户认证](/zh/3.14.x/administer/authentication/webapp-external-user-auth)。
</Info>
...
<Info>
如果该选项不可选，并提示「公开访问已被管理员禁用」，说明企业控制面板管理员已在整个部署关闭公开访问（Helm values 中 `enterprise.webappPublicAccessEnabled: false`）。
</Info>
