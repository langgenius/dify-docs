...
输出节点仅适用于 Workflow 应用。Chatflow 应用请使用 [直接回复](/zh/3.14.x/use/nodes/answer) 节点。
</Info>
...
当你将 [工作流发布为工具](/zh/3.14.x/use/workspace/tools#workflow) 时，输出节点定义了工具的返回结构。每个输出变量名称会成为工具结果中的一个键，供调用该工具的工作流使用。
