...
<Tip>
    若目标外部系统有可用的触发器集成，推荐使用 [集成触发器](/zh/3.14.x/use/nodes/trigger/plugin-trigger)。
</Tip>
...
<Tip>
    一个工作流可以包含多个 Webhook 触发器。如果这些触发器分支共享相同的下游节点，可以添加 [变量聚合器](/zh/3.14.x/use/nodes/variable-aggregator) 将它们汇聚，避免在每条分支上重复配置相同的节点。
</Tip>
