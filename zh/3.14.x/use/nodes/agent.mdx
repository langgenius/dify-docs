...
<Tabs>
  <Tab title="经典 Agent" id="classic-agent">
  ...
  **记忆管理** 根据你的用例要求平衡上下文保留与标记效率。
  ...
  <Tab title="新 Agent" id="new-agent">
  ...
  **安全提示：Agent 环境变量外传风险**
  ...
  <Note>
  新 Agent 节点目前处于 Beta 阶段，仅在 Workflow 应用中可用。使用前需在集群中启用 agent-backend 与沙箱环境，部署方法见 [Agent Sandbox 部署](/zh/3.14.x/deploy/advanced-configuration/agent-sandbox)。

  生产环境使用前，在 Helm values 中将 `agentBackend.serverSecretKey` 替换为你自己的随机值。
  </Note>

  新 Agent 节点将一个 [Agent](/zh/3.14.x/use/build/new-agent/overview) 作为工作流中的一步来运行。与经典节点「模型加工具」的组合不同，这里请来的是一名自带能力和沙箱的完整员工。
  ...
  <Tabs>
    <Tab title="邀请已有 Agent">
      ...
      - 要编辑它的能力，点击 **在 Agents 中编辑**。

      - 若只想在这一步做改动，点击 **创建副本**：节点会切换为一次性副本，不再跟随原 Agent 更新。

    </Tab>
    <Tab title="从空白开始">
      创建一个只属于此节点的一次性 Agent，直接在这里配置，方法与在 **Agents** 页面 [构建](/zh/3.14.x/use/build/new-agent/build) 相同。
  ...
  <Info>
  两个节点即使邀请同一个 Agent，工作时也彼此独立：每个节点都从该 Agent 已保存的配置出发，在各自的沙箱中工作，一个节点运行中产生的内容（写下的文件、安装的工具）不会带入另一个节点。

  要在节点间传递结果，请将它们声明为输出，供下游节点引用。
  </Info>
  ...
  <Note>
  你引用的变量会以文本形式传给 Agent，超过 2000 字符会被截断。要完整传递更长的内容，请改用文件：Agent 会收到整个文件，并在沙箱中读取。
  </Note>
  ...
  Agent 返回的文件，单个默认不超过 50 MB，可通过 `DIFY_AGENT_STUB_UPLOAD_FILE_SIZE_LIMIT` 调整，详见 [环境变量](/zh/3.14.x/deploy/advanced-configuration/environment-variables#agent-backend-配置)。
