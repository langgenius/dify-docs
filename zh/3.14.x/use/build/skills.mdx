...
- [Agent 应用](/zh/3.14.x/use/build/new-agent/build#skill)
- Workflow 应用中的 [新 Agent 节点](/zh/3.14.x/use/nodes/agent#新-agent)

<Info>
默认情况下，除 [受限成员](/zh/3.14.x/use/workspace/roles-and-permissions#系统角色) 外，所有系统角色都可查看和导出 Skill 库中的 Skill。所有者、管理者和编辑者还可创建、导入、复制、编辑和删除 Skill。只有所有者和管理者可发布和恢复版本。

管理员可通过自定义角色分别授予 **查看 Skill**、**编辑 Skill**、**发布 Skill** 和 **删除 Skill** 权限。详见 [权限参考](/zh/3.14.x/use/workspace/permission-reference#skills)。
</Info>
...
要把在其他地方构建的 Skill 加入 Skill 库，点击 **导入**，选择 `.zip` 或 `.skill` 包。包内需有 `SKILL.md`，大小默认不超过 50 MB，可通过 [`UPLOAD_SKILL_FILE_SIZE_LIMIT`](/zh/3.14.x/deploy/advanced-configuration/environment-variables#upload_skill_file_size_limit) 调整。导入后，Skill 会以草稿状态加入 Skill 库，供你检查并发布。
...
<Note>
恢复某个版本会用该版本替换当前草稿，不会发布 Skill，也不会创建新版本。发布恢复后的草稿，才能将它设为最新版本。
</Note>
