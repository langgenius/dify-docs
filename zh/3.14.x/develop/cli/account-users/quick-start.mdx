...
开始之前，请确认已 [安装](/zh/3.14.x/develop/cli/install) `difyctl`。

<Note>
CLI 依赖部署的 `/openapi/v1/*` 接口，默认关闭。登录前在 Helm values 中设置 `api.openapi.enabled: true`；`api.enableOauthBearer` 需保持为 `true`（默认值）。
</Note>

## 第 1 步：登录

1. 登录到公司的 Dify 主机。使用对应实例的 [控制台 API URL](/zh/3.14.x/deploy/advanced-configuration/environment-variables#console_api_url)。
...
<Tabs>
  <Tab title="聊天助手 / Chatflow / Agent / 文本生成应用">
    将消息作为位置参数传入：
    ...
    回复会打印到 stdout。对于聊天助手、Chatflow 和 Agent 应用，还会向 stderr 打印一条提示，方便你稍后继续同一段对话：
...
- **开箱即用的日常命令**：[常见任务](/zh/3.14.x/develop/cli/account-users/common-tasks)
- **token 存储位置与会话机制**：[认证](/zh/3.14.x/develop/cli/account-users/authenticate)
- **`difyctl` 的其余全部能力**：[命令索引](/zh/3.14.x/develop/cli/reference/command-index)
