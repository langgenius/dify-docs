---
title: 认证
description: 从 CLI 登录你的公司 Dify 主机，了解 token 的存储位置，并管理你的会话
---
...
<Steps>
  <Step title="运行登录命令">
    ...
    传入 Dify 主机的 URL。使用 [控制台 API URL](/zh/3.14.x/deploy/advanced-configuration/environment-variables#console_api_url)。
...
配置目录在 macOS 和 Linux 上是 `~/.config/difyctl`（Linux 遵循 `XDG_CONFIG_HOME`），在 Windows 上是 `%APPDATA%\difyctl`。设置 [`DIFY_CONFIG_DIR`](/zh/3.14.x/develop/cli/reference/environment-variables) 可覆盖此路径。
...
其余问题详见完整的 [故障排查](/zh/3.14.x/develop/cli/troubleshooting) 页面。
