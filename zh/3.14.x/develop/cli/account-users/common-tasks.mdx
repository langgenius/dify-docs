...
以下所有命令均假设你已 [登录](/zh/3.14.x/develop/cli/account-users/authenticate)。

## 发送消息

把消息作为位置参数传给 [`difyctl run app`](/zh/3.14.x/develop/cli/reference/apps#运行应用)，即可向聊天助手、Chatflow、Agent 或文本生成应用发送消息。回复会打印到 stdout。
...
用 [`--inputs`](/zh/3.14.x/develop/cli/reference/apps#运行应用) 把输入作为单个 JSON 对象传入，而不是传位置消息参数。输出会以 JSON 形式打印到 stdout。
...
用 [`difyctl get app`](/zh/3.14.x/develop/cli/reference/apps#列出应用) 列出当前工作空间中的应用。用 `--name` 或 `--mode` 缩小范围。
...
加上 [`--stream`](/zh/3.14.x/develop/cli/reference/apps#运行应用)，响应会边生成边打印，而不是在最后一次性输出。
...
从每条聊天助手或 Chatflow 回复后的提示中复制对话 ID，再用 [`--conversation`](/zh/3.14.x/develop/cli/reference/apps#运行应用) 把它传回，即可继续同一段对话。
...
为任意命令加上 [`-o json`](/zh/3.14.x/develop/cli/reference/output-formats-and-exit-codes)，即可获得便于管道处理的原始 JSON 响应。
...
用 [`difyctl use workspace`](/zh/3.14.x/develop/cli/reference/workspaces#切换工作空间) 切换当前工作空间。用 `get workspace` 列出可用的工作空间。
...
运行一个不熟悉的应用前，用 [`difyctl describe app`](/zh/3.14.x/develop/cli/reference/apps#查看应用) 查看它的应用类型和输入结构：各输入的名称、类型，以及哪些是必填项。
