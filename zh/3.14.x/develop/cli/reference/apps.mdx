...
每项应用操作对应一条命令，且都接受 [全局标志](/zh/3.14.x/develop/cli/reference/global-flags)。
...
<Tip>
日常调用方式参见常见任务，[账号用户](/zh/3.14.x/develop/cli/account-users/common-tasks#查找应用) 或 [外部 SSO 用户](/zh/3.14.x/develop/cli/sso-users/common-tasks)。
</Tip>

### 参数

- `[app-id]`：可选。要显示的单个应用的 ID。省略则列出所有应用。
...
| `--page <n>` | integer | `1` | 页码。 |
| `--limit <n>` | integer | `20` | 每页数量，1 到 200。该标志优先，其次是 [`DIFY_LIMIT`](/zh/3.14.x/develop/cli/reference/environment-variables)。 |
| `--workspace <id>` | string | 当前活跃工作空间 | 仅本次调用针对另一个工作空间运行。<br></br><br></br>`difyctl` 如何解析工作空间，参见 [difyctl 如何选择工作空间](/zh/3.14.x/develop/cli/reference/workspaces#difyctl-如何选择工作空间)。 |
| `-A, --all-workspaces` | boolean | `false` | 列出你的 token 可见的所有工作空间中的应用。 |
| `-o <format>` | string | 无 | 输出格式：`json`、`yaml`、`name` 或 `wide`。省略该标志则输出默认表格。 |

<Note>
外部 SSO 用户看到的是管理员授权的应用，而非工作空间列表；`--workspace` 和 `-A` 不适用。详见 [权限范围与限制](/zh/3.14.x/develop/cli/sso-users/scope-and-limitations)。
</Note>
...
完整方案参见 [输出格式与退出码](/zh/3.14.x/develop/cli/reference/output-formats-and-exit-codes)。
...
| `--think` | boolean | `false` | 当模型暴露其思考过程时，将其打印到 stderr。<br></br><br></br>不加该标志时，`<think>` 块会被静默剥离。 |
...
错误输出到 stderr。在 `-o json` 下，它们以带有稳定 `code` 字段的结构化 JSON 对象形式返回。错误的结构参见 [输出格式与退出码](/zh/3.14.x/develop/cli/reference/output-formats-and-exit-codes)。
...
| `--workflow-id <id>` | string | 无 | 按 ID 导出指定的已发布工作流版本，而非默认的草稿。<br></br><br></br>仅适用于工作流和对话流应用。 |
| `--workspace <id>` | string | 当前活跃工作空间 | 仅本次调用针对另一个工作空间运行。<br></br><br></br>`difyctl` 如何解析工作空间，参见 [difyctl 如何选择工作空间](/zh/3.14.x/develop/cli/reference/workspaces#difyctl-如何选择工作空间)。 |
...
| `--app-id <id>` | string | 无 | 覆盖一个已有应用，而非创建新应用。<br></br><br></br>仅适用于工作流和对话流应用。 |
| `--workspace <id>` | string | 当前活跃工作空间 | 仅本次调用导入到另一个工作空间。<br></br><br></br>`difyctl` 如何解析工作空间，参见 [difyctl 如何选择工作空间](/zh/3.14.x/develop/cli/reference/workspaces#difyctl-如何选择工作空间)。 |
