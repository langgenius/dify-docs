...
`difyctl` 专为脚本化设计：数据写入 stdout，其余内容写入 stderr，[`-o` 全局标志](/zh/3.14.x/develop/cli/reference/global-flags) 用于选择输出格式，失败时以可预期的退出码退出。
...
各错误码的含义及修复方法，详见 [故障排查](/zh/3.14.x/develop/cli/troubleshooting)。
...
| `64` | 兼容性检查未通过 | 仅 [`version --check-compat`](/zh/3.14.x/develop/cli/reference/version#在脚本中根据兼容性进行控制)：服务器未确认兼容。 |
...
负载结构及恢复协议，参见 Apps 参考页的 [工作流暂停时](/zh/3.14.x/develop/cli/reference/apps#工作流暂停时)。
