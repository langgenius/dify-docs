...
| `DIFY_WORKSPACE_ID` | 未设置 `--workspace` 参数时，`difyctl` 命令所针对的工作空间。必须为 UUID。完整解析顺序详见 [difyctl 如何选择工作空间](/zh/3.14.x/develop/cli/reference/workspaces#difyctl-如何选择工作空间)。 | 无 |
| `DIFYCTL_HTTP_RETRY` | 幂等请求遇到瞬时故障时的重试次数（`0` 表示禁用重试）。[`--http-retry`](/zh/3.14.x/develop/cli/reference/global-flags#http-重试) 会覆盖它。 | `3` |
...
<Note>
`env list` 接受的是 `--json`，而非 [`-o` 全局参数](/zh/3.14.x/develop/cli/reference/global-flags)。
</Note>
