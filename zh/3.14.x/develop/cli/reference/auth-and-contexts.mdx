...
`difyctl` 可同时保持对多个 Dify 主机和账户的登录状态。每一对主机和账户构成一个上下文：其元数据保存在 [配置目录](/zh/3.14.x/develop/cli/reference/environment-variables) 下的 `hosts.yml` 中，其 bearer token 则保存在操作系统的凭据存储中（并以受保护文件作为回退方案）。
...
分步登录的完整说明，详见 [账户用户](/zh/3.14.x/develop/cli/account-users/authenticate) 或 [外部 SSO 用户](/zh/3.14.x/develop/cli/sso-users/authenticate) 的认证文档。
...
如果你通过外部 SSO 登录，同样的设备流程会经由公司的身份提供商进行路由，并生成一个 `external SSO` 上下文（一个 `dfoe_` token），而非标准的账户上下文；具体细节详见 [外部 SSO 用户：认证](/zh/3.14.x/develop/cli/sso-users/authenticate)。
...
完整的退出码方案，详见 [输出格式与退出码](/zh/3.14.x/develop/cli/reference/output-formats-and-exit-codes)。
...
| 标志 | 类型 | 默认值 | 说明 |
|:---|:---|:---|:---|
| `--domain <host>` | string | none | 要切换到的已存储主机，写作其裸域名（`dify.example.com`），不带协议。<br></br><br></br>在终端中可省略此标志，从已存储的主机中选取。在非交互式会话（脚本、CI、管道）中，此标志为必填。 |
...
| `--page <n>` | integer | 1 | 页码。 |
| `--limit <n>` | integer | 20 | 每页条数，范围 1 到 200。此标志优先，其次是 [`DIFY_LIMIT`](/zh/3.14.x/develop/cli/reference/environment-variables)。 |
