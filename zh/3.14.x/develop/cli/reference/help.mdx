...
该概览列出每个命令及其简介、三个入门示例、[全局标志](/zh/3.14.x/develop/cli/reference/global-flags)，以及 [指南主题](#指南主题)。若想查看同样的清单并附带指向本参考文档的链接，参见 [命令索引](/zh/3.14.x/develop/cli/reference/command-index)。
...
| `external` | 面向外部 SSO 用户的登录帮助。 |

这些主题已内置于 CLI 中。要了解 `difyctl` 实际读取哪些变量，参见 [环境变量](/zh/3.14.x/develop/cli/reference/environment-variables)。
