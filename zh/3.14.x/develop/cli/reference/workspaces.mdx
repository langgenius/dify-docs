...
两者都接受 [全局标志](/zh/3.14.x/develop/cli/reference/global-flags)。

<Note>
工作空间仅适用于账户用户。如果你通过外部 SSO 登录，你被限定在单一上下文中，没有工作空间，本文所述的命令对你不适用。详见 [权限范围与限制](/zh/3.14.x/develop/cli/sso-users/scope-and-limitations)。
</Note>
...
<Tip>
日常调用方式参见常见任务中的 [切换工作空间](/zh/3.14.x/develop/cli/account-users/common-tasks#切换工作空间)。
</Tip>
...
仅 [全局标志](/zh/3.14.x/develop/cli/reference/global-flags)。
...
2. [`DIFY_WORKSPACE_ID`](/zh/3.14.x/develop/cli/reference/environment-variables) 环境变量。
