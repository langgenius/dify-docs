...
<Note>
每个 `difyctl` 构建都对应特定的 Dify 版本，安装与服务器匹配的构建（见下文 `DIFY_VERSION`）。
</Note>
...
<Steps>
  <Step title="运行安装程序">
    每个 Dify 发布版本都会随附一个 `difyctl` 构建版本。Dify Enterprise 3.13.x 运行的是 Dify `1.17.0`，安装对应构建：
  ...
  <Tip>
    ...
    | 变量 | 说明 | 示例 |
    | --- | --- | --- |
    | `DIFY_VERSION` | 用于安装 `difyctl` 的 Dify 发布标签，取自 [Dify Releases](https://github.com/langgenius/dify/releases)。<br></br><br></br>默认使用最新发布版本；当服务器不是最新版本时，将其设置为服务器的发布标签。 | `1.17.0` |
...
要在卸载前退出登录，运行 [`difyctl auth logout`](/zh/3.14.x/develop/cli/reference/auth-and-contexts#退出登录)。

## 后续步骤

装好 `difyctl` 后，选择与你登录 `dify.example.com` 的方式相匹配的路径：

<CardGroup cols={2}>
  <Card title="账户用户" icon="user" href="/zh/3.14.x/develop/cli/account-users/quick-start">
    登录你的 Dify 账户，可使用邮箱和密码，或所在组织的 SSO。
  </Card>
  <Card title="外部 SSO 用户" icon="id-card" href="/zh/3.14.x/develop/cli/sso-users/quick-start">
    没有 Dify 账户：通过外部 SSO 登录，运行管理员授权给你的特定应用。
  </Card>
</CardGroup>
