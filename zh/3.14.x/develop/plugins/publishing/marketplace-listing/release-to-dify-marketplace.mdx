...
如果你还没有构建过插件，先从 [Tool 插件实战教程](/zh/3.14.x/develop/plugins/dev-guides-and-walkthroughs/tool-plugin) 开始。
...
<Tabs>
  <Tab title="项目文件">
    ...
    manifest 字段详见 [通用规范](/zh/3.14.x/develop/plugins/features-and-specs/plugin-types/general-specifications)，隐私政策详见 [隐私保护指南](/zh/3.14.x/develop/plugins/publishing/standards/privacy-protection-guidelines)。
  </Tab>
  ...
  <Tab title="语言">
    ...
    - **`README.md`** 不得包含任何中文字符。改为以 `readme/README_<lang>.md` 的形式添加翻译。详见 [多语言 README](/zh/3.14.x/develop/plugins/features-and-specs/plugin-types/multilingual-readme)。
  </Tab>
</Tabs>
...
<Steps>
  <Step title="阅读插件开发指南">
    浏览 [插件开发指南](/zh/3.14.x/develop/plugins/publishing/standards/contributor-covenant-code-of-conduct)。审核者依据它来判断非机械性的方面：独特性、品牌契合度、内容质量、知识产权，以及维护承诺。
  </Step>
  <Step title="编写隐私政策">
    在插件根目录创建 `PRIVACY.md`（或将其托管在别处，并把 URL 写入 manifest）。遵循 [隐私保护指南](/zh/3.14.x/develop/plugins/publishing/standards/privacy-protection-guidelines)：声明插件本身及其调用的任何第三方服务会收集哪些数据。
  </Step>
  <Step title="打包插件">
    在插件项目所在目录的上一级运行：
...
- **版本管理。** 在 `manifest.yaml` 中提升 `version`，重新打包，再带上新的 `.difypkg` 提交一个新 PR。[自动发布 PR 工作流](/zh/3.14.x/develop/plugins/publishing/marketplace-listing/plugin-auto-publish-pr) 可通过 GitHub Action 自动完成这一过程。
...
<CardGroup cols={2}>
  <Card title="发布方式总览" icon="signs-post" href="/zh/3.14.x/develop/plugins/publishing/marketplace-listing/release-overview">
    对比市场、GitHub 和本地文件三种分发方式。
  </Card>
  <Card title="插件开发指南" icon="clipboard-check" href="/zh/3.14.x/develop/plugins/publishing/standards/contributor-covenant-code-of-conduct">
    审核者所遵循的完整内容与质量标准。
  </Card>
  <Card title="隐私保护指南" icon="shield-halved" href="/zh/3.14.x/develop/plugins/publishing/standards/privacy-protection-guidelines">
    如何编写一份能通过审核的 `PRIVACY.md`。
  </Card>
  <Card title="自动发布 PR 工作流" icon="robot" href="/zh/3.14.x/develop/plugins/publishing/marketplace-listing/plugin-auto-publish-pr">
    在每次推送时自动打包并提交 PR 的 GitHub Action。
  </Card>
  <Card title="多语言 README" icon="language" href="/zh/3.14.x/develop/plugins/features-and-specs/plugin-types/multilingual-readme">
    为非英文用户添加 `readme/README_<lang>.md` 文件。
  </Card>
  <Card title="通用规范" icon="file-code" href="/zh/3.14.x/develop/plugins/features-and-specs/plugin-types/general-specifications">
    manifest 字段参考。
  </Card>
</CardGroup>
