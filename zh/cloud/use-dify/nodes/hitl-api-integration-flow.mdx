...
<Steps>
  ...
  <Step title="继续监听工作流">
    若原 SSE 流已关闭，使用第 1 步中的 `workflow_run_id`，并携带启动运行时的同一个 `user`（`user` 不一致会返回 404），调用 [Workflow](/api-reference/工作流/流式获取工作流事件) 或 [Chatflow](/api-reference/对话流/流式获取工作流事件) 的流式获取工作流事件接口重新连接。
...
- **先上传，再使用 `upload_file_id`**（推荐）
  ...
  [上传文件](/api-reference/文件操作/上传文件) 在上传阶段即校验文件大小限制，接收人可立即看到错误并在提交前重试。
