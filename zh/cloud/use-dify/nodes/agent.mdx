...
<Tabs>
  ...
  **记忆管理** 根据你的用例要求平衡上下文保留与标记效率。
  ...
  <Info>
  新 Agent 节点目前处于 Beta 阶段，仅在 Workflow 应用中可用。
  </Info>

  新 Agent 节点将一个 [Agent](/zh/cloud/use-dify/build/new-agent/overview) 作为工作流中的一步来运行。与经典节点「模型加工具」的组合不同，这里请来的是一名自带能力和沙箱的完整员工。
  ...
  <Tabs>
    ...
    <Tab title="从空白开始">
      创建一个只属于此节点的一次性 Agent，直接在这里配置，方法与在 **Agents** 页面 [构建](/zh/cloud/use-dify/build/new-agent/build) 相同。
