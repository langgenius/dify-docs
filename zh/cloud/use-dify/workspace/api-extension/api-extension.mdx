...
> 本文档由 AI 自动翻译。如有任何不准确之处，请参考 [英文原版](/en/cloud/use-dify/workspace/api-extension/api-extension)。

开发者可通过自定义端点扩展模块能力，当前支持以下模块扩展：
...
| `Authorization` | Bearer \{api_key\} | API Key 作为 Bearer 令牌传输，你需要解析该 `api_key` 并确认是否和提供的 API Key 一致，保证接口安全。 |
...
在 Dify 配置自定义端点时，Dify 将会发送一个请求至 API Endpoint，以检验 API 的可用性。
...
由于 Dify 无法访问内网 API 服务，为了方便本地调试 API 服务，可以使用 [Ngrok](https://ngrok.com) 将 API 服务的端点暴露到公网，实现云端调试本地代码。操作步骤：

1.  进入 [https://ngrok.com](https://ngrok.com) 官网，注册并下载 Ngrok 文件。

    <Frame>
      ![下载](https://assets-docs.dify.ai/dify-enterprise-mintlify/zh_CN/guides/extension/api-based-extension/c44d6cc5425508daac8d31bc4af113df.png)
    </Frame>
...
此 API 端点即可公网访问。至此，我们即可在 Dify 配置该 API 端点进行本地调试代码，配置步骤请参考 [外部数据工具](/zh/cloud/use-dify/workspace/api-extension/external-data-tool-api-extension)。

## 使用 Cloudflare Workers 部署自定义端点

我们推荐你使用 Cloudflare Workers 来部署你的自定义端点，因为 Cloudflare Workers 可以方便的提供一个公网地址，而且可以免费使用。

详细说明请参考 [使用 Cloudflare Workers 部署自定义端点](/zh/cloud/use-dify/workspace/api-extension/cloudflare-worker)。
