---
title: 多模态工具
description: 配置工具插件以输出图片、音频或视频，使知识库节点能够将多模态输出与文本一同向量化
---

> 本文档由 AI 自动翻译。如有任何不准确之处，请参考 [英文原版](/en/develop-plugin/dev-guides-and-walkthroughs/develop-multimodal-data-processing-tool)。

在知识流水线中，知识库节点支持两种多模态数据格式的入参：`multimodal-Parent-Child` 和 `multimodal-General`。

为使知识库节点能够识别并向量化工具插件输出的多模态数据（如文字、图片、音频或视频），需完成两项配置：
...
处理图片等多模态数据时，需先通过 Dify 的工具会话上传文件，以获取文件元数据。

以下示例取自 Dify 官方插件 **Dify Extractor** ，展示如何上传文件并构造 `files` 对象。
...
上传接口会返回一个 `UploadFileResponse` 对象，包含文件信息：
...
Dify 官方的 JSON Schema 定义了多模态数据的结构。

为使知识库节点识别插件的多模态输出类型，需在插件的提供者 YAML 文件中将 `output_schema` 下的 `result` 字段指向对应的官方 Schema URL。
...
例如，使用 `multimodal-Parent-Child` 的完整 YAML 配置如下：
