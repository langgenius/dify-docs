...
language: zh
title: Endpoint 插件
description: 以 Neko Cat 项目为例，定义、配置并实现 Dify 插件中的 HTTP Endpoint
---

> 本文档由 AI 自动翻译。如有任何不准确之处，请参考 [英文原版](/en/develop-plugin/dev-guides-and-walkthroughs/endpoint)。

Endpoint 是插件对外暴露的 HTTP 接口，用于与外部系统集成。本文以 [Neko Cat](https://github.com/langgenius/dify-plugin-sdks/tree/739b16ebd300e0b0c5e8c977f70d9c43094b1af7/python/examples/neko) 项目为例，讲解其结构。

<Info>
该示例已从 SDK 仓库移除，链接指向最后包含它的版本。持续维护的插件示例参考 [官方插件示例](https://github.com/langgenius/dify-official-plugins)。
</Info>
...
<Frame>
  ![Endpoint 分组配置表单](https://assets-docs.dify.ai/2024/11/763dbf86e4319591415dc5a1b6948ccb.png)
</Frame>

除了 **Endpoint Name** 之外，可通过编写分组的配置来添加表单项。保存后，即可看到该分组包含的多个接口，它们共用同一份配置。

<Frame>
  ![Endpoint 分组接口列表](https://assets-docs.dify.ai/2024/11/b778b7093b7df0dc80a476c65ddcbe58.png)
</Frame>
...
如果你刚接触插件开发，可先阅读 [插件开发入门](/zh/develop-plugin/dev-guides-and-walkthroughs/tool-plugin) 和 [开发者速查表](/zh/develop-plugin/dev-guides-and-walkthroughs/cheatsheet)。

## 相关资源

- [插件开发基本概念](/zh/develop-plugin/getting-started/getting-started-dify-plugin)：插件开发的整体架构。
- [Neko Cat 示例](/zh/develop-plugin/dev-guides-and-walkthroughs/endpoint)：扩展插件开发示例。
- [通用规范定义](/zh/develop-plugin/features-and-specs/plugin-types/general-specifications)：ProviderConfig 等通用结构。
...
- [反向调用 Dify 服务](/zh/develop-plugin/features-and-specs/advanced-development/reverse-invocation-app)：使用反向调用功能。
