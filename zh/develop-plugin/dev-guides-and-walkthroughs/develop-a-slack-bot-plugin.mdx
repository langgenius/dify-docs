...
深入了解如何构建一个由 AI 驱动的 Slack Bot——它可以直接在 Slack 中回答用户问题。如果你之前没有开发过插件，我们建议先阅读[插件开发快速入门指南](/plugin-dev-en/0211-getting-started-dify-tool)。
...
- **Dify 插件开发工具**：更多信息，请参阅[初始化开发工具](/plugin-dev-en/0221-initialize-development-tools)。
...
现在我们将深入实际编码。在开始之前，请确保你已阅读[快速入门：开发扩展插件](/plugin-dev-en/9231-extension-plugin)或已经构建过 Dify 插件。
...
有关在插件中反向调用 Dify 服务的更多详细信息，请参阅[反向调用：App](/plugin-dev-en/9242-reverse-invocation-app)。
...
确认插件工作正常后，你可以通过以下命令打包并命名它。运行后，你会在当前目录中找到一个 `slack_bot.difypkg` 文件——这就是你的最终插件包。有关详细的打包步骤，请参阅[打包为本地文件并分享](/plugin-dev-en/0322-release-by-file)。
...
你现在可以将其上传到 [Dify Marketplace 仓库](https://github.com/langgenius/dify-plugins) 进行公开发布。在发布之前，请确保你的插件符合[发布到 Dify Marketplace 指南](/plugin-dev-en/0322-release-to-dify-marketplace)。一旦获得批准，你的代码将合并到主分支，插件将在 [Dify Marketplace](https://marketplace.dify.ai/) 上线。

---

## 相关资源

- [插件开发基础](/plugin-dev-en/0111-getting-started-dify-plugin) - Dify 插件开发的全面概述
- [插件开发快速入门指南](/plugin-dev-en/0211-getting-started-dify-tool) - 从零开始开发插件
- [开发扩展插件](/plugin-dev-en/9231-extension-plugin) - 了解扩展插件开发
- [反向调用 Dify 服务](/plugin-dev-en/9241-reverse-invocation) - 了解如何调用 Dify 平台功能
- [反向调用：App](/plugin-dev-en/9242-reverse-invocation-app) - 了解如何调用平台内的应用
- [发布插件](/plugin-dev-en/0321-release-overview) - 了解发布流程
- [发布到 Dify Marketplace](/plugin-dev-en/0322-release-to-dify-marketplace) - Marketplace 发布指南
- [端点详细定义](/plugin-dev-en/0432-endpoint) - 端点详细定义
...
**快速入门：**
- [开发扩展插件](/plugin-dev-en/9231-extension-plugin)
- [开发模型插件](/plugin-dev-en/0211-getting-started-new-model)
- [Bundle 插件：打包多个插件](/plugin-dev-en/9241-bundle)

**插件接口文档：**
- [通过 Manifest 文件定义插件信息](/plugin-dev-en/0411-plugin-info-by-manifest) - Manifest 结构
- [端点](/plugin-dev-en/0432-endpoint) - 端点详细定义
- [反向调用](/plugin-dev-en/9241-reverse-invocation) - 反向调用 Dify 功能
- [通用规范](/plugin-dev-en/0411-general-specifications) - 工具规范
- [模型 Schema](/plugin-dev-en/0412-model-schema) - 模型
