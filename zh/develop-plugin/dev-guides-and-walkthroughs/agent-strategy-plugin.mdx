...
**智能体策略插件**帮助 LLM 执行推理或决策等任务，包括选择和调用工具以及处理结果。这使系统能够更自主地解决问题。
...
初始化完成后，你将获得一个包含插件开发所需全部资源的文件夹。熟悉智能体策略插件的整体结构将简化开发过程：
...
在智能体策略插件中，**调用模型**是工作流的核心。你可以使用 SDK 中的 `session.model.llm.invoke()` 高效地调用 LLM，处理文本生成、对话等任务。
...
此代码实现以下功能：用户输入命令后，智能体策略插件自动调用 LLM，根据生成的结果构造工具调用所需的参数，并使模型能够灵活调度集成的工具以高效完成复杂任务。

![生成工具的请求参数](https://assets-docs.dify.ai/2025/01/01e32c2d77150213c7c929b3cceb4dae.png)

### 4. 处理工具

指定工具参数后，智能体策略插件必须实际调用这些工具。使用 `session.tool.invoke()` 来发起这些请求。
...
完成这些设置后，你的智能体策略插件可以自动执行 **Function Calling**——例如，获取当前时间。

![工具调用](https://assets-docs.dify.ai/2025/01/80e5de8acc2b0ed00524e490fd611ff5.png)

### 5. 创建日志

在**智能体策略插件**中，通常需要多个步骤才能完成复杂任务。对于开发者来说，跟踪每个步骤的结果、分析决策过程和优化策略至关重要。使用 SDK 中的 `create_log_message` 和 `finish_log_message`，你可以在调用前后记录实时状态，有助于快速诊断问题。
...
以下代码展示了如何为智能体策略插件实现模型调用并向工具发送规范化请求。
