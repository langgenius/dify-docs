...
- [步骤 2：知识库管道编排](/en/guides/knowledge-base/knowledge-pipeline/knowledge-pipeline-orchestration)
- [Dify 插件开发：Hello World 指南](/en/develop-plugin/dev-guides-and-walkthroughs/tool-plugin)
...
<Info>
  要了解如何通过继承父类来实现插件功能，请参阅 [Dify 插件开发：Hello World 指南 - 4.4 实现工具逻辑](/en/develop-plugin/dev-guides-and-walkthroughs/tool-plugin#4-4-implementing-tool-logic)。
</Info>
...
<Info>
  有关创建提供者 YAML 文件的更多信息，请参阅 [Dify 插件开发：Hello World 指南 - 4.3 配置提供者凭证](/en/develop-plugin/dev-guides-and-walkthroughs/tool-plugin#4-3-configuring-provider-credentials)。
</Info>

<Info>
  数据源插件支持通过 OAuth 2.0 或 API Key 进行认证。

  要配置 OAuth，请参阅[为你的工具插件添加 OAuth 支持](/en/develop-plugin/dev-guides-and-walkthroughs/tool-oauth)。
</Info>
...
<Info>
  有关插件发布流程，请参阅[发布插件](/en/develop-plugin/publishing/marketplace-listing/release-overview)。
</Info>
