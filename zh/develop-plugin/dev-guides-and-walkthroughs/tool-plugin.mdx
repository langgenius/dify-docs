...
关于如何准备插件开发脚手架工具的详细说明，请参考[初始化开发工具](/zh/develop-plugin/getting-started/cli)。
...
-   `parameters` 参数列表
    ...
    -   `form`（必填）表单类型，目前支持 `llm`、`form` 两种类型。
        -   在 Agent 应用中，`llm` 表示该参数由 LLM 自行推断，`form` 表示参数可以预先设置以使用此工具。
        -   在工作流应用中，`llm` 和 `form` 都需要由前端填写，但 `llm` 参数将用作工具节点的输入变量。
    -   `required` 是否必填
        -   在 `llm` 模式下，如果参数是必填的，Agent 将被要求推断此参数。
