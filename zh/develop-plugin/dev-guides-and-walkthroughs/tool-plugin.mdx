...
关于如何准备插件开发脚手架工具的详细说明，请参考[初始化开发工具](/plugin-dev-en/0221-initialize-development-tools)。如果你是首次开发插件，建议先阅读 [Dify 插件开发：Hello World 指南](/plugin-dev-en/0211-getting-started-dify-tool)。
...
> 如果你已经熟悉插件开发且不需要依赖模板，可以参考[通用规范](/plugin-dev-en/0411-general-specifications)指南来完成不同类型插件的开发。
...
-   `credentials_for_provider` 的子级结构需要满足[通用规范](/plugin-dev-en/0411-general-specifications)的要求。
...
此示例表示请求 `serpapi` 并使用 `self.create_json_message` 返回格式化的 `json` 数据字符串。如果你想了解更多关于返回数据类型的信息，可以参考[远程调试插件](/plugin-dev-en/0411-remote-debug-a-plugin)和[持久化存储 KV](/plugin-dev-en/0411-persistent-storage-kv) 文档。
...
如果你想将插件发布到 Dify Marketplace，请确保你的插件符合[发布到 Dify Marketplace](/plugin-dev-en/0322-release-to-dify-marketplace) 中的规范。通过审核后，代码将被合并到主分支并自动上架到 [Dify Marketplace](https://marketplace.dify.ai/)。

[发布概述](/plugin-dev-en/0321-release-overview)
...
-   [通用规范](/plugin-dev-en/0411-general-specifications) - Manifest 结构和工具规范
-   [Endpoint](/plugin-dev-en/0432-endpoint) - 详细的 Endpoint 定义
-   [反向调用](/plugin-dev-en/9241-reverse-invocation) - 反向调用 Dify 能力
-   [模型 Schema](/plugin-dev-en/0412-model-schema) - 模型
-   [智能体插件](/plugin-dev-en/9232-agent) - 扩展智能体策略

## 下一步学习

- [远程调试插件](/plugin-dev-en/0411-remote-debug-a-plugin) - 学习更多高级调试技巧
- [持久化存储](/plugin-dev-en/0411-persistent-storage-kv) - 学习如何在插件中使用数据存储
- [Slack Bot 插件开发示例](/plugin-dev-en/0432-develop-a-slack-bot-plugin) - 查看更复杂的插件开发案例
- [工具插件](/plugin-dev-en/0411-tool) - 探索工具插件的高级功能
