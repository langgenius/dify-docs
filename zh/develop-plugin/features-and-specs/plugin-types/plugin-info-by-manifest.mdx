...
Manifest 是一个符合 YAML 规范的文件，定义了**插件**最基本的信息，包括但不限于插件名称、作者、包含的工具、模型等。关于插件的整体架构，请参阅[插件开发基本概念](/plugin-dev-en/0111-getting-started-dify-plugin)和[开发者速查表](/plugin-dev-en/0131-cheatsheet)。
...
<ParamField path="plugins" type="object" required>
  ...
  <ParamField path="tools" type="array[string]">
    [工具](/plugin-dev-en/0222-tool-plugin) provider 的插件扩展。
  </ParamField>
  
  <ParamField path="models" type="array[string]">
    [模型](/plugin-dev-en/0411-model-plugin-introduction) provider 的插件扩展。
  </ParamField>
  
  <ParamField path="endpoints" type="array[string]">
    [Endpoints](/plugin-dev-en/0432-develop-a-slack-bot-plugin) provider 的插件扩展。
  </ParamField>
  
  <ParamField path="agent_strategies" type="array[string]">
    [智能体策略](/plugin-dev-en/9433-agent-strategy-plugin) provider 的插件扩展。
  </ParamField>
</ParamField>
...
<ParamField path="privacy" type="string">
  指定插件隐私政策文件的相对路径或 URL，例如 `"./privacy.md"` 或 `"https://your-web/privacy"`。如果你计划在 Dify Marketplace 上架插件，**此字段为必填**，以提供清晰的用户数据使用和隐私声明。详细填写指南请参阅[插件隐私数据保护指南](/plugin-dev-en/0312-privacy-protection-guidelines)。
</ParamField>
...
[编辑此页面](https://github.com/langgenius/dify-docs/edit/main/plugin-dev-en/0411-plugin-info-by-manifest.mdx) | [报告问题](https://github.com/langgenius/dify-docs/issues/new?template=docs.yml)