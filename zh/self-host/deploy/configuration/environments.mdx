...
Web 应用前端（已发布应用）的后端 API URL。此变量仅由 Web 前端容器使用，不影响 Python 后端。如果为空，Docker 镜像默认使用 `http://127.0.0.1:5001`。
...
已发布 Web 应用的公开访问 URL。工作流中的 **人工输入节点** 需要此变量；邮件通知中的表单链接以 `{APP_WEB_URL}/form/{token}` 格式构建。如果为空，人工输入邮件将不包含有效的表单链接。
...
如果协作者编辑大型工作流时编辑器反复断开重连，可调高 `WEBSOCKET_MAX_HTTP_BUFFER_SIZE`。
...
| `DEPLOYMENT_EDITION` | `COMMUNITY` | 部署运行的版本：`COMMUNITY`、`ENTERPRISE` 或 `CLOUD`。保持默认值即可。<br/><br/>取代原有的 `EDITION`、`ENTERPRISE_ENABLED` 和 `BILLING_ENABLED`，这些旧变量自 1.17.0 起会被忽略。 |
...
决定 Dify 把加密存储凭证（模型供应商 API 密钥、工具凭证等工作空间机密）所用的密钥保存在哪里：

- `local`：为每个工作空间生成一对 RSA 密钥，私钥保存在 `STORAGE_TYPE` 对应的存储后端中。

- `azure-keyvault`：将每个工作空间的 RSA 密钥保存在 Azure Key Vault 中，私钥不会离开 Key Vault。Dify 通过 `DefaultAzureCredential`（托管身份、环境变量或 Azure CLI 登录）完成认证。

<Warning>
在创建第一个工作空间前确定密钥提供方。两种提供方加密的凭证互不兼容，在已存有凭证的部署上切换会导致这些凭证无法解密，只能重新录入。
</Warning>

| 变量 | 默认值 | 说明 |
|---|---|---|
| `AZURE_KEYVAULT_VAULT_URL` | （空） | Key Vault 实例 URL。`KEY_PROVIDER_TYPE` 为 `azure-keyvault` 时必填，缺失会导致 API 服务无法启动。示例：`https://your-vault-name.vault.azure.net` |
| `AZURE_KEYVAULT_KEY_SIZE` | `2048` | Dify 在 Key Vault 中为工作空间新建密钥时使用的 RSA 密钥长度（位）。仅在创建新密钥时生效，已有密钥保持原长度。 |
| `AZURE_KEYVAULT_ROTATION_INTERVAL_DAYS` | （空） | 设置后（最小值 7），新建的工作空间密钥会附带 Key Vault 自动轮换策略，每 N 天轮换一次。旧版本密钥保持可用，轮换前加密的凭证无需重新加密即可继续解密。留空则自行在 Azure 中管理轮换。 |
...
| `APP_MAX_EXECUTION_TIME` | `3600` | 应用执行的最大运行时间（秒），超时后终止。属于外层限制，与 `WORKFLOW_MAX_EXECUTION_TIME` 配合使用：两者默认均为一小时，此变量作用于应用队列层面，另一个作用于工作流引擎层面。如果工作流需要更多时间，请同时增大两者。<br/><br/>如果你使用 Agent，请让这两个外层限制不低于 Agent 的运行时限（`DIFY_AGENT_RUN_TIMEOUT_SECONDS`），否则 Agent 运行会被提前截断。 |
...
| `WEBSOCKET_MAX_HTTP_BUFFER_SIZE` | `10485760` | 协作 WebSocket 接受的单条消息大小上限，单位为字节（默认 10 MiB）。保存大型工作流时，如果工作流图超过此上限，导致连接反复断开，可调高此值。<br/><br/>在处理 `/socket.io/` 的 API 进程上设置，Docker Compose 部署中对应 `api_websocket` 容器。 |
...
| `HUAWEI_OBS_ACCESS_KEY` | （空） | OBS access key。仅在 `HUAWEI_OBS_USE_OIDC` 为 `false` 时必填。 |
| `HUAWEI_OBS_SECRET_KEY` | （空） | OBS secret key。仅在 `HUAWEI_OBS_USE_OIDC` 为 `false` 时必填。 |
...
| `HUAWEI_OBS_USE_OIDC` | `false` | 使用 OBS SDK 内置的 OIDC 凭证提供者，代替静态 AK/SK 凭证。需要配置华为云 IAM 身份提供商和 Kubernetes/CCE service account token。 |
| `HUAWEI_CLOUD_IDP_ID` | （空） | 华为云 IAM 身份提供商 ID。`HUAWEI_OBS_USE_OIDC` 为 `true` 时必填。 |
| `OIDC_TOKEN_FILE` | `/var/run/secrets/tokens/oidc-token` | OBS SDK 读取的 Kubernetes/CCE projected service account token 文件。仅在 token 路径不是默认值时设置。 |
...
用于归档工作流运行日志的独立 S3 兼容存储。`archive-workflow-runs` 维护命令将超过指定期限的工作流运行打包为按月的 Parquet 归档包；由归档生成的 CSV 导出文件暂存于导出存储桶。
...
| `ARCHIVE_STORAGE_EXPORT_BUCKET` | （空） | 存放由归档日志生成的临时 CSV 导出压缩包，可通过存储桶生命周期规则自动清理。 |
...
| `TIDB_VECTOR_ENABLE_FULLTEXT_SEARCH` | `false` | 为 TiDB Vector 上的知识库解锁全文检索和混合检索两种检索方式。<br/><br/>新建的知识库表会带有多语言全文索引；已有表会在下次索引文档时补建该索引。<br/><br/>要求 TiDB 部署支持全文索引。 |
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `ENABLE_LEARN_APP` | `true` | 显示 Learn Dify 版块：探索页的教程模板、工作室中还没有应用时的入门面板，以及帮助菜单中的显示开关。<br/><br/>设为 `false` 可同时隐藏三处。 |
| `ENABLE_SKILL` | `true` | 在主导航中显示 [Skills](/zh/self-host/use-dify/build/skills) 入口。<br/><br/>设为 `false` 后，工作区界面中不再显示 Skills。 |

#### 认证与账户

默认情况下，用户以邮箱和密码登录，新账户通过工作空间邀请或初始设置向导创建。以下开关可放宽或收紧这一策略。

| 变量 | 默认值 | 说明 |
|---|---|---|
| `ENABLE_EMAIL_PASSWORD_LOGIN` | `true` | 使用邮箱和密码登录控制台。关闭后，登录页不再显示密码表单，密码登录、密码重置和自助注册接口均返回 403。至少保留一种登录方式：若全部关闭，登录页会提示未配置认证方式，在改回设置前无人能登录。 |
| `ENABLE_EMAIL_CODE_LOGIN` | `false` | 使用邮件发送的 6 位一次性验证码登录，无需密码。需要先配置邮件发送（参见 [邮件配置](#邮件配置)），否则验证码无法送达。当 `ALLOW_REGISTER=true` 时，未注册邮箱输入验证码即可同时创建账户，相当于一条自助注册途径。 |
| `EMAIL_CODE_LOGIN_TOKEN_EXPIRY_MINUTES` | `5` | 登录验证码的有效时长（分钟）。过期的验证码会被拒绝，需重新获取。 |
| `EMAIL_CODE_LOGIN_MAX_ATTEMPTS` | `5` | 单个登录验证码允许的最大输错次数，超过后验证码失效，需重新获取。 |
| `ENABLE_SOCIAL_OAUTH_LOGIN` | `false` | 在控制台登录页显示 GitHub 和 Google 登录按钮。按钮需在 API 服务上配置相应的 GitHub 或 Google OAuth 应用凭据后才可用；只开启开关而未配置凭据时，登录会失败。首次 OAuth 登录仅在 `ALLOW_REGISTER=true` 时创建账户。 |
| `ALLOW_REGISTER` | `false` | 允许自助创建账户：在登录页显示注册链接（需同时启用密码登录），并允许首次邮箱验证码或 OAuth 登录时创建账户。保持 `false` 即为仅限邀请的部署，工作空间邀请和初始设置向导始终不受影响。 |
| `ALLOW_CREATE_WORKSPACE` | `false` | 为不属于任何工作空间的账户自动创建工作空间，作用于自助注册及邮箱验证码或 OAuth 登录。设为 `false` 时，这类账户需等待工作空间邀请。设置向导创建的管理员工作空间不受影响。 |
| `ENABLE_CHANGE_EMAIL` | `true` | 允许用户在账户设置页修改账户邮箱。修改流程通过邮件验证码确认，因此需要配置邮件发送。设为 `false` 可锁定账户邮箱，例如需要与外部目录保持一致时。 |
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `OPS_TRACE_RETRYABLE_DISPATCH_MAX_RETRIES` | `780` | 追踪投递遇到瞬时失败后的最大重试次数，超出后该条追踪数据被丢弃。<br/><br/>默认重试窗口（次数 × `OPS_TRACE_RETRYABLE_DISPATCH_DELAY_SECONDS`）按覆盖一小时工作流上限加导出宽限期设定。<br/><br/>目前只有 Arize/Phoenix 追踪嵌套工作流时会触发（子追踪先于父级上下文到达）。不能设为 `0`。 |
| `OPS_TRACE_RETRYABLE_DISPATCH_DELAY_SECONDS` | `5` | 两次重试的间隔秒数；总容忍窗口约为间隔 × 最大次数（默认约 65 分钟）。<br/><br/>调低可让嵌套追踪更快送达，但重试次数消耗更快。不能设为 `0`。 |
| `OPS_TRACE_PARENT_CONTEXT_TTL_SECONDS` | `3900` | 父工作流追踪上下文的保留时长（秒），供后到的嵌套子追踪挂接。<br/><br/>取值应不低于重试窗口（次数 × 间隔）。 |
...
| `WORKFLOW_MAX_EXECUTION_TIME` | `3600` | 每次工作流运行的最大实际运行时间（秒）。超过此值将终止工作流。<br/><br/>与 `APP_MAX_EXECUTION_TIME` 配对的外层限制（两者默认均为一小时）：请同时增大两者。如果你使用 Agent，还需保持不低于 Agent 的运行时限（`DIFY_AGENT_RUN_TIMEOUT_SECONDS`）。 |
...
| `GRAPH_ENGINE_SCALE_UP_THRESHOLD` | `0` | 允许多少个任务等待空闲工作线程，超过即新增工作线程，上限为 `GRAPH_ENGINE_MAX_WORKERS`。<br/><br/>默认值 `0` 表示一旦有任务等待就立即新增；调大可先容忍短暂积压再扩容。 |
...
| `MARKDOWN_FORM_FIELD_NAME_EXTRA_CHARS` | （空） | 应用在聊天回复中通过 Markdown 渲染表单时，表单字段 `name` 属性额外接受的字符。<br/><br/>默认情况下，字段名必须以字母开头，可包含字母、数字、下划线和连字符；名称含其他字符的字段不会被渲染。<br/><br/>将额外字符连成一个字符串填入，半角和全角字符均可。示例：`()！？` |
| `MARKDOWN_FORM_FIELD_NAME_MAX_LENGTH` | `128` | 应用在聊天回复中通过 Markdown 渲染表单时，表单字段 `name` 属性的最大长度，名称超长的字段不会被渲染。与 `MARKDOWN_FORM_FIELD_NAME_EXTRA_CHARS` 配合使用，后者决定字段名可包含哪些字符。<br/><br/>如果生成的字段名超过上限，导致渲染出的表单缺少字段，可调高此值。必须为正整数。 |
...
新 Agent 在 Docker Compose 部署中以额外服务运行：一个 Agent 后端、一个沙箱，以及供沙箱使用的专用出站代理，它们会随 `docker compose up` 自动启动。该功能默认启用，且大部分设置会从现有的 Compose 配置中自动推导，因此通常只需关注下面这几个变量。
...
默认值：（`.env.example` 中预填了开发令牌，生产环境必须替换）

Dify API 调用 Agent 后端运行接口时出示的 Bearer 令牌。Compose 会把同一个值传给两侧服务，设置一次即可保持一致。

默认值是一个公开的开发令牌，部署可开箱即用；但任何能访问 Agent 后端且知道该令牌的人都能发起运行，生产环境使用前务必替换为你自己的随机值。
...
| `DIFY_AGENT_RUN_RETENTION_SECONDS` | `7200` | 每次 Agent 运行的记录和事件流在 Redis 中保留的秒数，从最后一次写入算起（默认 2 小时）。调低可更早释放 Redis 内存，但连接中断后可回放事件的时间也会缩短。 |
| `DIFY_AGENT_RUN_EVENT_STREAM_MAX_LENGTH` | `5000` | 每次 Agent 运行的 Redis 事件流保留的事件数上限（近似值）。调低可减少单次运行占用的 Redis 内存。<br/><br/>达到上限后，写入新事件会删除最旧的事件。连接中断后，已删除的事件无法回放。 |
| `DIFY_AGENT_STREAM_TEXT_DELTA_COALESCING_ENABLED` | `true` | 将模型连续输出的文本增量合并成批，再写入 Redis 并流式发送。保持启用可减少 Redis 写入次数和事件数量。<br/><br/>设为 `false` 可单独发送每个增量。工具调用等非文本事件的内容和顺序不变。 |
| `DIFY_AGENT_STREAM_TEXT_DELTA_FLUSH_INTERVAL_MS` | `100` | 合并后的文本增量按此间隔发布，单位为毫秒。调低可更频繁地更新文本，调高则每批文本更多、批次数更少。<br/><br/>此间隔并非严格的时间限制。达到 `DIFY_AGENT_STREAM_TEXT_DELTA_MAX_CHARS` 时可提前发布。最小值为 1。要关闭合并，将 `DIFY_AGENT_STREAM_TEXT_DELTA_COALESCING_ENABLED` 设为 `false`。 |
| `DIFY_AGENT_STREAM_TEXT_DELTA_MAX_CHARS` | `4096` | 一批文本增量达到此字符数后，无需等待刷新间隔即可发布。调低可让较小的批次更早发布。<br/><br/>每个增量都会完整加入批次，因此一批的字符数可能超过此阈值。最小值为 1。 |
| `DIFY_AGENT_RUN_TIMEOUT_SECONDS` | `3600` | 单次 Agent 运行中模型与工具工作的总时限，超时的运行以 `agent_run_limit_exceeded` 失败。取代原 `AGENT_BACKEND_RUN_TIMEOUT_SECONDS`。<br/><br/>API 侧的 `APP_MAX_EXECUTION_TIME` 与 `WORKFLOW_MAX_EXECUTION_TIME`（默认 `3600`）若设得低于此值，会更早终止运行。<br/><br/>在 `e2b` 后端下，沙箱自身的 `DIFY_AGENT_E2B_ACTIVE_TIMEOUT_SECONDS`（上限 3600）仍然生效，因此高于 1 小时的值只在 `local` 后端下有效。 |
...
| `DIFY_AGENT_OUTBOUND_HTTP_CONNECT_TIMEOUT` | `10` | Agent 服务对外 HTTP 调用（如工具与插件流量）的连接超时（秒）。 |
| `DIFY_AGENT_OUTBOUND_HTTP_READ_TIMEOUT` | `600` | 读取超时（秒）；默认较宽松，以便较慢的工具响应和大文件下载能够完成。 |
| `DIFY_AGENT_OUTBOUND_HTTP_WRITE_TIMEOUT` | `30` | 对外请求的写入超时（秒）。 |
| `DIFY_AGENT_OUTBOUND_HTTP_POOL_TIMEOUT` | `10` | 请求等待连接池空闲连接的时长（秒）。 |
| `DIFY_AGENT_OUTBOUND_HTTP_MAX_CONNECTIONS` | `100` | Agent 服务对外 HTTP 的连接池大小。 |
| `DIFY_AGENT_OUTBOUND_HTTP_MAX_KEEPALIVE_CONNECTIONS` | `20` | 保留供复用的空闲连接数。 |
| `DIFY_AGENT_OUTBOUND_HTTP_KEEPALIVE_EXPIRY` | `30` | 空闲连接在关闭前保留的秒数。 |
...
| `DIFY_AGENT_LOCAL_SANDBOX_ENDPOINT` | `http://local_sandbox:5004` | 内置沙箱服务的地址。取代原 `DIFY_AGENT_SHELLCTL_ENTRYPOINT`，旧名仍可作为兼容别名使用。 |
| `DIFY_AGENT_LOCAL_SANDBOX_AUTH_TOKEN` | （空） | 可选的 Bearer 令牌，用于保护 Agent 后端对沙箱的调用；Compose 会把同一值传给两个服务。<br/><br/>取代原 `DIFY_AGENT_SHELLCTL_AUTH_TOKEN`，旧名仍可作为兼容别名使用。 |
| `EXPOSE_LOCAL_SANDBOX_PORT` | `5004` | [源码部署](/zh/self-host/deploy/advanced-deployments/local-source-code) 所用的中间件栈将 Agent 沙箱发布到宿主机的端口，供从源码运行的 Agent 后端通过 `http://localhost:5004` 访问。在该栈的 `middleware.env` 中设置；改动后，需将 Agent 后端 `.env` 中的 `DIFY_AGENT_LOCAL_SANDBOX_ENDPOINT` 指向新端口。<br/><br/>仅适用于中间件栈。完整的 Compose 栈将沙箱保留在内部网络中。 |
| `DIFY_AGENT_STUB_API_BASE_URL` | `http://agent_backend:5050/agent-stub` | 沙箱内 Agent 回调 Agent 后端的地址。 |
| `DIFY_AGENT_SANDBOX_FILES_BASE_URL` | `http://api:5001` | 沙箱内 Agent 进行文件传输时使用的 Dify API 地址。Docker Compose 部署保持默认即可；使用远程沙箱后端时，需填写可公网访问的 Dify 地址。 |
| `AGENT_BACKEND_BINDING_FILE_DOWNLOAD_TIMEOUT_SECONDS` | `240` | API 从 Agent 沙箱取文件时的等待时长，超时则传输失败。应高于 Agent 侧的 `DIFY_AGENT_BINDING_FILE_DOWNLOAD_COMMAND_TIMEOUT_SECONDS`。 |
| `AGENT_BACKEND_HOME_SNAPSHOT_TIMEOUT_SECONDS` | `45` | API 等待 Agent 后端传输 home snapshot（捕获、删除快照，以及启动需要恢复快照的运行）的时长。<br/><br/>如果 home 环境较大的 Agent 在应用构建或启动运行时失败，可调大该值。 |
| `DIFY_AGENT_BINDING_FILE_DOWNLOAD_COMMAND_TIMEOUT_SECONDS` | `210` | 沙箱中负责输出该文件的命令可运行的时长。默认值低于 API 侧超时，以便先呈现 Agent 自身的错误。 |
| `DIFY_AGENT_STUB_UPLOAD_FILE_SIZE_LIMIT` | `50` | 沙箱中的 Agent 向 Dify 回传单个文件的大小上限（MiB），例如它为回复生成的文件。 |
...
| `DIFY_AGENT_SHELL_REDACT_PATTERNS` | （空） | JSON 正则数组，命中的内容会在 Agent 读取命令行输出前被遮盖，用于保护 API 密钥等机密。Agent 的内部令牌始终会被遮盖。 |

### DIFY_AGENT_RUNTIME_BACKEND

默认值：`local`

选择 Agent 沙箱的运行位置。默认的 `local` 使用内置沙箱容器；设为 `e2b` 时，每个 Agent 的沙箱会运行在 [E2B](https://e2b.dev) 云虚拟机中，隔离性比内置容器更强。

`e2b` 后端使用自己的一组变量，`docker/docker-compose.e2b.yaml` overlay 会相应调整 Compose 配置：
...
| `MARKETPLACE_ENABLED` | `true` | 控制 Dify Marketplace 中的插件功能是否启用。禁用后，控制台会隐藏 Marketplace 插件的浏览和安装入口，API 拒绝从 Marketplace 安装和升级插件，周期性升级检查（`ENABLE_CHECK_UPGRADABLE_PLUGIN_TASK`）也会停止。<br/><br/>模板导入操作本身不受影响。但如果导入的模板依赖尚未安装的 Marketplace 插件，禁用 Marketplace 时将无法安装这些依赖。已安装的插件仍可正常使用；安装策略允许时，也仍可从 GitHub 或本地包安装插件。 |
| `MARKETPLACE_API_URL` | `https://marketplace.dify.ai` | 用于浏览和下载插件与模板，以及检查插件升级的 Marketplace API 端点。<br/><br/>Dify API 服务、后台任务进程、Web 服务端和用户浏览器都会使用该 URL，因此每个运行环境都需要能访问它。 |
| `MARKETPLACE_URL` | （空；Compose 部署回退到 `https://marketplace.dify.ai`） | Marketplace 链接使用的基础 URL，包括插件详情页和「集成」页面的 Marketplace 入口。Dify 还会根据该值生成 Creator Center 指南链接。<br/><br/>与 `MARKETPLACE_API_URL` 不同：后者是提供 Marketplace 数据和安装包的 API 端点。 |
...
Creator Center（`creators.dify.ai`）是用户将 Dify 应用以模板形式提交的入口。提交的模板经审核通过后会在 Dify Marketplace 上线。Dify 控制台会在新的浏览器标签页中打开 Creator Center 页面。

控制台中的「发布到 Marketplace」操作会导出应用的 DSL（应用的 YAML 格式定义文件）并上传至 Creator Center，用户随后在该处完成提交。详见 [发布应用到市场](/zh/self-host/use-dify/publish/publish-to-marketplace)。

| 变量 | 默认值 | 说明 |
|---|---|---|
| `CREATORS_PLATFORM_FEATURES_ENABLED` | `true` | 控制应用的「发布」面板中是否显示「发布到 Marketplace」操作。设为 `false` 时该操作被隐藏，对应的控制台 API 也会返回 403。适用于不希望用户拥有一键对外提交模板路径的部署。 |
| `CREATORS_PLATFORM_API_URL` | `https://creators.dify.ai` | Creator Center 服务的基础 URL。「发布到 Marketplace」由 API 服务将应用 DSL 上传至该地址，用户浏览器也会访问同一地址来完成提交，因此两端都需要能访问它。 |
| `CREATORS_PLATFORM_OAUTH_CLIENT_ID` | （空） | 用于免登录 SSO 进入 Creator Center 的 OAuth 客户端 ID。自部署环境请保持为空。留空时仍可发布，但尚未登录的用户需要手动登录 Creator Center。填入未在本部署 OAuth 服务中注册的 ID 会导致「发布到 Marketplace」失败。 |
...
| `ENABLE_CLEAN_MESSAGES` | `false` | 每天凌晨 4:00 删除超过 `SANDBOX_EXPIRED_RECORDS_RETENTION_DAYS` 保留期的对话消息。 |
...
| `ENABLE_CONVERSATION_CLEANUP_TASK` | `true` | 对话删除的兜底清理。对话删除后立即不可见，其消息以及对话中由工具生成的文件由后台任务清除，终端用户上传的文件会保留。<br/><br/>该定时清理会为清理任务丢失的对话重新派发任务，避免数据残留。 |
| `CONVERSATION_CLEANUP_TASK_INTERVAL` | `5` | 清理任务的运行频率（分钟）。 |
| `CONVERSATION_CLEANUP_BATCH_SIZE` | `100` | 每次最多重新派发的对话数。 |

#### 记录保留与清理

这些变量控制旧消息与工作流运行记录的定时清理，超过保留天数的记录会分批删除。
