...
| `MARKETPLACE_ENABLED` | `true` | 控制 Dify Marketplace 中的插件功能是否启用。禁用后，控制台会隐藏 Marketplace 插件的浏览和安装入口，API 拒绝从 Marketplace 安装和升级插件，周期性升级检查（`ENABLE_CHECK_UPGRADABLE_PLUGIN_TASK`）也会停止。<br/><br/>模板导入操作本身不受影响。但如果导入的模板依赖尚未安装的 Marketplace 插件，禁用 Marketplace 时将无法安装这些依赖。已安装的插件仍可正常使用，也仍可从 GitHub 或本地包安装插件。 |
| `MARKETPLACE_API_URL` | `https://marketplace.dify.ai` | 用于浏览和下载插件与模板以及检查插件升级的 Marketplace API 端点。<br/><br/>Dify API 服务、Web 服务端和用户浏览器都会使用该 URL，因此三端都需要能访问它。 |
| `MARKETPLACE_URL` | （空；Compose 部署回退到 `https://marketplace.dify.ai`） | Marketplace 链接使用的基础 URL，包括插件详情页和「集成」页面的 Marketplace 入口。Dify 还会根据该值生成 Creator Center 指南链接。<br/><br/>与 `MARKETPLACE_API_URL` 不同：后者供控制台和插件守护进程获取 Marketplace 数据。 |
...
| `CREATORS_PLATFORM_OAUTH_CLIENT_ID` | （空） | 用于免登录 SSO 进入 Creator Center 的 OAuth 客户端 ID。自部署环境请保持为空。留空时仍可发布，但用户需要手动登录 Creator Center。填入未在本部署 OAuth 服务中注册的 ID 会导致「发布到 Marketplace」失败。 |
