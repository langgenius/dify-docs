...
在 Web 界面中显示新 Agent（Beta）：主导航中出现 **Agent**，工作流节点选择器中提供新 Agent 节点。

关闭时，Agent 页面返回 404，工作流仅提供经典 Agent 节点。开启后仅显示界面，Agent 的运行还需要部署 Agent 运行时服务。

源码部署时改用 `NEXT_PUBLIC_ENABLE_AGENT_V2`。
