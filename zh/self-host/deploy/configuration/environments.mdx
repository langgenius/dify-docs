...
| `MARKETPLACE_ENABLED` | `true` | 启用 Dify Marketplace 作为插件与模板来源。禁用后，控制台会隐藏 Marketplace 浏览和安装入口，API 拒绝来自 Marketplace 的安装与升级，周期性升级检查（`ENABLE_CHECK_UPGRADABLE_PLUGIN_TASK`）也随之停止。<br/><br/>已安装的插件不受影响，仍可从 GitHub 或本地包安装插件。 |
| `MARKETPLACE_API_URL` | `https://marketplace.dify.ai` | 用于浏览、下载插件与模板以及检查升级的 Marketplace API 端点。<br/><br/>API 服务和用户浏览器都会直接调用它（控制台在浏览器端拉取 Marketplace 列表），两侧都需要能访问该地址。 |
| `MARKETPLACE_URL` | （空；Compose 部署回退到 `https://marketplace.dify.ai`） | 跳转到 Marketplace 网站的链接基础 URL（插件详情页和「集成」页面的 Marketplace 入口）。<br/><br/>与 `MARKETPLACE_API_URL` 不同：后者供控制台和插件守护进程获取 Marketplace 数据。 |
...
Creator Center（`creators.dify.ai`）是用户将 Dify 应用以模板形式提交的入口。提交的模板经审核通过后会在 Dify Marketplace 上线。

控制台中的「发布到 Marketplace」操作会导出应用的 DSL（应用的 YAML 格式定义文件）并上传至 Creator Center，再将用户浏览器重定向到该处完成提交。详见 [发布应用到市场](/zh/self-host/use-dify/publish/publish-to-marketplace)。
...
| `CREATORS_PLATFORM_API_URL` | `https://creators.dify.ai` | Creator Center 服务的基础 URL。「发布到 Marketplace」由 API 服务将应用 DSL 上传至该地址，随后将用户浏览器重定向到同一地址完成提交，因此 API 服务和用户浏览器都需要能访问它。 |
| `CREATORS_PLATFORM_OAUTH_CLIENT_ID` | （空） | 供 Creator Center 将模板提交归属到对应 Dify 账户的 OAuth 客户端 ID。保持留空即可：留空不影响发布，而填入未在本部署 OAuth 服务中注册的 ID 会导致「发布到 Marketplace」失败。 |
