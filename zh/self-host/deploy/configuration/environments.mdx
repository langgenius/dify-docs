...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `PLUGIN_BASED_TOKEN_COUNTING_ENABLED` | `false` | 使用基于插件的词元计数以实现精确用量跟踪。禁用时词元计数返回 0（更快但成本跟踪不够精确）。 |
...
| 变量 | 默认值 | 说明 |
|---|---|---|
| `INDEXING_MAX_SEGMENTATION_TOKENS_LENGTH` | `4000` | 知识库文档分块时每个文本段落的最大词元长度。值越大每个块保留的上下文越多；值越小提供更细的粒度。 |
