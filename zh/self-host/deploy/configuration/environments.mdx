...
承载协作流量的专用 `api_websocket` 容器可通过 [容器启动配置](#容器启动配置) 中的 `API_WEBSOCKET_*` 变量调优。
...
| `API_WEBSOCKET_WORKER_AMOUNT` | `1` | 仅适用于 Docker Compose。专用 `api_websocket` 容器（为实时协作提供 WebSocket 服务）的 Gunicorn 工作进程数。协作事件通过 Redis 跨进程同步，因此可放心调高；当单个进程在大量并发编辑下占满一个 CPU 核心时再增加。 |
...
| `API_WEBSOCKET_WORKER_CONNECTIONS` | `1000` | 仅适用于 Docker Compose。每个 `api_websocket` 工作进程可同时接受的 WebSocket 连接数上限。每个打开的工作流编辑器标签页占用一个连接，因此默认值下每个进程最多支持 1000 个并发编辑会话；如有更多需求，可调高此值或增加进程数。 |
...
| `NEXT_PUBLIC_ENABLE_FEATURE_PREVIEW` | `true` | 在控制台中显示开发中的预览功能。当前用于控制命令面板中的 `/create` 与 `/refine` 斜杠命令（AI 辅助生成工作流与对话流）。设为 `false` 可隐藏这些预览功能。 |
