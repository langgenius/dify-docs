...
| `MARKETPLACE_ENABLED` | `true` | 控制 Dify Marketplace 中的插件功能是否启用。禁用后，控制台会隐藏 Marketplace 插件的浏览和安装入口，API 拒绝从 Marketplace 安装和升级插件，周期性升级检查（`ENABLE_CHECK_UPGRADABLE_PLUGIN_TASK`）也会停止。<br/><br/>模板导入不受影响。已安装的插件仍可正常使用，也仍可从 GitHub 或本地包安装插件。 |
| `MARKETPLACE_API_URL` | `https://marketplace.dify.ai` | 用于浏览和下载插件与模板以及检查插件升级的 Marketplace API 端点。<br/><br/>Web 服务端和用户浏览器都会使用该 URL，因此两端都需要能访问它。 |
...
Creator Center（`creators.dify.ai`）是用户将 Dify 应用以模板形式提交的入口。提交的模板经审核通过后会在 Dify Marketplace 上线。Creator Center 相关页面会在新的浏览器标签页中打开。
...
| `CREATORS_PLATFORM_OAUTH_CLIENT_ID` | （空） | 用于免登录 SSO 进入 Creator Center 的 OAuth 客户端 ID。自部署环境请保持为空。留空不影响发布，填入未在本部署 OAuth 服务中注册的 ID 会导致「发布到 Marketplace」失败。 |
