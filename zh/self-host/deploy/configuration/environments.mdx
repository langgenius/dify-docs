...
`/openapi/v1/*` 是面向用户的编程 API，[Dify CLI](/zh/cli/quick-start)（`difyctl`）等客户端通过 OAuth Bearer 令牌访问。该接口默认关闭：将 `OPENAPI_ENABLED` 和 `ENABLE_OAUTH_BEARER` 都设为 `true` 后，CLI 才能连接你的部署。
...
| `OPENAPI_RATE_LIMIT_PER_TOKEN` | `60` | 每个 OAuth Bearer 令牌在 `/openapi/v1/*` 上的每分钟请求上限，经 Redis 在多副本间共享。<br/>超限返回 429 并附 `Retry-After` 头。设为 `0` 可关闭限流。 |
