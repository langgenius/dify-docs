...
> 本指南说明如何从 Weaviate 客户端 v3 迁移到 weaviate-client v4（v4.17.0 或更高版本），并将 Weaviate 服务器从 1.19.0 升级到 1.27.0 或更高版本。此迁移适用于包含 weaviate-client v4 升级的 Dify 版本。

<Info>
  已在 1.27 上并准备升级到 Dify 1.17.1？其内置的 Weaviate 镜像为 `1.39.2`。跨越这段版本差需要按顺序逐个小版本推进。完整升级阶梯详见 [Weaviate 服务器升级路径](/zh/self-host/deploy/troubleshooting/weaviate-server-migration-path)。
</Info>
...
| 1.17.0～1.17.1 | v4.22.0 | 1.27.0 或更高版本 |
...
对于 Docker 安装，确保运行正确的 Dify 版本。在 `docker` 目录下执行：
...
对于源码安装，安装当前代码固定的客户端版本。需在仓库根目录执行，而不是 `docker` 目录：
