...
<Info>
  已在 1.27 上，想升到更新的服务器版本？Dify 内置的 Weaviate 镜像即将升级到 1.39，跨越这段版本差需要按顺序逐个小版本推进。完整阶梯详见 [Weaviate 服务器升级路径](/zh/self-host/deploy/troubleshooting/weaviate-server-migration-path)。
</Info>
...
服务器一列没有上限，因为 Dify 会把客户端和内置服务器版本一起固定。如果让 Dify 连接外部 Weaviate，需保持在 1.27.0 或更高版本，并按 [Weaviate 服务器升级路径](/zh/self-host/deploy/troubleshooting/weaviate-server-migration-path) 逐个小版本升级。
...
对于 Docker 安装，请确保运行正确的 Dify 版本。在 `docker` 目录下执行：
...
对于源码安装，请安装当前代码固定的客户端版本。需在仓库根目录执行，而不是 `docker` 目录：
