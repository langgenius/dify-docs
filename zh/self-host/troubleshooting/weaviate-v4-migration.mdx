...
按照[步骤 A5](#step-a5-migrate-the-schema)中的相同命令操作。如果需要，创建 virtualenv，安装 `weaviate-client` 4.x，运行 `migrate_weaviate_collections.py`，然后重启 `api`、`worker` 和 `worker_beat`。
