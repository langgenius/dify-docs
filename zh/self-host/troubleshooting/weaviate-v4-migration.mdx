...
- 可用 Python 3.11+ 用于 [schema 迁移脚本](/assets/migrate_weaviate_collections.py)
...
2. **运行[迁移脚本](/assets/migrate_weaviate_collections.py)**：
...
- 已安装 Docker + Docker Compose
- Python 3.11+ 用于[迁移脚本](/assets/migrate_weaviate_collections.py)
...
按照[步骤 A5](#步骤-a5迁移-schema)中的相同命令操作。如果需要，创建 virtualenv，安装 `weaviate-client` 4.x，运行 `migrate_weaviate_collections.py`，然后重启 `api`、`worker` 和 `worker_beat`。
