...
> 本文档由 AI 自动翻译。如有任何不准确之处，请参考 [英文原版](/en/self-host/use-dify/publish/publish-to-marketplace)。

将你的应用作为模板发布到 Dify 市场，让其他 Dify 用户可以发现和使用。

## 提交模板

若要发布模板，请通过 [创作者中心](https://creators.dify.ai) 提交。通过个人账户提交可自行管理模板，通过组织提交则可与团队共同管理。提交后，模板进入 **Pending** 状态。通过审核后，即可发布到市场。
...
模板发布后，可在 [**模板**](https://creators.dify.ai/dashboard/templates) 页面查看使用总数、整体评分、评分数量和点赞总数。打开 [**消息**](https://creators.dify.ai/dashboard/inbox)，逐条查看评分、点赞和私密反馈。
...
- **修改应用本身**（如提示词、工具、模型设置等）
    ...
    先下架模板，再将其保存为草稿，使其从 **Delisted** 进入 **Draft** 状态。替换模板文件后，重新提交审核。这会更新现有的 Marketplace 条目。
