...
- **个人账户**：只有你可以查看和管理模板。
- **组织**：所有组织成员都可以查看和管理模板。提交前选择组织；如有需要，可创建组织并邀请成员。
...
模板发布后，可在创作者中心的以下页面查看：

- [**模板**](https://creators.dify.ai/dashboard/templates)：汇总每个模板的使用总数、整体评分、评分数量和点赞总数。
- [**消息**](https://creators.dify.ai/dashboard/inbox)：逐条查看模板收到的评分、点赞和私密反馈。

当前所选个人账户或组织决定了两个页面显示哪些模板和回应。
