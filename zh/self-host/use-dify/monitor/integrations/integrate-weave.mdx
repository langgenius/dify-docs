---
title: 集成 W&B Weave
description: 通过 W&B Weave 实现 Dify 应用的 LLM 可观测性，将追踪数据发送至 W&B Weave
---

> 本文档由 AI 自动翻译。如有任何不准确之处，请参考 [英文原版](/en/self-host/use-dify/monitor/integrations/integrate-weave)。

<Info>
  **前提条件**：Dify v1.3.1 或更高版本。
</Info>

### 什么是 W&B Weave

Weights & Biases (W&B) Weave 是一个用于跟踪、实验、评估、部署和改进基于 LLM 的应用的框架。Weave 专为灵活性和可扩展性而设计，支持 LLM 应用开发工作流的每个阶段。

<Info>
    更多详情，请参考 [Weave](https://docs.wandb.ai/weave)。
</Info>
...
在 Dify 应用中配置 Weave。打开需要监控的应用，在侧边菜单中打开 **监控**，在页面上选择 **跟踪应用性能**。

<Frame>
  ![在 Dify 应用中配置 Weave](https://assets-docs.dify.ai/2025/04/c33e8fda75ee9052ed23c8690e314862.png)
</Frame>
...
<Frame>
  ![成功保存后，你可以在当前页面查看监控状态](https://assets-docs.dify.ai/2025/04/9486cee7bbb61f069842c9ea860e679c.png)
</Frame>
...
当你切换到 Weave 时，可以在仪表板中查看 Dify 应用的详细操作日志。

<Frame>
  ![](https://assets-docs.dify.ai/2025/04/2cb04027c00b606029fcc26af2801bfe.png)
</Frame>

通过 Weave 获取的详细 LLM 操作日志将帮助你优化 Dify 应用的性能。

### 监控数据列表

#### **Workflow/Chatflow 追踪信息**

**用于追踪 Workflow 和 Chatflow**
...
* status - 运行状态
* version - 工作流版本
* total_tokens - 当前运行中使用的总 token 数
* file_list - 处理的文件列表
...
* message_tokens - 消息中的 token 数
* answer_tokens - 回答中的 token 数
* total_tokens - 消息和回答中的总 token 数
* error - 错误信息
* inputs - 输入数据
* outputs - 输出数据
* file_list - 处理的文件列表
