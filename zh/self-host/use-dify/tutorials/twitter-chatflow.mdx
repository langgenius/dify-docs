...
<Note> ⚠️ 本文档由 AI 自动翻译。如有任何不准确之处，请参考 [英文原版](/en/self-host/use-dify/tutorials/twitter-chatflow)。</Note>
...
打开你的 Dify 实例并登录。若尚未部署，参见 [使用 Docker Compose 部署 Dify](/zh/self-host/deploy/ce/quick-start/docker-compose)。

本教程需要使用 LLM 处理 Crawlbase 抓取的社交平台数据，因此需自备至少一个模型供应商的凭据，如 OpenAI API Key 或本地的 Ollama。在 **右上角头像 - 设置 - 模型供应商** 中配置：
