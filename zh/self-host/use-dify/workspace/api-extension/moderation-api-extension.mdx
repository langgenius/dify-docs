...
> 本文档由 AI 自动翻译。如有任何不准确之处，请参考 [英文原版](/en/self-host/use-dify/workspace/api-extension/moderation-api-extension)。
...
当在 Chatflow、Agent、聊天助手等应用下开启 **内容审查>审查输入内容** 时，Dify 会给相应的自定义端点发送下列 HTTP POST 请求：
...
当在 Chatflow、Agent、聊天助手等应用下开启 **内容审查>审查输出内容** 时，Dify 会给相应的自定义端点发送下列 HTTP POST 请求：
...
## 代码示例
下面展示一段可部署在 Cloudflare 的 `src/index.ts` 代码。（Cloudflare 完整的使用方法参见 [此文档](/zh/self-host/use-dify/workspace/api-extension/cloudflare-worker)）
