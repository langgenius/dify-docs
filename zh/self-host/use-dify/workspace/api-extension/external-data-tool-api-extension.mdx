...
<Note> ⚠️ 本文档由 AI 自动翻译。如有任何不准确之处，请参考 [英文原版](/en/self-host/use-dify/workspace/api-extension/external-data-tool-api-extension)。</Note>

在创建 AI 应用时，开发者可以通过 [API 扩展](/zh/self-host/use-dify/workspace/api-extension/api-extension) 的方式实现使用外部工具获取额外数据组装至 Prompt 中作为 LLM 额外信息。
