...
> 本文档由 AI 自动翻译。如有任何不准确之处，请参考 [英文原版](/en/self-host/use-dify/nodes/llm)。

大型语言模型节点调用语言模型来处理文本、图像和文档。它向你配置的模型发送提示词并捕获其响应，支持结构化输出、上下文管理和多模态输入。

<Frame caption="大型语言模型节点配置界面">
  ![大型语言模型节点配置界面](https://assets-docs.dify.ai/dify-enterprise-mintlify/en/guides/workflow/node/85730fbfa1d441d12d969b89adf2670e.png)
</Frame>

<Info>
  在使用大型语言模型节点之前，请先在 **集成** > **模型供应商** 中配置至少一个模型供应商。
</Info>
...
多个 LLM 节点需要使用同一个模型时，可将模型选择存入环境变量，而不必逐个节点配置。编辑变量（例如改为指向另一个模型）后，所有绑定该变量的节点会一并更新。

打开 **环境变量** 面板，添加一个 **LLM** 类型的变量，选择模型及其参数。

然后在每个 LLM 节点中，将 **模型** 字段切换为变量模式，选择该变量。变量承载模型供应商、模型和模型参数；提示词等其他设置仍保留在各节点中。
...
启用记忆以在 Chatflow 对话中的多个大型语言模型调用之间维护上下文。启用后，之前的交互将作为格式化的用户 - 助手输出包含在后续提示词中。你可以通过编辑 `USER` 模板来自定义用户提示词的内容。记忆是节点特定的，不会在不同对话之间持续存在。
...
处理图像时，你可以控制细节级别：
- **高** - 对复杂图像具有更好的准确性但使用更多 token
- **低** - 对简单图像进行更快处理，使用较少 token
...
在 API 调用中，该开关对应 `reasoning_format` 参数。开关打开时，`reasoning_format` 为 `separated`，流式 API 客户端会通过独立的 `reasoning_chunk` 事件接收推理内容，与回答流分开。事件详情参见 [发送对话消息](/zh/api-reference/chat-messages/send-chat-message) 和 [执行工作流](/zh/api-reference/workflow-runs/run-workflow)。

<Info>
此设置仅对用 `<think>` 标签包裹推理内容的模型生效。
</Info>
