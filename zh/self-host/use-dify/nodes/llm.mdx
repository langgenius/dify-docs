...
在 API 调用中，该开关对应 `reasoning_format` 参数。开关打开时，`reasoning_format` 为 `separated`，流式 API 客户端会通过独立的 `reasoning_chunk` 事件接收推理内容，与回答流分开。事件详情参见 [发送对话消息](/zh/api-reference/chats/send-chat-message) 和 [执行工作流](/zh/api-reference/workflows/run-workflow)。
