  ...
  **数据安全须知**
  ...
  <Tabs>
    <Tab title="邀请已有 Agent">
      ...
      受邀的 Agent 就像全职员工：它的能力统一在 **Agent** 页面管理。你在该页面发布更新后，所有用到该 Agent 的工作流都会同步更新。
    ...
    <Tab title="从空白开始">
      创建一个只属于此节点的一次性 Agent，直接在这里配置，方法与在 **Agent** 页面 [构建](/zh/self-host/use-dify/build/new-agent/build) 相同。

      它就像为一件事请来的临时工。如果确实好用，可将它转入 **Agent** 页面，在其他地方复用。
