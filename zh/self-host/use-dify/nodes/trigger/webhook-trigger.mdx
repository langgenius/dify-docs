...
<Note> ⚠️ 本文档由 AI 自动翻译。如有任何不准确之处，请参考 [英文原版](/en/self-host/use-dify/nodes/trigger/webhook-trigger)。</Note>
...
1. 在 Workflow 中添加 Webhook 触发器后，系统将自动生成一个 webhook URL 作为监听外部 HTTP 请求的专用端点。
    ...
    <Info>
        将 `TRIGGER_URL` 环境变量（默认 `http://localhost`）设为此 URL 应使用的基础前缀。将其指向调用该 webhook 的外部系统可访问的公共域名或 IP 地址。
    </Info>
...
<Tip>
    若目标外部系统有可用的触发器插件，推荐使用 [插件触发器](/zh/self-host/use-dify/nodes/trigger/plugin-trigger)。
</Tip>
...
<Tip>
    一个工作流可以包含多个 Webhook 触发器。如果这些触发器分支共享相同的下游节点，可以添加 [变量聚合器](/zh/self-host/use-dify/nodes/variable-aggregator) 将它们汇聚，避免在每条分支上重复配置相同的节点。
</Tip>
