...
<Note> ⚠️ 本文档由 AI 自动翻译。如有任何不准确之处，请参考 [英文原版](/en/self-host/use-dify/getting-started/quick-start)。</Note>
...
<Steps>
  <Step title="部署 Dify">
    ...
    如果还没有运行中的实例，参照 [使用 Docker Compose 部署 Dify](/zh/self-host/deploy/ce/quick-start/docker-compose) 在本地启动一个。Dify 也支持源码安装和基于 AMI 的部署，其他方式详见 [部署](/zh/self-host/deploy/ce/quick-start/docker-compose) 章节。

  </Step>

  <Step title="登录">

    在浏览器中打开你的 Dify 实例，使用安装时创建的管理员账户登录。登录后将进入默认工作空间。
  ...
  <Step title="设置模型供应商">
    ...
    在插件配置中填入你自己的 OpenAI API Key。未添加至少一个有效凭据前，插件无法使用。

    <Tip>
      你可以从 [市场](https://marketplace.dify.ai/) 安装任意 LLM 插件并使用自己的 API Key。Ollama 等本地模型供应商同样可用，在对应插件中配置访问端点即可。
    </Tip>
  ...
  <Step title="配置默认模型">
    ...
    2. 将 **系统推理模型** 设为 `gpt-5.2` 或能力相当的其他模型，它将成为工作流中的默认模型。
  </Step>
...
<Frame>
  ![列表操作器](/images/use-dify/get-started/deeper-dive-list-operator.png)
</Frame>
...
<Frame>
  ![模板节点](/images/use-dify/get-started/deeper-dive-template.png)
</Frame>
...
1. 确保你的检查列表已清除。
    ...
    <Frame>
      ![检查检查列表](/images/use-dify/get-started/deeper-dive-checklist-clear.png)
    </Frame>
