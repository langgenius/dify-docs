...
<Note> ⚠️ 本文档由 AI 自动翻译。如有任何不准确之处，请参考 [英文原版](/en/self-host/use-dify/knowledge/external-knowledge-api)。</Note>

本页定义了外部知识服务必须实现的 API 协议，以便 Dify 从中检索内容。API 准备就绪后，详见 [连接外部知识库](/zh/self-host/use-dify/knowledge/connect-external-knowledge-base) 在 Dify 中完成注册。
