...
- **时长**：单次 Agent 运行无论从哪个访问点发起（Web 应用、服务 API 或工作流），默认都会在 20 分钟后被终止，未完成的回复会被清除。

    若运行被中断，可重试或把任务拆分成更小的步骤。要允许运行最长 1 小时，将 `APP_MAX_EXECUTION_TIME` 和 `WORKFLOW_MAX_EXECUTION_TIME` 调高到 `3600`；超过 1 小时还需调高 `DIFY_AGENT_RUN_TIMEOUT_SECONDS`。
