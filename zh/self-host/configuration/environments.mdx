...
工作流画布实时协作的总开关，涵盖多人同时编辑、画布评论和 @ 提及。设为 `true` 时，前端激活协作 UI。

仅设置此变量还不够。协作通过 WebSocket 运行，还需要：

- 将 `SERVER_WORKER_CLASS` 设为 `geventwebsocket.gunicorn.workers.GeventWebSocketWorker`，使 Gunicorn 能够接受 WebSocket 升级（参见 [容器启动配置](#容器启动配置)）。
- 将 `NEXT_PUBLIC_SOCKET_URL` 设为浏览器可访问的 WebSocket URL（参见 [CORS 配置](#cors-配置)）。

**运维注意事项：**

- **反向代理**：如果你通过自己的 nginx、traefik 或云负载均衡器（而非 Dify 内置 nginx）终止 TLS 或转发流量，请在 `/socket.io/` 路径上转发 `Upgrade` 与 `Connection` 请求头。Dify 自带的 `docker/nginx/conf.d/default.conf.template` 已包含此配置。
- **API 多副本部署**：socket.io 服务端按会话保存内存状态。运行多个 API 副本时，需在负载均衡层启用会话保持（sticky sessions），避免客户端在不同副本间跳转。
...
| `SERVER_WORKER_CLASS` | `gevent` | Gunicorn 工作进程类型。`gevent` 提供轻量级异步并发。当 `ENABLE_COLLABORATION_MODE` 为 `true` 时，需设为 `geventwebsocket.gunicorn.workers.GeventWebSocketWorker` 以接受 WebSocket 升级；若仍保留 `gevent`，协作客户端将在 WebSocket 握手阶段失败，且不会抛出启动错误。其他值会破坏 psycopg2 和 gRPC 补丁，不受支持。 |
...
| `NEXT_PUBLIC_SOCKET_URL` | `ws://localhost` | 实时协作的浏览器端 WebSocket 端点。当 `ENABLE_COLLABORATION_MODE` 为 `true` 时，工作流画布通过此地址进行实时编辑和评论。仅填写协议 + 主机 + 端口（路径 `/socket.io/` 由客户端固定），把 `http(s)://` 替换为 `ws(s)://`。示例：`wss://dify.example.com`。默认值 `ws://localhost` 在浏览器中解析为 80 端口，对应 Dify 自带 nginx。通过运行时环境注入生效，修改后容器重启即可应用，无需重新构建镜像。 |
