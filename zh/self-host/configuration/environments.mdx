...
控制台 API 的后端 URL。用于构建授权回调。如果留空，则默认为与应用程序相同的域。示例：`https://api.console.dify.ai`

### CONSOLE_WEB_URL

控制台 Web 界面的前端 URL。用于构建前端地址和 CORS 配置。如果留空，则默认为与应用程序相同的域。示例：`https://console.dify.ai`

### SERVICE_API_URL

Service API URL，用于在前端显示 Service API Base URL。如果留空，则默认为与应用程序相同的域。示例：`https://api.dify.ai`

### TRIGGER_URL

用于 webhook 触发器和插件触发器中 webhook 回调 URL 的基础 URL 前缀。

确保它指向外部系统可访问的公共域名或 IP 地址。

### APP_API_URL

WebApp API 后端 URL，用于指定前端 API 的后端 URL。如果留空，则默认为与应用程序相同的域。示例：`https://app.dify.ai`

### APP_WEB_URL

WebApp URL，用于向前端显示文件预览或下载 URL，或作为多模态输入；如果留空，则默认为与应用程序相同的域。示例：`https://udify.app/`

### FILES_URL

文件预览或下载 URL 的前缀，用于在前端显示这些 URL 并提供给多模态模型作为输入。为防止伪造，图片预览 URL 会进行签名，有效期为 5 分钟后过期。
...
调试模式：默认禁用。建议在本地开发时启用此设置，以防止因 monkey patching 引起的问题。

### FLASK_DEBUG

Flask 调试模式：启用后，会在 API 响应中输出追踪信息，便于调试。
...
- PRODUCTION（默认）：生产环境。
- TESTING：测试环境。前端页面上会有明显的颜色标签，表明这是一个测试环境。
...
对于从源代码启动，你需要在 api 目录中手动运行 `flask db upgrade`。
...
用于更改 OpenAI base 地址，默认为 [https://api.openai.com/v1](https://api.openai.com/v1)。

当在中国无法访问 OpenAI 时，替换为国内镜像地址，或者当本地模型提供 OpenAI 兼容 API 时，可以进行替换。
...
- SERVER_WORKER_AMOUNT：API 服务器 worker 数量，即 gevent worker 的数量。公式：`cpu 核心数 x 2 + 1`
...
- GUNICORN_TIMEOUT：请求处理超时。默认为 200。建议值为 360 以支持更长的 SSE（Server-Sent Events）连接时间。
- CELERY_WORKER_CLASS：类似于 `SERVER_WORKER_CLASS`。默认为 gevent。如果使用 windows，可以切换为 sync 或 solo。
- CELERY_WORKER_AMOUNT：Celery worker 的数量。默认为 1，可根据需要设置。
...
- DB_TYPE：数据库类型。
  ...
  - `postgresql`（默认）
  - `mysql`（MySQL 兼容数据库如 OceanBase 和 seekdb 也可使用此值）
...
此 Redis 配置用于缓存和对话过程中的发布/订阅。

- REDIS_HOST：Redis 主机
- REDIS_PORT：Redis 端口，默认为 6379
- REDIS_DB：Redis Database，默认为 0。请使用与 Session Redis 和 Celery Broker 不同的 Database。
...
- REDIS_SENTINEL_SERVICE_NAME：Sentinel 服务名，与 Master Name 相同
...
- CELERY_SENTINEL_MASTER_NAME：Sentinel 的服务名，即 Master Name
...
- CONSOLE_CORS_ALLOW_ORIGINS\
  控制台 CORS 跨域策略，默认为 `*`，即所有域都可访问。

- WEB_API_CORS_ALLOW_ORIGINS\
  WebAPP CORS 跨域策略，默认为 `*`，即所有域都可访问。

- COOKIE_DOMAIN\
  当前端和后端运行在不同子域时，将 `COOKIE_DOMAIN` 设置为站点的顶级域名（例如 `example.com`）。\
  前端和后端必须在同一顶级域名下才能共享认证 cookie。

- NEXT_PUBLIC_COOKIE_DOMAIN\
  当前端和后端运行在不同子域时，将 `NEXT_PUBLIC_COOKIE_DOMAIN` 设置为 `1`。\
  前端和后端必须在同一顶级域名下才能共享认证 cookie。
...
- STORAGE_TYPE
  ...
  - s3：S3 对象存储，如果选择此选项，需要设置下面 S3\_ 前缀的配置。

  - azure-blob：Azure Blob 对象存储，如果选择此选项，需要设置下面 AZURE_BLOB\_ 前缀的配置。

  - aliyun-oss：阿里云 OSS 对象存储，如果选择此选项，需要设置下面 ALIYUN_OSS\_ 前缀的配置。

  - huawei-obs：华为 OBS 对象存储，如果选择此选项，需要设置下面 HUAWEI_OBS\_ 前缀的配置。

  - volcengine-tos：火山引擎 TOS 对象存储，如果选择此选项，需要设置下面 VOLCENGINE_TOS\_ 前缀的配置。

  - tencent-cos：腾讯云 COS 对象存储，如果选择此选项，需要设置下面 TENCENT_COS\_ 前缀的配置。

- STORAGE_LOCAL_PATH

  默认为 storage，即存储在当前目录的 storage 目录中。

  如果使用 docker 或 docker-compose 部署，请确保在两个容器中都将 `/app/api/storage` 目录挂载到同一本地目录，否则可能会遇到文件未找到错误。

- S3_ENDPOINT：S3 endpoint 地址
...
- ALIYUN_OSS_PATH：your-path # 不要以 '/' 开头。OSS 不支持对象名以斜杠开头。参考：https://www.alibabacloud.com/help/en/oss/support/0016-00000005
...
- TENCENT_COS_SCHEME：指定 http/https 协议来访问 COS
...
- WEAVIATE_ENDPOINT
  ...
  Weaviate endpoint 地址，如：`http://weaviate:8080`。
...
- WEAVIATE_BATCH_SIZE
  ...
  在 Weaviate 中批量创建索引 Objects 的数量，默认为 100。
...
- WEAVIATE_GRPC_ENABLED
  ...
  是否使用 gRPC 方式与 Weaviate 交互，启用后性能会大幅提升，可能在本地无法使用，默认为 true。

- QDRANT_URL

  Qdrant endpoint 地址，如：`https://your-qdrant-cluster-url.qdrant.tech/`
...
- MILVUS_URI
  ...
  Milvus uri 配置。例如 `http://host.docker.internal:19530`。对于 [Zilliz Cloud](https://docs.zilliz.com/docs/free-trials)，将 uri 和 token 调整为 Public Endpoint 和 API Key。
...
- ANALYTICDB_KEY_ID
  ...
  用于阿里云 OpenAPI 认证的 access key ID。阅读 [Analyticdb 文档](https://help.aliyun.com/zh/analyticdb/analyticdb-for-postgresql/support/create-an-accesskey-pair) 创建你的 AccessKey。
...
- ANALYTICDB_INSTANCE_ID
  ...
  你的 AnalyticDB 实例的唯一标识符，如：`gp-xxxxxx`。阅读 [Analyticdb 文档](https://help.aliyun.com/zh/analyticdb/analyticdb-for-postgresql/getting-started/create-an-instance-1) 创建你的实例。
...
- ANALYTICDB_NAMESPACE
  ...
  你希望交互的 AnalyticDB 实例内的命名空间（schema），如 `dify`。如果此命名空间不存在，将自动创建。
...
- TABLESTORE_ENDPOINT
  ...
  TableStore 服务器的 endpoint 地址（例如 'https://instance-name.cn-hangzhou.ots.aliyuncs.com'）

- TABLESTORE_INSTANCE_NAME

  访问 TableStore 服务器的实例名（例如 'instance-name'）
...
- TABLESTORE_ACCESS_KEY_SECRET
  ...
  实例名的 accessKey secret

- LINDORM_URL

  LINDORM 搜索引擎的 URL，你可以从[控制台](https://lindorm.console.aliyun.com/)获取
...
- TENCENT_VECTOR_DB_API_KEY
  ...
  VectorDB 服务器的 API key（密码），用于身份认证。[密钥管理](https://cloud.tencent.com/document/product/1709/95108)。
...
- SINGLE_CHUNK_ATTACHMENT_LIMIT\
  可附加到单个 chunk 的最大文件数量。默认 10。

- ATTACHMENT_IMAGE_FILE_SIZE_LIMIT\
  可附加到 chunk 的图片最大允许大小（以兆字节为单位）。默认 2。

- ATTACHMENT_IMAGE_DOWNLOAD_TIMEOUT\
  下载图片附件的超时时间（以秒为单位）。默认 60。
...
- MULTIMODAL_SEND_IMAGE_FORMAT
  ...
  多模态模型输入时发送图片的格式，默认为 `base64`，可选 `url`。`url` 模式的调用延迟会低于 `base64` 模式。一般建议使用更兼容的 `base64` 模式。如果配置为 `url`，需要将 `FILES_URL` 配置为可外部访问的地址，以便多模态模型可以访问图片。
...
- SENTRY_DSN：Sentry DSN 地址，默认为空，为空时所有监控信息不上报给 Sentry。
- SENTRY_TRACES_SAMPLE_RATE：Sentry 事件的上报比例，如果是 0.01，则是 1%。
- SENTRY_PROFILES_SAMPLE_RATE：Sentry profiles 的上报比例，如果是 0.01，则是 1%。
...
- NOTION_CLIENT_SECRET：Notion OAuth client secret（用于公共集成类型）
- NOTION_CLIENT_ID：OAuth client ID（用于公共集成类型）
- NOTION_INTERNAL_SECRET：Notion 内部集成 secret。如果 `NOTION_INTEGRATION_TYPE` 的值为 "internal"，则需要配置此变量。
...
- MAIL_TYPE
  ...
  - resend
    ...
    - RESEND_API_KEY：Resend 邮件提供商的 API-Key，可从 API-Key 获取。
  ...
  - sendgrid
    - SENDGRID_API_KEY：SendGrid 邮件提供商的 API key。
    - MAIL_DEFAULT_SEND_FROM：发件人邮箱地址，如 your_email@sendgrid.com。此字段是 SendGrid 认证所必需的。

有关 SendGrid 邮件提供商的更多详情，请参阅 [SendGrid 文档](https://sendgrid.com/docs/for-developers/sending-email/)。

### 模型提供商和工具位置配置

用于指定可在应用中使用的模型提供商和工具。这些设置允许你自定义哪些工具和模型提供商可用，以及它们在应用界面中的顺序和包含/排除情况。

有关可用的[工具](https://github.com/langgenius/dify/blob/main/api/core/tools/provider/_position.yaml)和[模型提供商](https://github.com/langgenius/dify/blob/main/api/core/model_runtime/model_providers/_position.yaml)列表，请参阅提供的链接。

- POSITION_TOOL_PINS

  将特定工具置顶到列表顶部，确保它们首先出现在界面中。（使用逗号分隔值，项目之间**不要有空格**。）

  示例：`POSITION_TOOL_PINS=bing,google`

- POSITION_TOOL_INCLUDES

  指定要包含在应用中的工具。只有此处列出的工具才可使用。如果未设置，除非在 POSITION_TOOL_EXCLUDES 中指定，否则将包含所有工具。（使用逗号分隔值，项目之间**不要有空格**。）

  示例：`POSITION_TOOL_INCLUDES=bing,google`

- POSITION_TOOL_EXCLUDES

  排除特定工具，使其不在应用中显示或使用。此处列出的工具将从可用选项中省略，置顶工具除外。（使用逗号分隔值，项目之间**不要有空格**。）

  示例：`POSITION_TOOL_EXCLUDES=yahoo,wolframalpha`

- POSITION_PROVIDER_PINS

  将特定模型提供商置顶到列表顶部，确保它们首先出现在界面中。（使用逗号分隔值，项目之间**不要有空格**。）

  示例：`POSITION_PROVIDER_PINS=openai,openllm`

- POSITION_PROVIDER_INCLUDES

  指定要包含在应用中的模型提供商。只有此处列出的提供商才可使用。如果未设置，除非在 POSITION_PROVIDER_EXCLUDES 中指定，否则将包含所有提供商。（使用逗号分隔值，项目之间**不要有空格**。）

  示例：`POSITION_PROVIDER_INCLUDES=cohere,upstage`

- POSITION_PROVIDER_EXCLUDES

  排除特定模型提供商，使其不在应用中显示或使用。此处列出的提供商将从可用选项中省略，置顶提供商除外。（使用逗号分隔值，项目之间**不要有空格**。）
...
- ENABLE_CLEAN_MESSAGES
  ...
  是否启用消息清理任务。默认为 false。启用后，每天凌晨 4:00 清理过期的对话消息记录。
...
- QUEUE_MONITOR_INTERVAL
  ...
  数据集队列监控间隔（以分钟为单位）。默认为 30 分钟。仅在队列监控任务启用时有效。

- ENABLE_CHECK_UPGRADABLE_PLUGIN_TASK

  是否启用检查可升级插件任务。默认为 true。启用后，每 15 分钟检查可升级的插件版本。需要启用 MARKETPLACE_ENABLED。
...
- WORKFLOW_LOG_CLEANUP_ENABLED
  ...
  是否启用自动工作流日志清理任务。默认为 false。启用后，每天凌晨 2:00 清理超过保留期的工作流执行日志。

- WORKFLOW_LOG_RETENTION_DAYS

  工作流日志保留天数。默认为 30 天。仅在工作流日志清理任务启用时有效。
...
Sentry DSN 地址，默认为空，为空时所有监控信息不上报给 Sentry。
...
控制台 URL，用于拼接授权回调、控制台前端地址和 CORS 配置使用。如果为空，则为同一域。示例：`https://console.dify.ai`。

### API_URL

> ⚠️ 在 0.3.8 中修改，将在 0.4.9 中弃用，替换为 `SERVICE_API_URL`。

API URL，用于向前端显示 Service API Base URL。如果为空，则为同一域。示例：`https://api.dify.ai`

### APP_URL

> ⚠️ 在 0.3.8 中修改，将在 0.4.9 中弃用，替换为 `APP_API_URL` 和 `APP_WEB_URL`。

WebApp Url，用于向前端显示 WebAPP API Base Url。如果为空，则为同一域。示例：`https://udify.app/`
...
- SESSION_TYPE：Session 组件类型
  ...
  - redis（默认）：如果选择此项，需要设置下面以 SESSION_REDIS\_ 开头的环境变量。
  - sqlalchemy：如果选择此项，将使用当前数据库连接并使用 sessions 表来读写 session 记录。

- SESSION_REDIS_HOST：Redis 主机
- SESSION_REDIS_PORT：Redis 端口，默认为 6379
- SESSION_REDIS_DB：Redis Database，默认为 0。请使用与 Redis 和 Celery Broker 不同的 Database。
...
文档 chunk 长度配置。用于控制处理长文档时文本段的大小。默认：500。最大：4000。

**较大的 Chunk**
- 在每个 chunk 中保留更多上下文，适合需要对文本进行更广泛理解的任务。
- 减少 chunk 总数，降低处理时间和存储开销。

**较小的 Chunk**
- 提供更细的粒度，提高提取或摘要等任务的准确性。
- 降低超过模型令牌限制的风险，对于具有更严格约束的模型更安全。

**配置建议**
- 对于上下文密集型任务（如情感分析或文档摘要），选择较大的 chunk。
- 对于细粒度任务（如关键词提取或段落级处理），选择较小的 chunk。