...
> 您好，我想问一下 Dify 是什么？另外，它都支持那些模型？还有免费套餐吗？
...
<Frame>
  ![参数提取器](/images/difyworkflow101-lesson06-parameterextractor.png)
</Frame>

你可以把它想象成一位_信息搜集员_。它的工作就是阅读一段文本（比如邮件），然后根据你给定的规则，把里面所有符合条件的关键信息（比如所有独立的问题）都提取出来，整理成一个列表。
...
<Steps>
  <Step title="添加节点">
    ...
    <Frame>
      ![添加参数提取器](/images/difyworkflow101-lesson06-addparameterextractor.png)
    </Frame>
  </Step>
  <Step title="选择输入变量值">
    ...
    <Frame>
      ![设置输入变量](/images/difyworkflow101-lesson06-settheinput.png)
    </Frame>
  ...
  <Step title="添加提取参数">
    ...
    <Frame>
      ![添加提取参数](/images/difyworkflow101-lesson06-addextractparameter.png)
    </Frame>
    <Info>
      **参数类型**
      ...
      **单项类型（只装一个）**
      ...
      - **Boolean (布尔值)**：用来装_是_或_否_这样的判断结果。

      **列表类型（装多个）**

      - **Array[String]**（文本列表）：Array 代表_列表_，String 代表_文本_。所以，`Array[String]` 就意味着，我们要用一个能装多个文本的篮子，比如邮件里的所有问题。
      - **Array[Number]**（数字列表）：用来装多个数字，比如一份清单里的所有商品价格。
      - **Array[Boolean]**（布尔值列表）：用来装多个_是/否_判断结果。比如，检查一份包含多个待办事项的清单，返回每一项是否完成的结果，如 `[是，否，是]`。
      - **Array[Object]**（对象列表）：这是更高级的类型，用来装多个_结构化信息包_，比如一份包含_姓名_和_电话_的完整通讯录。
    </Info>
  </Step>
  <Step title="完成提取参数">
    ...
    <Frame>
      ![完成添加提取参数](/images/difyworkflow101-lesson06-finishaddextractparameter.png)
    </Frame>
  </Step>
  <Step title="添加指令">
...
<Frame>
  ![迭代节点](/images/difyworkflow101-lesson06-iteration.png)
</Frame>

这就像是给助理配备了一个_分身_。当你把一个列表（比如上面提取到的问题列表）交给它时，它会为列表中的每一个项目，都单独执行一遍你设定好的子流程（比如查询知识库并回答）。
...
<Steps>
  <Step title="添加节点">
    ...
    <Frame>
      ![添加迭代节点](/images/difyworkflow101-lesson06-additeration.png)
    </Frame>
    **迭代的高级选项**
    ...
    <Frame>
      ![迭代高级选项](/images/difyworkflow101-lesson06-advancediterationoptions.png)
    </Frame>
    **并行模式**：默认关闭。
  ...
  <Step title="添加知识检索节点">
    ...
    <Frame>
      ![添加知识检索节点并设置查询文本](/images/difyworkflow101-lesson06-knowledgeretrievalquerytext.png)
    </Frame>
  ...
  <Step title="添加 LLM 节点">
    ...
    <Frame>
      ![添加 LLM 节点和提示词](/images/difyworkflow101-lesson06-addllmandprompt.png)
    </Frame>
  ...
  <Step title="选择迭代节点的输出">
    ...
    <Frame>
      ![设置迭代输出](/images/difyworkflow101-lesson06-setiterationoutput.png)
    </Frame>
  </Step>
  <Step title="添加变量聚合器">
    1. 在迭代节点后新建**变量聚合器**节点。
    2. 在**变量赋值**内选择迭代 `{x} output Array[String]`。

    <Frame>
      ![添加变量聚合器](/images/difyworkflow101-lesson06-variableaggregator.png)
    </Frame>
  </Step>
  <Step title="添加最后的 LLM 节点">
    ...
    <Frame>
      ![添加最终 LLM 节点](/images/difyworkflow101-lesson06-addfinalllmnode-1.png)
    </Frame>
  </Step>
  <Step title="检查">
    ...
    <Frame>
      ![选择输出变量](/images/difyworkflow101-lesson06-ourputvariable.png)
    </Frame>
  </Step>
</Steps>
