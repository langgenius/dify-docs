...
如何让我们的助理学会看人下菜碟，对不同类型的邮件进行差异化处理呢？让我们为工作流设置一个_十字路口_，它能根据不同的信号，让流程走向不同的轨道。
...
- **是 / 不是**
  ...
  完全等于或不等于某个值。可以把它想象成_一把钥匙开一把锁_，只有当变量内容和我们设定的值一模一样时，条件才算满足。
- **包含 / 不包含**
...
<Frame>
  ![LLM 2 的提示词](/images/difyworkflow101-lesson05-promptforllm2.png)
</Frame>
...
可以把它想象成一个_漏斗_或一个_交通枢纽_。无论上游有多少不同分支的数据流，变量聚合器都能将它们汇集到一个出口，确保数据流的整洁和稳定，方便后续统一处理。
