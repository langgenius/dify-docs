...
变量赋值器节点通过写入会话变量（在[这里](/zh/use-dify/getting-started/key-concepts#variables)了解不同类型的变量）来管理对话流应用中的持久化数据。与每次执行都会重置的常规工作流变量不同，会话变量在整个聊天会话期间持续存在。
