...
2. 填写以下字段：
    ...
    - **API 端点**：你的外部知识服务的检索 URL。请参阅[外部知识库 API 规范](/zh/use-dify/knowledge/external-knowledge-api)了解所需的端点格式。
...
2. 填写以下字段：
    ...
    - **检索设置**：
      ...
      - **分数阈值**：返回分段的最低相似度分数。启用此选项可过滤掉低相关性结果。使用较高阈值以获得更严格的相关性，或使用较低阈值以包含更广泛的匹配。

          禁用时，将返回所有达到 Top K 限制的结果，而不考虑分数。

创建完成后，外部知识库可以像任何内置知识库一样在你的应用中使用。请参阅[在应用内集成知识库](/zh/use-dify/knowledge/integrate-knowledge-within-application)了解后续步骤。
...
如果检索失败或返回意外结果，请根据[外部知识库 API 规范](/zh/use-dify/knowledge/external-knowledge-api#response-elements)验证你的 API 响应。
