...
Dify 会在每个请求中将你配置的 API Key 作为 Bearer 令牌发送：
...
Dify 会在你配置的端点 URL 后追加 `/retrieval`。如果你注册的地址为 `https://your-service.com`，Dify 会将请求发送至 `https://your-service.com/retrieval`。

### 请求体

| 属性 | 必填 | 类型 | 说明 |
|:---------|:---------|:-----|:------------|
| `knowledge_id` | 是 | string | 外部系统中知识源的标识符。这是你在连接时填写的 **外部知识库 ID** 字段的值。用于将查询路由到正确的知识源。 |
...
<Info>
Dify 会将元数据条件传递给你的 API，但目前未提供供用户配置的界面。此参数仅用于程序化调用。
</Info>
...
| `value` | 否 | string、number 或 array[string] | 比较值。使用 `empty`、`not empty`、`null` 或 `not null` 时可省略。 |
...
Dify 不会拒绝缺少字段的记录，但缺少 `content` 或 `score` 会导致结果不完整或无法排序。

<Warning>
如果在记录中包含 `metadata`，它必须是一个对象（`{}`），不能为 `null`。`null` 的 metadata 值会导致 Dify 检索流程出错。
</Warning>
...
Dify 会检查响应的 HTTP 状态码。非 200 状态码会触发一个错误并展示给用户。

你可以选择在 JSON 中返回结构化的错误信息：

| 属性 | 类型 | 说明 |
|:---------|:-----|:------------|
| `error_code` | int | 你自定义的应用级错误码。 |
| `error_msg` | string | 可读的错误描述。 |

以下是建议的错误码。这些仅为约定，Dify 不会强制要求：
