...
Dify v1.10.0 引入了触发器（Trigger）类型的开始节点。区别于代码（Code）、工具（Tool）、知识检索（Knowledge Retrieval）等功能性节点，触发器的作用是**将第三方事件转化为 Dify 能够接收的输入参数格式**。
...
- Gmail 的原始请求格式不是 Dify 可接收的标准格式；

- 全世界有成千上万的平台，每个平台的格式都不尽相同。

因此，我们需要触发器来定义和解析这些来自不同平台、不同格式的事件，并将它们统一转换为 Dify 可接收的标准入参格式。
...
- **事件**：一个平台可能会发送多种类型的事件，例如 *新邮件*、*邮件删除*、*邮件已读* 等，所有事件均会被推送至预先配置的地址。在一个触发器插件中，可配置多个事件以处理不同类型的事件，一个事件对应 Dify 工作流中的一个触发器节点。

## 插件开发

触发器插件的开发方式与工具（Tool）、数据源（Data Source）、模型（Model）等插件类型一致，均可通过 `dify plugin init` 命令创建开发模板，文件结构遵循统一的插件格式规范：
...
对于 Notion 这类平台，订阅的创建需要用户手动将 Dify 提供的回调地址（Callback URL）复制粘贴到 Notion 后台中，以完成 Webhook 配置。此流程对应 Dify 界面上的 **粘贴 URL 以创建新订阅（Paste URL to create a new subscription）** 选项。

<Columns cols={2}>
    <img src="/images/trigger_plugin_manual_webhook_setup.PNG" alt="Manually Create Subscription" />
    <img src="/images/trigger_plugin_manual_webhook_setup_config.PNG" alt="Manual Configuration" />
</Columns>
...
<Tabs>
    ...
    <Tab title="github.py">
        在右图中可以看到，界面下方有一个 **Request Logs** 区域。首先，我们需要编写 `dispatch_event` 接口。所有发送到 Callback URL 的请求都会先经过该接口处理，处理后的事件将显示在 Request Logs 中，便于调试与验证。

        在代码中，可通过 `subscription.properties` 获取在 YAML 文件中声明的 `webhook_secret`。
...
提取出对应事件后，需要由相应的事件实现对原始 HTTP 请求进行过滤，并将其转换为 Dify 工作流可接收的入参。

以 `Issue` 事件为例，可分别通过 `issue_open.yaml` 和 `issue_open.py` 来定义事件与实现事件。事件的输出可在 `output_schema` 中定义，与工具类型插件相同，遵循 JSON Schema 规范。
...
若希望插件能够筛选掉部分事件，例如只关注具有某个特定标签的 Issue 事件，可在事件定义中为 `issue` 添加 `parameters`。在 `_on_event` 方法中，通过抛出 `EventIgnoreError` 异常，即可在实际运行中根据配置的参数过滤掉不符合条件的事件。
...
<Tabs>
    <Tab title="github.yaml">
        ...
        - `credentials_schema`（可选）：通过 API Key 创建订阅时，需要声明所需的密钥，例如在 GitHub 中定义 `access_tokens`。

        - `oauth_schema`（可选）：通过 OAuth 创建订阅时，必须填写该字段。具体定义方式，可参考 [为插件添加 OAuth 支持](/plugin-dev-zh/0222-tool-oauth)。
    </Tab>
    <Tab title="github.py">
...
完成以上配置后，你将在 Dify 的前端界面中看到 **Create with API Key** 选项。
...
受篇幅所限，本文档仅展示最基础的实现示例。更多详细的接口说明，请参考以下接口文档。
