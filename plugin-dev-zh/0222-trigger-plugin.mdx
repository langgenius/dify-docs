...
因此，我们需要触发器来定义和解析这些来自不同平台、不同格式的事件，并将它们统一为 Dify 可接受的标准入参格式。
...
对于 Notion 这类平台，订阅的创建需要用户手动将 Dify 提供的回调地址（Callback URL）复制粘贴到 Notion 后台中，以完成 Webhook 配置。此流程对应 Dify 界面中的 **粘贴 URL 以创建新订阅（Paste URL to create a new subscription）** 选项。

![Paste URL to create a new subscription](/images/trigger_plugin_manual_webhook_setup.PNG)
...
<Tabs>
    ...
    <Tab title="github.py">
        ...
        ![Manual Setup](/images/trigger_plugin_manual_webhook_setup_config.PNG)

       首先，我们需要编写 `dispatch_event` 接口。所有发送到 Callback URL 的请求都会先经过该接口处理，处理后的事件将显示在界面上的 **Request Logs** 中，便于调试与验证。
...
提取出对应事件后，需要由相应的事件实现对原始 HTTP 请求进行过滤，并将其转换为 Dify 工作流可接受的入参。

以 Issue 事件为例，可分别通过 `issue_open.yaml` 和 `issue_open.py` 来定义事件与实现事件。事件的输出可在 `issue_open.yaml` 的 `output_schema` 中定义，与工具类型插件相同，遵循 JSON Schema 规范。
...
完成以上两个文件的配置后，你将在 Dify 界面中看到 **Create with API Key** 选项。
...
受篇幅所限，本文仅展示最基础的实现示例。更多详细的接口说明，请参考以下接口文档。
