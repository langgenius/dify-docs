---
title: "数据源插件"
description: "本文介绍如何开发数据源插件（包括插件结构、代码示例、调试方法及发布流程等），帮助你快速完成插件开发与上线。 "
---
...
数据源（Datasource）插件是 Dify 1.9.0 新引入的一种插件类型。在知识流水线（Knowledge Pipeline）中，其作为文档数据的来源并充当整个流水线的起始点。
...
<Note>
  关于如何继承父类以实现插件功能，请阅读 [Dify 插件开发：Hello World 指南-4.4 实现工具 (Tool) 逻辑](plugin-dev-zh/0211-getting-started-dify-tool#4.4-实现工具-\(Tool\)-逻辑) 章节。
</Note>
...
接下来，我们将分别介绍数据源插件的结构，以及如何开发三种数据源插件。

## 插件开发

### 创建数据源插件

和其他插件一样，你可以使用脚手架命令行工具来创建数据源插件，并选择 `datasource` 类型。
...
一般情况下，数据源插件无需使用 Dify 平台的其他功能，因此无需为其设置额外权限。完成设置后，命令行工具将自动生成插件项目代码。

#### 数据源插件结构

与其他类型的插件相似，数据源插件包含三个主要部分：
...
- 在 `manifest.yaml` 文件中，支持的最低 Dify 版本需设置如下：

  ```yaml
  minimum_dify_version: 1.9.0
  ```
- 在 `requirements.txt` 文件中，数据源插件开发使用的插件 SDK 版本需设置如下：

  ```yaml
  dify-plugin>=0.5.0,<0.6.0
  ```
- 在 `manifest.yaml` 文件中，需添加如下数据源标签，以便插件在 Dify Marketplace 中以数据源分类展示：
...
```yaml
# 指定插数据源的 provider 类型，可设置为 online_drive，online_document 或 website_crawl
...
<Info>
  更多关于创建供应商 YAML 文件的信息，请阅读 [Dify 插件开发：Hello World 指南 - Dify Docs](plugin-dev-zh/0211-getting-started-dify-tool#4-3-配置-provider-凭证)。
</Info>

数据源插件同样支持以 OAuth 2.0 或 API Key 两种方式进行认证。关于 OAuth 配置的详细信息，请阅读 [为工具插件添加 OAuth 支持](zh-hans/plugins/quick-start/develop-plugins/tool-oauth)。

#### 创建供应商代码文件

- **使用 API Key 认证模式时**，数据源插件的供应商代码文件与工具插件完全相同，仅需将供应商类继承的父类修改为 `DatasourceProvider` 即可。
  ...
  ```
- **使用 OAuth 认证模式时**，数据源插件则与工具插件略有不同。在使用 OAuth 获取访问权限时，数据源插件可同时返回用户名和头像并显示在前端。因此，`_oauth_get_credentials` 和 `_oauth_refresh_credentials` 需要返回包含 `name`、`avatar_url`、`expires_at` 和 `credentials` 的 `DatasourceOAuthCredentials` 类型。
...
三种数据源需要创建的 YAML 文件格式与数据源代码格式不完全相同，下面将分别介绍。

#### 网络爬虫（Website Crawl）

网页爬虫类型插件的供应商 YAML 文件中， `output_schema` 需固定返回四个参数：`source_url`、`content`、`title` 和 `description`。
...
在 `Webcrawl` 类型数据源的主逻辑代码中，需继承 `WebsiteCrawlDatasource` 类并实现 `_get_website_crawl` 方法，然后使用 `create_crawl_message` 方法返回网页爬虫消息。
...
在线文档类型插件的返回值至少需包含 `content` 字段，用于表示文档内容。示例如下：
...
在线文档类型数据源插件的主逻辑代码中，需继承 `OnlineDocumentDatasource` 类并实现 `_get_pages` 和 `_get_content` 两个方法。当用户运行插件时，首先调用 `_get_pages` 方法获取文档列表；当用户从列表中选择某个文档后，再调用 `_get_content` 方法获取文档内容。
...
在线网盘类型数据源插件的返回值类型为一个文件，因此需遵循以下规范：
...
在线网盘类型数据源插件的主逻辑代码中，需继承 `OnlineDriveDatasource` 类并实现 `_browse_files` 和 `_download_file` 两个方法。

当用户运行插件时，首先调用 `_browse_files` 方法获取文件列表。此时 `prefix` 为空，表示获取根目录下的文件列表。文件列表中包含文件夹与文件两种类型的变量。当用户继续打开文件夹时，将再次调用 `_browse_files` 方法。此时， `OnlineDriveBrowseFilesRequest` 中的 `prefix` 为文件夹 ID，用于获取该文件夹内的文件列表。
...
对于 AWS S3 等存储服务商，`prefix`，`bucket` 和 `id` 变量有特殊的使用方法，可在实际开发中按需灵活使用。
...
<Tip>
  更多详细信息，请参考 [官方 Google Drive 插件](https://github.com/langgenius/dify-official-plugins/blob/main/datasources/google_cloud_storage/datasources/google_cloud_storage.py) 和 [官方 AWS S3 插件](https://github.com/langgenius/dify-official-plugins/blob/main/datasources/aws_s3_storage/datasources/aws_s3_storage.py) 中的具体实现。
</Tip>

## 调试插件

数据源插件支持两种调试方式：远程调试或安装为本地插件。需注意：

- 若插件使用 OAuth 认证模式，其远程调试时的 `redirect_uri` 与本地插件的设置并不一致，需及时修改服务商 OAuth App 的相关配置。
- 虽然数据源插件支持单步调试，但为了确保功能的正确性，我们仍推荐你在完整的知识流水线中进行测试。

## 最终检查

在打包与发布前，请确认以下事项已全部完成：
