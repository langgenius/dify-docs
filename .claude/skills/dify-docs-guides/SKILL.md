...
- **Pull the latest Dify code before verifying.** `git fetch origin`, then fast-forward `main` only if the working tree is clean (substitute the branch if the user specifies one); don't force a checkout over a dirty or feature tree.
