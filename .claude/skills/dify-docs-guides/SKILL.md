...
| Dify-Enterprise-Docs `{lang}/{X.Y.x}/use/` | App builders whose company runs Dify Enterprise | Read as `en/self-host/use-dify/` above: the deployment exists and the reader is a builder, not the operator; EE features are enabled | Same as `en/self-host/use-dify/` |
| Dify-Enterprise-Docs `{lang}/{X.Y.x}/develop/plugins/` | Read as `en/develop-plugin/` above | Same | Same |
