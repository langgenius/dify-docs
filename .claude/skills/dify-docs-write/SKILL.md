...
For a new page or a rewrite, the outline is the summary's sections in the reader's order, each named by the question it answers; the owner approved it with the summary, so re-propose only what changed since. Exclusions need no defense; an inclusion that answers no question is the thing to defend.
