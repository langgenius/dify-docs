...
- **Message provenance**: an example `message` must be the string constructed at the actual raise site, not a class-default `description` and not a log line. A `BaseServiceError` (`api/services/errors/`) stores the constructor's positional argument in `description` and shadows its class-level template, so `str(e)` and `.description` are that argument, or `""` when the raise passed none. Which of those reaches the wire is the controller's choice: uncaught, the global handler returns `str(e)` as a 400 `invalid_param`; caught, the message belongs to the `BaseHTTPException` the controller raises, the service error's string when it re-raises with `error.description` and the HTTP class's own default when it re-raises with nothing. `api/controllers/service_api/app/file.py` does both, so read the controller's raise as well as the service's.
...
- Over-generalizing message behavior across an error-class hierarchy or across controllers (one controller re-raising with the service message doesn't mean its sibling does — read each raise).
