...
4. **Fix authors**: fix technical contract issues in Dify and import a new export. Apply approved documentation edits to the three generated specs, then save them with `build_specs.py capture`. Apply edits serially; all authors share those files and overlays.

Shared endpoints exist **once** per language with mode-aware descriptions. The imported technical contract is common to all three outputs. Translate human-language annotations; keep wire `message`/`code` values and example literals verbatim. Review existing annotations whenever the raw contract changes, even when their targets still exist.

After every fix batch, run `build_specs.py build --check`, `validate_specs.py`, `lint_specs.py`, and `parity_check.py` using the commands in `tools/api-pipeline/README.md`; all exit nonzero on failure. Regenerate navigation and check coverage when operations, labels, ordering, or availability change. Validation and parity are mechanical checks, not substitutes for the code audit or review of translations and streaming descriptions.
