...
- **Binary/file responses**: use `content` with the right media type and `{ "type": "string", "format": "binary" }` (`audio/mpeg` for fixed-format audio, `application/octet-stream` for generic files or when the code selects the media type at runtime — mirror the controller annotation). Put details in the response `description`, not the endpoint description.
