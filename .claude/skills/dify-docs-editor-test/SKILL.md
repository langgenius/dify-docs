...
3. {SHAPE}: the conventions this kind of page follows (its sections, tables,
   synopsis blocks), given as a file or as one named section of a file;
   read only that section. A structure the conventions require is not a
   fault; judge the sentences inside it. If this line reads "none", skip it.
