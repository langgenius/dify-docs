...
[代码节点](/guides/workflow/node/code) 支持运行 Python 和 JavaScript 代码，用于在工作流或聊天流中对数据进行改造。以下是常见的 4 种运行错误：
...
[LLM 节点](/guides/workflow/node/llm)是 Chatflow 和 Workflow 的核心组件，利用大语言模型的对话、生成、分类和处理能力，基于使用者输入的指令完成各种任务。
...
2.  **上下文结构无效（InvalidContextStructureError）**
    ...
    LLM 节点内的 [上下文](/guides/workflow/node/llm#explanation-of-special-variables) 接收到非法数据结构（如 `array[object]`）时会报错。
...
5.  **LLM 需授权（LLMModeRequiredError）**
    ...
    LLM 节点所选中的模型未配置 API Key，你可以阅读 [此文档](/guides/tools/tool-configuration) 授权模型。
...
[HTTP 节点](/guides/workflow/node/http-request)允许通过发送 HTTP 请求获取数据、触发Webhook、生成图像或下载文件，从而通过可定制的请求与外部服务无缝集成。以下是该节点常见的 5 种错误：
