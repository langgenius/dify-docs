...
Dify 平台尚不能直接连接 AWS Bedrock Knowledge Base，需要开发团队参考 Dify 关于外部知识库连接的 [API 定义](/en/../../guides/knowledge-base/external-knowledge-api-documentation)，手动创建后端 API 服务，建立与 AWS Bedrock 的连接。具体架构示意图请参考：
...
登录 AWS Bedrock Knowledge 后台，获取已创建 Knowledge Base 的 ID。此参数将会在[后续步骤](/en/how-to-connect-aws-bedrock#id-5.-lian-jie-wai-bu-zhi-shi-ku)用于与 Dify 平台的连接。
...
* API 接口地址，外部知识库的连接地址，可在[第二步](/en/how-to-connect-aws-bedrock#id-2.-gou-jian-hou-duan-api-fu-wu)中自定义。示例 `api-endpoint/retrieval`；
* API Key，外部知识库连接密钥，可在[第二步](/en/how-to-connect-aws-bedrock#id-2.-gou-jian-hou-duan-api-fu-wu)中自定义。
...
*   **外部知识库 API**
    ...
    选择在[第四步](/en/how-to-connect-aws-bedrock#id-4.-guan-lian-wai-bu-zhi-shi-api)中关联的外部知识库 API
*   **外部知识库 ID**

    填写在[第三步](/en/how-to-connect-aws-bedrock#id-3.-huo-qu-aws-bedrock-knowledge-base-id)中获取的 AWS Bedrock knowledge base ID
*   **调整召回设置**
