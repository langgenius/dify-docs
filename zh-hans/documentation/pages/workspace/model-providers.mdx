...
模型供应商为您的工作区提供AI模型访问。您构建的每个应用程序都需要模型才能运行，在工作区级别配置供应商意味着所有团队成员都可以在所有项目中使用它们。

## 系统供应商 vs 自定义供应商

**系统供应商**由Dify管理。您无需设置即可立即访问模型，通过Dify订阅计费，并在新模型可用时自动更新。最适合快速入门。

**自定义供应商**使用您自己的API密钥直接访问模型供应商，如OpenAI、Anthropic或Google。您获得完全控制权、直接计费，通常还有更高的速率限制。最适合生产应用程序。

您可以同时使用两种方式——系统供应商用于原型设计，自定义供应商用于生产。
...
<Steps>
  ...
  <Step title="添加凭据">
    输入您的API密钥和供应商要求的任何其他配置。
  </Step>
  
  <Step title="测试并保存">
    Dify在使供应商对您的工作区可用之前会验证您的凭据。
  </Step>
</Steps>
...
系统供应商通过您的Dify订阅计费，使用限制基于您的计划。自定义供应商直接通过供应商（OpenAI、Anthropic等）向您收费，通常提供更高的速率限制。
...
[编辑此页面](https://github.com/langgenius/dify-docs/edit/main/en/guides/workspace/model-providers.mdx) | [报告问题](https://github.com/langgenius/dify