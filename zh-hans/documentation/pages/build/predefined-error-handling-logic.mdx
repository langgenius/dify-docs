...
<AccordionGroup>
  <Accordion title="无">
    默认行为。当节点失败时，整个工作流停止。您会收到原始错误消息。
  ...
  <Accordion title="默认值">
    ...
    **要求**
    - 默认值必须与节点的输出类型匹配——如果输出字符串，您的默认值也必须是字符串。

    **示例**

    您的大型语言模型节点通常返回分析结果，但有时由于速率限制而失败。设置一个默认值，如：
  ...
  <Accordion title="失败分支">
    ...
    **示例**
    ...
    您的主 API 失败，因此失败分支调用备用 API。用户永远不会知道出现了问题。
  </Accordion>
</AccordionGroup>
