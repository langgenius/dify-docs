...
要查看完整的功能实现，请参考模型调用[示例代码](/en/agent-strategy-plugin#diao-yong-gong-ju-1)。
...
如需查看完整的功能代码，请阅读调用工具[示例代码](/en/agent-strategy-plugin#diao-yong-gong-ju-1)。
...
现在可以将它上传至 [Dify Plugins 代码仓库](https://github.com/langgenius/dify-plugins)来发布你的插件了！上传前，请确保插件已遵循[插件发布规范](/zh-hans/plugins/publish-plugins/publish-to-dify-marketplace)。审核通过后，代码将合并至主分支并自动上线至 [Dify Marketplace](https://marketplace.dify.ai/)。
