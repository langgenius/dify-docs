...
* Dify 插件脚手架工具，详细说明请参考[初始化开发工具](/en/../initialize-development-tools)。
...
现在开始实际的插件编码工作。在开始之前，请确保你已经阅读过[快速开始：开发 Extension 插件](/en/../extension-plugin)，或已动手开发过一次 Dify 插件。
...
现在可以将它上传至 [Dify Marketplace 仓库](https://github.com/langgenius/dify-plugins) 来发布你的插件了！不过在发布前，请确保你的插件遵循了[插件发布规范](/zh-hans/plugins/publish-plugins/publish-to-dify-marketplace)。
