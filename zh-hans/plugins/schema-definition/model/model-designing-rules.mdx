...
* 模型供应商规则基于 [Provider](/en/model-designing-rules#provider) 实体。
* 模型规则基于 [AIModelEntity](/en/model-designing-rules#aimodelentity) 实体。
...
* `help` (object) \[optional] 帮助信息
  ...
  * `url` (object) 帮助链接，i18n
    ...
    * `en_US` (string) 英文链接
* `supported_model_types` (array\[[ModelType](/en/model-designing-rules#modeltype)]) 支持的模型类型
* `configurate_methods` (array\[[ConfigurateMethod](/en/model-designing-rules#configuratemethod)]) 配置方式
* `provider_credential_schema` (\[[ProviderCredentialSchema](/en/model-designing-rules#providercredentialschema)]) 供应商凭据规格
* `model_credential_schema` (\[[ModelCredentialSchema](/en/model-designing-rules#modelcredentialschema)]) 模型凭据规格
...
* `parameter_rules` (array\[[ParameterRule](/en/model-designing-rules#parameterrule)]) \[optional] 模型调用参数规则
* `pricing` (\[[PriceConfig](/en/model-designing-rules#priceconfig)]) \[optional] 价格信息
...
* `credential_form_schemas` (array\[[CredentialFormSchema](/en/model-designing-rules#credentialformschema)]) 凭据表单规范
...
* `required` (bool) 是否必填
* `default`(string) 默认值
* `options` (array\[[FormOption](/en/model-designing-rules#formoption)]) 表单项为 `select` 或 `radio` 专有属性，定义下拉内容
* `placeholder`(object) 表单项为 `text-input` 专有属性，表单项 PlaceHolder
  * `en_US`(string) 英文
  * `zh_Hans` (string) \[optional] 中文
* `max_length` (int) 表单项为`text-input`专有属性，定义输入最大长度，0 为不限制。
* `show_on` (array\[[FormShowOnObject](/en/model-designing-rules#formshowonobject)]) 当其他表单项值符合条件时显示，为空则始终显示。
