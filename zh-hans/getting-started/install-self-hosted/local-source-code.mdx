...
# 如果不使用 postgres,请将 profile 改为 mysql
# 如果不使用 weaviate,请将 profile 改为其他向量数据库
...
此外,如果您想调试 celery 定时任务或运行 Schedule Trigger 节点,可以在另一个终端中运行以下命令来启动 beat 服务：
...
- 安装 NodeJS
  ...
  请访问 [https://nodejs.org/en/download](https://nodejs.org/en/download) 并选择适合您操作系统的 v22.x 或更高版本的安装包。推荐使用 LTS 版本进行常规使用。
