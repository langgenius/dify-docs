...
- **自分が公開したバージョンを探す：** **フィルター**ボタンをクリックするとフィルターオプションが表示されます。以下の2つの選択肢があります：  
    ...
    - **自分が公開したバージョン：** あなたが公開したバージョンのみを表示します。  必要に応じて適切なフィルターを選択してください。
...
1. Difyのチャットフロー・ワークフロー管理インターフェースでチャットフローやワークフローの作成が完了したら、パネル右上の**更新を公開**ボタンをクリックします。すると、**このバージョンに名前を付ける**ダイアログが表示されます。

![]()

2. *（任意）*ダイアログの**バージョン名**欄にバージョン名を、**バージョン説明**欄に説明を入力します。

> 情報を入力せずに**公開**ボタンをクリックした場合、システムはデフォルトのバージョン名（Untitled Version）を設定し、説明は空のままで保存します。

![]()

3. ダイアログ内の**公開**ボタンをクリックして、現在のバージョンを公開します。

4. 公開後、この最新バージョンは`Latest`としてマークされ、関連情報がバージョン管理インターフェースに表示されます。
...
3. **確定**をクリックすると、そのバージョンがバージョン管理画面から削除されます。
...
3. **確定**をクリックすると、現在の下書きバージョンがその履歴バージョンに置き換えられます。
