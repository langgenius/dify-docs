...
この機能を有効にしても、LLMにファイルを直接読み取る権限は与えられません。ファイルをLLMが理解できるテキストに解析するためにはが必要です。音声、映像、およびその他のファイルタイプには対応するエクストラクターが存在せず、アプリ開発者が[外部ツール](../tools/advanced-tool-integration.md)を接続して処理する必要があります。
