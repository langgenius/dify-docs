...
Opikは、大規模言語モデル（LLM）アプリケーションを評価、テスト、および監視するためのオープンソースプラットフォームです。Cometによって開発され、LLMベースのアプリケーションに対する直感的なコラボレーション、テスト、および監視を促進することを目的としています。
...
右上のユーザーメニューからOpik APIキーを取得します。**API Key**をクリックし、APIキーをコピーしてください。
