...
コンテンツ検索の精度向上のため、一部のチームは独自の検索アルゴリズムを開発したり、クラウドサービスプロバイダー（例：[AWS Bedrock](https://aws.amazon.com/bedrock/)）にナレッジベースをホストすることを選択しています。中立的なLLMアプリケーション開発プラットフォームであるDifyは、開発者がナレッジベースを選択する際の柔軟性を高めることにコミットしています。

Difyは、APIを通じて外部のナレッジベースに接続する機能を提供し、開発者にナレッジベースの選択肢やコンテンツの拡張のための多様なオプションを提供しています。外部のナレッジベースからのテキストリソースは、DifyのAIアプリケーションエコシステムにシームレスに統合され、アプリケーションの知識の幅と深さを向上させます。
...
* Name
  異なる外部ナレッジAPIを区別するためのカスタマイズ可能な名前；
* API Endpoint
  [外部ナレッジベースAPI](external-knowledge-api-documentation.md)エンドポイントのURL、例：api-endpoint/retrieval；詳しい指示については外部ナレッジAPIを参照してください；
* API Key
  外部ナレッジへの接続キー、詳しい指示については[外部ナレッジベースAPI](external-knowledge-api-documentation.md)を参照してください。
