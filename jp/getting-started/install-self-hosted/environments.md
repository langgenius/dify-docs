...
* OCEANBASE\_VECTOR\_DATABASE
  ...
  OceanBase Vector データベース。

* OCEANBASE\_CLUSTER\_NAME（オプション）

  OceanBase クラスタ名，OceanBase Docker にのみ有効。

* OCEANBASE\_MEMORY\_LIMIT（オプション）

  OceanBase メモリ使用上限，OceanBase Docker のみに有効。
