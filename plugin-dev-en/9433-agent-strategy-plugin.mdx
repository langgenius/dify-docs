...
dimensions:
  type:
    ...
    detail: examples
  level: advanced
standard_title: Agent Strategy Plugin Framework
language: en
title: Agent Strategy Plugin Framework
...
Agent strategies give your assistants a point of view. Use this framework to craft orchestration logic that selects tools intelligently, reasons through complex tasks, and delivers reliable outcomes.
...
The full working example—including model invocation, tool coordination, and logging helpers—is available in the [`basic_agent.py` reference implementation](https://github.com/langgenius/dify-plugin-sdks/blob/main/python/examples/basic_agent/strategies/basic_agent.py). Review that file alongside this section; the key integration points to focus on are:

- **Model orchestration** – `self.session.model.llm.invoke` streams results and exposes tool calls.
- **Tool dispatch** – `self.session.tool.invoke` executes built-in and custom tools, merging runtime parameters with arguments from the LLM.
- **PromptTool conversion** – `_convert_tool_to_prompt_message_tool` translates the tool manifest into the schema the LLM expects.
- **Logging utilities** – `create_log_message` and `finish_log_message` capture each round of reasoning so you can monitor execution in Dify.
...
While you are in the project, follow the [Remote Debugging Playbook](/plugin-dev-en/0411-remote-debug-a-plugin) to copy `.env.example`, paste in the host and key you just captured, and relaunch the runner with `python -m main`.
