...
dimensions:
  type:
    ...
    detail: examples
  level: intermediate
standard_title: Slack Bot Plugin Blueprint
language: en
title: Slack Bot Plugin Blueprint
...
Deliver AI answers right where your team collaborates. This blueprint shows you how to pair Slack’s real-time canvas with Dify intelligence for a polished in-channel assistant.
...
Use the [Remote Debugging Playbook](/plugin-dev-en/0411-remote-debug-a-plugin) as you copy `.env.example`, add the host and key values, and launch the session with `python -m main`. The playbook walks through every dialog so you can confirm the plugin is installed in your workspace.
