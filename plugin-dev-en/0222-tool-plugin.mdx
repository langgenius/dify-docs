...
dimensions:
  type:
    ...
    detail: standard
  level: intermediate
standard_title: Build a Tool Plugin
language: en
title: Build a Tool Plugin
description: Design and ship production-grade tool plugins, using Google Search as the end-to-end blueprint.
---

Tool plugins are how you connect Dify to the rest of your product ecosystem. In this playbook you will architect, implement, and polish a tool integration that feels first-party to your users while remaining simple for your team to maintain.
...
Use the `dify plugin init` scaffolding command to generate a fresh project. The [Dify CLI Essentials](/plugin-dev-en/0111-cli) guide covers installation paths and every command-line option if you need a refresher.
...
Inside Dify, open ["Plugin Management"](https://cloud.dify.ai/plugins) and click **Debug Plugin** to grab the workspace host and debug key.

![Remote Debug Key](https://assets-docs.dify.ai/2024/12/053415ef127f1f4d6dd85dd3ae79626a.png)

Follow the steps in the [Remote Debugging Playbook](/plugin-dev-en/0411-remote-debug-a-plugin) to copy `.env.example`, drop in the workspace debug host and key, and launch `python -m main`. That workflow gives you screen-by-screen guidance for validating the plugin inside Dify.
