...
description: "This article describes how to develop a data source plugin, covering the plugin structure, code samples, debugging methods, and publishing process to help you quickly develop and launch your data source plugin."
---
...
Before reading on, ensure you have a basic understanding of the Knowledge Pipeline and some knowledge of plugin development. Here's where you can find relevant information:
...
Dify supports three types of data source plugins: online drive, online document, and website crawler. When implementing the plugin code, the class that provides the plugin's functionality must inherit from a specific data source class. Each of the three plugin types corresponds to a different parent class.
...
The return value for an online document plugin must include at least a `content` field to represent the document's content. For example:
...
In the main logic code for an online document data source plugin, the class must inherit from `OnlineDocumentDatasource` and implement two methods: `_get_pages` and `_get_content`.
