...
description: "This article introduces how to develop a data source plugin, including its structure, code samples, debugging methods, and publishing process, to help you quickly develop and launch your plugin."
---

## Prerequisites

Before reading on, ensure you have a basic understanding of the Knowledge Pipeline and some knowledge of plugin development. You can find relevant information in the following documents:
...
Data source plugins are a new type of plugin introduced in Dify 1.9.0. In the Knowledge Pipeline, they serve as the document data source and the starting point for the entire pipeline.

## **Data Source Plugin Types**

Dify supports three types of data source plugins: online drive, online doc, and website crawler. When implementing the plugin code, the class that provides the plugin's functionality must inherit from a specific data source class. Each of the three plugin types corresponds to a different parent class.
...
- **Online Doc**: Notion, Confluence, GitHub

The relationship between data source types and data source plugin types is illustrated below.

![Datasource Type And Datasource Plugin Type Pn](/images/datasource_type_and_datasource_plugin_type.png)

## Data Source Plugin Development

### Create a Data Source Plugin

Like other plugins, you can use the scaffolding command-line tool to create a data source plugin by selecting the `datasource` type.

```powershell
dify plugin init
```

![Datasource Plugin Init Pn](/images/datasource_plugin_init.png)

Typically, a data source plugin does not need to use other features of the Dify platform, so no extra permissions are required. After completing the setup, the command-line tool will automatically generate the plugin project code.

#### Data Source Plugin Structure

A data source plugin consists of three main parts:

- The `manifest.yaml` file: Describes the basic information of the plugin.
...
The content of a provider YAML file is essentially the same as for tool plugins, with only the following two differences:
...
<Info>
  To learn more about creating a provider YAML file, see [Dify Plugin Development: Hello World Guide-4.3 Configuring Provider Credentials](/plugin-dev-en/0211-getting-started-dify-tool#4-3-configuring-provider-credentials).
</Info>

<Info>
  Data source plugins support authentication via OAuth 2.0 or API keys.

  To learn more about how to configure OAuth, see [Add OAuth Support to Your Tool Plugin](/plugin-dev-en/tool-oauth).
</Info>

#### Create a Provider Code File

- When using API Key authentication mode, the provider code file for data source plugins is identical to tool plugins. You only need to change the parent class inherited by the provider class to `DatasourceProvider`.
  ...
  ```
- When using OAuth authentication mode, data source plugins differ slightly from tool plugins. When obtaining access permissions via OAuth, data source plugins can simultaneously return the username and avatar to be displayed in the frontend. Therefore, `_oauth_get_credentials` and `_oauth_refresh_credentials` need to return a `DatasourceOAuthCredentials` type that contains `name`, `avatar_url`, `expires_at`, and `credentials`.
...
The YAML file format and data source code format differ for the three types of data sources.

#### Web Crawler

In the provider YAML file for a web crawler plugin, `output_schema` must always return four parameters: `source_url`, `content`, `title`, and `description`.
...
In the main logic code for a web crawler data source plugin, the class must inherit from `WebsiteCrawlDatasource` and implement the `_get_website_crawl` method. Then, you need to use the `create_crawl_message` method to return the web crawl message.
...
The return value for an online doc plugin must include at least a `content` field to represent the document's content. For example:
...
In the main logic code for an online doc data source plugin, the class must inherit from `OnlineDocumentDatasource` and implement two methods: `_get_pages` and `_get_content`.
...
In the main logic code for an online drive data source plugin, the class must inherit from `OnlineDriveDatasource` and implement two methods: `_browse_files` and `_download_file`.

When a user runs the plugin, it first calls `_browse_files` to get a file list. At this point, `prefix` is empty, indicating a request for the root directory's file list. The file list contains both folder and file type variables. If the user proceeds to open a folder, the `_browse_files` method is called again. This time, the `prefix` in `OnlineDriveBrowseFilesRequest` will be the folder ID, used to retrieve the file list within that folder.

After a user selects a file, the plugin uses the `_download_file` method and the file ID to get the file's content. You can use the `_get_mime_type_from_filename` method to get the file's MIME type, allowing the pipeline to handle various file types differently.
...
<Info>
  For more details, refer to the specific implementations of the [official Google Drive plugin](https://github.com/langgenius/dify-official-plugins/blob/main/datasources/google_cloud_storage/datasources/google_cloud_storage.py) and the [official AWS S3 plugin](https://github.com/langgenius/dify-official-plugins/blob/main/datasources/aws_s3_storage/datasources/aws_s3_storage.py).
</Info>

## Debug the Plugin

Data source plugins support two debugging methods: remote debugging or installing as a local plugin. Note the following:

- If the plugin uses OAuth authentication, the `redirect_uri` for remote debugging is different from that of a local plugin. Update the relevant configuration in your service provider's OAuth App accordingly.
...
<Info>
  To learn about the plugin publishing process, see [Publishing Plugins](/plugin-dev-en/0321-release-overview).
</Info>